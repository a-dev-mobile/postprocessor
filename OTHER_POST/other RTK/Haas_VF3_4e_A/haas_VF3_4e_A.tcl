...
#  haas_VF3_4e_A.tcl -
#
#  Created by ronconi  @  quinta-feira, 23 de maio de 2013 09:19:15 E. South America Standard Time
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "haas_VF3_4e_A__IN.pui"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_5th_axis_center_offset(2)         "-3.048"
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   PB_CMD_verif_rot
   MOM_force Once G_motion G_mode fourth_axis
   MOM_do_template rapid_AB
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_fix G_motion G_mode X Y Z fourth_axis}
   set rapid_traverse_xy_blk {G_fix G_motion G_mode X Y fourth_axis}
   set rapid_traverse_yz_blk {G_fix G_motion G_mode Y Z fourth_axis}
   set rapid_traverse_xz_blk {G_fix G_motion G_mode X Z fourth_axis}
...
#=============================================================
   ...
   PB_CMD_def_mom_z_rot
   MOM_set_seq_off
   PB_CMD_ini_variaveis
   PB_CMD_redef_MOM_RAPID
   PB_CMD_init_lintol
   MOM_do_template start_of_program
   PB_CMD_cabecalho
   MOM_force Once G_cutcom G_plane G_motion G
   MOM_do_template absolute_mode
   MOM_force Once G_adjust G_motion G_mode G
   MOM_do_template start_of_program_1
   MOM_force Once G_feed G_return G_mode
...
#=============================================================
global mom_arc_axis   feed  mom_kin_rapid_feed_rate

if {$feed > $mom_kin_rapid_feed_rate} {set feed    $mom_kin_rapid_feed_rate}

PB_CMD_calc_invTime_feed
#-------------------------------------

if {$mom_arc_axis(2) != 0} {
    MOM_suppress once K
    MOM_force once I J
} elseif {$mom_arc_axis(1) != 0} {
          MOM_suppress once J
          MOM_force once I K
} elseif {$mom_arc_axis(0) != 0} {
          MOM_suppress once I
          MOM_force once J K
}
}
...
global mom_logname  mom_date mom_output_file_basename
global tempo_anterior
global mom_sys_control_out  mom_sys_control_in
global co ci  mom_kin_pivot_gauge_offset mom_kin_linearization_tol
...
set tempo_anterior   0
 ...
 MOM_output_literal "$co MAQUUINA : HASS VF-3 4 EIXOS *A* - REV: $usr_controle_rev - $usr_data_rev $ci"
 MOM_output_literal " "
}


#=============================================================
proc PB_CMD_calc_invTime_feed { } {
#=============================================================
#   Esta proc calcula o valor do avanco no tempo inverso (Inverse Time Feedrate),
# utilizado com o codigo G93 nas movimentacoes em 5 eixos e representa o inverso do tempo
# necessario para que o movimento seja executado por todos os eixos  envolvidos. Desta forma
# o comando define o avanco necessario para cada eixo.

#   Este avanco é calculado pela formula:
#     F = (F em mm/min)/(60 * distancia percorrida)
#
#     Nesta proc definiremos o avanço como sendo F= 1/$mom_event_time,
# onde a variavel 'mom_event_time' é o tempo calculado pelo UG para a execução de cada bloco.

#   A chamada desta proc deve ser colocada na proc "PB_CMD_before_motion"
# PB_CMD_calc_invTime_feed


global mom_event_time  feed  tipo_feed
global mom_rotary_delta_4th  mom_rotary_delta_5th

set mov_angular    [expr $mom_rotary_delta_4th + $mom_rotary_delta_5th]

#-----------------------------------------------------------------------------------------------
#-- se não hoouver variação angular no movimento, sai da proc. Caso contrário recalcula o avanço
#-----------------------------------------------------------------------------------------------
if {$mov_angular == 0} {
    if {$tipo_feed == 93} {MOM_force once G_motion}
        set tipo_feed    94
    return
} else {
    set feed  [expr 1 / $mom_event_time]
    if {$tipo_feed == 94} {MOM_force once G_motion}
    set tipo_feed    93
}

MOM_force once F fourth_axis fifth_axis
}
...
#=============================================================
proc PB_CMD_def_mom_z_rot { } {
#=============================================================
#--- inicializa variavel --------------
 set Z00    [format "%.3f" 250]

proc MOM_Z_ROT {} {
#----------------------------------
# Define o valor de retracao do eixo Z
#
global mom_Z_pos1 Z00

 set Z00    [format "%.3f" $mom_Z_pos1]
}
}


#=============================================================
proc PB_CMD_define_mom_reset_angle { } {
#=============================================================
proc MOM_reset_angle {} {
#-------------------------------
 global mom_usr_reset_AB usr_reset_AB

set usr_reset_AB    $mom_usr_reset_AB
}
}
...
#=============================================================
proc PB_CMD_feed_parameters { } {
#=============================================================
global mom_tool_number mom_spindle_rpm  mom_feed_per_tooth mom_tool_flutes_number

set max_rpm  "\[\$TOOL_DATA($mom_tool_number)MAX_RPM\]= [format %.0f $mom_spindle_rpm]"
set max_feed "\[\$TOOL_DATA($mom_tool_number)MAX_FEED\]= [format %.3f $mom_feed_per_tooth]"
set teeth    "\[\$TOOL_DATA($mom_tool_number)TEETH\]= $mom_tool_flutes_number"

MOM_output_literal $max_rpm
MOM_output_literal $max_feed
MOM_output_literal $teeth
}
...
global co ci  usr_reset_AB

set tempo_oper    [expr $mom_machine_time - $tempo_anterior]

set tempo_oper    [format %.2f $tempo_oper]
...
set tempo_u    [format %.2f $mom_machine_time]
...
#=============================================================
proc PB_CMD_force_Z_retract { } {
#=============================================================
#----------------------------------------------------------------------------------------------
# Retrai a ferramenta ate Z250 (default Z00 = 250), mas pode ser redefinido usando a UDE Z_ROT
#
# Colocar esta proc antes do RAPID_MOVE
#
MOM_output_literal "Z_retract"
global mom_rotary_delta_4th mom_rotary_delta_5th  mom_Z_pos_1 Z00

if {$mom_rotary_delta_4th != 0 || $mom_rotary_delta_5th != 0} {
     MOM_output_literal "G0 Z$Z00 (Z_rot)"
}
}
...
#=====================================================================================
# 12/jan/10    .01    Kleber        - Liberacao inicial;
#-------------------------------------------------------------------------------------
#
}
...
#=============================================================
proc PB_CMD_ini_variaveis { } {
#=============================================================
global tipo_feed  last_rpm
global clamp_flag
...
#=============================================================
global mom_path_name mom_part_name mom_date
global mom_tool_diameter mom_tool_name mom_tool_number mom_tool_corner1_radius
global mom_tool_flute_length  mom_tool_length
global mom_tool_z_offset
global co ci

  MOM_output_literal "$co OPERACAO   : $mom_path_name $ci"
  MOM_output_literal "$co FERRAMENTA : T$mom_tool_number - $mom_tool_name $ci"
}
...
#=============================================================
proc PB_CMD_init_lintol { } {
#=============================================================
#-- Ativa a linearizacao dos movimento 4-5 eixos simultaneos.
#-- OBS.: so funciona se a ferramenta for definida com ZOFF ou, para maquinas
#--  HEAD/HEAD, definindo-se o parametro PIVOT DISTANCE.
...
#=============================================================
#  Cópia do MOM_rapid_move.
#  Esta proc é chamada pela nova proc MOM_rapid_move, redefininda
# dentro da proc "PB_CMD_redef_MOM_RAPID"
#--------------------------------------------------------------
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion X Y Z fourth_axis fifth_axis S Text G_cutcom M_spindle}
   set rapid_traverse_xy_blk {G_motion X Y fourth_axis fifth_axis S Text G_cutcom M_spindle}
   set rapid_traverse_yz_blk {G_motion Y Z fourth_axis fifth_axis S Text G_cutcom M_spindle}
   set rapid_traverse_xz_blk {G_motion X Z fourth_axis fifth_axis S Text G_cutcom M_spindle}
...
#=============================================================
proc PB_CMD_redef_MOM_RAPID { } {
#=============================================================

  proc MOM_rapid_move { } {
  #=========================================================
  #   Verifica se a ferramenta está inclinada e usa o
  # template "RAPID XYZ", que deverá ser criada no TAB Definition,
  # contendo os seguintes elementos: "G0 X Y Z fouth_axis fifth_axis"
  #   A rotina original do RAPID MOVE está definida na proc
  # "PB_CMD_rapid_move"
  #
  #  Para ter efeito, colocar esta proc no inicio do programa.
  #--------------------------------------------------------------------------
    global mom_rotary_delta_4th  mom_rotary_delta_5th mom_kin_rapid_feed_rate
    global mom_programmed_feed_rate mom_pos feed mom_motion_type

    if {$mom_pos(3) != 0 || $mom_pos(4) != 0 ||
        $mom_rotary_delta_4th !=0 || $mom_rotary_delta_5th  !=0} {
        if {$mom_motion_type == "APPROACH" || $mom_motion_type == "DEPARTURE" || $mom_motion_type == "RETRACT"} {
           ...
           #MOM_do_template rapid_AB
           #MOM_output_literal ";==> ELSE... $mom_motion_type"
           PB_CMD_rapid_move
   }
   #---------------------------------------------------------------------------
}
...
#=============================================================
proc PB_CMD_reset_AB { } {
#=============================================================
global mom_out_angle_pos mom_prev_out_angle_pos

if {![exists mom_prev_out_angle_pos(0)]} {set mom_prev_out_angle_pos(0)    0}
if {![exists mom_prev_out_angle_pos(1)]} {set mom_prev_out_angle_pos(1)    0}

#set last_A    [ format %.3f $mom_prev_out_angle_pos(0) ]
#set last_B    [ format %.3f $mom_prev_out_angle_pos(1) ]
set last_A    [expr (int($mom_prev_out_angle_pos(0)*100)/100)]
set last_B    [expr (int($mom_prev_out_angle_pos(1)*100)/100)]

if {$last_A != 0 && $last_B != 0} {
    PB_CMD_unclamp_fourth_axis
    PB_CMD_unclamp_fifth_axis
    MOM_output_literal "G93 G1 A0.0 B0.0 F6000."
    PB_CMD_clamp_fourth_axis
    PB_CMD_clamp_fifth_axis
}
}
...
#=============================================================
  MOM_force once S M_spindle X Y Z F fourth_axis fifth_axis
}
...
#=============================================================
proc PB_CMD_verif_angulos_AB { } {
#=============================================================
global mom_out_angle_pos  clamp_flag
...
if {$mom_out_angle_pos(0) != 0 || $mom_out_angle_pos(1) != 0 } {
    ...
    MOM_output_literal "G93 G1 A0.0 F6000."
    if {$clamp_flag == 0} {
        PB_CMD_clamp_fourth_axis
        PB_CMD_clamp_fifth_axis
    set clamp_flag    1
...
#=============================================================
proc PB_CMD_verif_multi_axis { } {
#=============================================================
global mom_operation_type clamp_flag

if {[string range $mom_operation_type 0 4] == "Seque" || [string range $mom_operation_type 0 4] == "Varia"} {
    if {$clamp_flag == 1} {
        PB_CMD_unclamp_fourth_axis
        PB_CMD_unclamp_fifth_axis
    set clamp_flag    0
    }
    MOM_output_literal "(---- eixos destravados ----)"
} else {
    if {$clamp_flag    == 0} {
            PB_CMD_clamp_fourth_axis
            PB_CMD_clamp_fifth_axis
        MOM_output_literal "(---- eixos travados ----)"
            set clamp_flag    1
        }
}

MOM_force once G_feed  G
}


#=============================================================
proc PB_CMD_verif_plano_interpolacao { } {
#=============================================================
}


#=============================================================
proc PB_CMD_verif_rot { } {
#=============================================================
global mom_spindle_speed last_rpm
global mom_motion_event

if {$mom_motion_event == "initial_move"} {
    MOM_force once G_fix S M_spindle

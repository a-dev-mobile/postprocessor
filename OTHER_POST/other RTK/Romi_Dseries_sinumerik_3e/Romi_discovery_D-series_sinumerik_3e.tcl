########################## TCL Event Handlers ##########################
#
#  Romi_discovery_D-series_sinumerik_3e.tcl -
#
#  Created by ronconi  @  terça-feira, 31 de julho de 2012 08:58:22 E. South America Standard Time
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_output_file_suffix                "dnc"
...
#=============================================================
   ...
   MOM_force Once G_motion G Z D M_spindle
...
#=============================================================
   ...
   PB_CMD_check_cycle87
   PB_call_macro CYCLE87
...
#=============================================================
  ...
  # First tool only gets handled once
   ...
   MOM_output_literal ";\#### T: $mom_tool_number - FERRAMENTA: $mom_tool_name"
   MOM_force Once T
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_plane G_motion G_mode G X Y Z M_coolant}
   set rapid_spindle_x_blk {G_plane G_motion G_mode G X M_coolant}
   set rapid_spindle_y_blk {G_plane G_motion G_mode G Y M_coolant}
   set rapid_spindle_z_blk {G_plane G_motion G_mode G Z M_coolant}
   set rapid_traverse_blk {G_plane G_motion G_mode G X Y Z M_coolant}
   set rapid_traverse_xy_blk {G_plane G_motion G_mode G X Y M_coolant}
   set rapid_traverse_yz_blk {G_plane G_motion G_mode G Y Z M_coolant}
   set rapid_traverse_xz_blk {G_plane G_motion G_mode G X Z M_coolant}
...
#=============================================================
   ...
   MOM_set_seq_on
   PB_CMD_cabec
   MOM_force Once G_plane G_mode G Text
...
set usr_partname     [lindex [split $mom_part_name "\\"] end]

 MOM_output_literal "$co\## PRODUTO    : [string toupper $usr_partname] $ci"
 MOM_output_literal "$co\## RESPONSAVEL: [string toupper $mom_logname] $ci"
 MOM_output_literal "$co\## DATA       : [string toupper $mom_date] $ci"
 MOM_output_literal "$co\## MAQUINA    : ROMI D SERIES - SINUMERIK - 3EIXOS $ci"
 MOM_output_literal "$co\## PP. REV.   : $usr_controle_rev  - $usr_data_rev $ci"
...
set tempo_anterior   0
 ...
 MOM_output_literal "$co TECNICO  : [ string toupper $mom_logname] $ci"
 MOM_output_literal "$co DATA     : [ string toupper $mom_date] $ci"
 MOM_output_literal "$co ARQUIVO  : [ string toupper $mom_part_name] $ci"
 MOM_output_literal "$co MAQUINA  : DISCOVERY 760 - SIEMENS 810D 3eixos $ci"

# MOM_output_literal "$co POSPROCESSADOR: [ string toupper $mom_event_handler_f
 set tr_flag    0
}
...
global mom_cycle_delay_mode mom_tool_text
...
if {$cycle_flag == 0} {
    ...
    } else { set usr_PIT    [expr -1 * $mom_cycle_feed_rate_per_rev] }
    set usr_POSS   0
    set usr_SST    $mom_spindle_speed

   if {[info exists mom_tool_text] && $mom_tool_text > 0} {
       set usr_SST1    $mom_tool_text
   } else {
    ...
    MOM_force once X Y fourth_axis
        ...
        if {$mom_spindle_direction ==  "CLW"} {
            ...
            set cycle_flag    0
                ...
                if {$usr_PIT != [expr -1 * $mom_cycle_feed_rate_per_rev]} {
                    set usr_PIT    [expr -1 * $mom_cycle_feed_rate_per_rev]
                    set cycle_flag    0
                }
        }
...
global mom_cycle_delay_mode mom_feed_return_value
...
if {$cycle_flag == 0} {
    ...
    set usr_DTB    $_DTB
    set usr_FFR    $feed
    set usr_RFF    $mom_feed_return_value
...
if {$mom_fixture_offset_value > 6 } {
    MOM_abort "==> FIXTURE OFFSET deve ser menor que 7 !! "
}
...
#=============================================================
MOM_force once G_motion X Y fourth_axis D F G G_plane
...
#==============================================================================
# 30/JUL/12     00    Kleber            - liberação inicial
#-----------------------------------------------------------------------------------
#
...
#MOM_output_literal "$co\#### T: $mom_tool_number - FERRAMENTA: $mom_tool_name $ci"

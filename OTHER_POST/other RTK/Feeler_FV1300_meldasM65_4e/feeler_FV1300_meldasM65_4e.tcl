########################## TCL Event Handlers ##########################
#
#  feeler_FV1300_meldasM65_4e.tcl - 4_axis_table
#
#    This is a 4-Axis Milling Machine With
#     Rotary Table.
#
#  Created by ronconi @ sexta-feira, 5 de janeiro de 2018 16:11:29 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "feeler_FV1300_meldasM65_4e__IN.pui"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_min_arc_radius                    "0.3"
...
#=============================================================
   ...
   MOM_force Once G_motion G Z
   MOM_do_template end_of_program_1
   MOM_force Once G_motion G Y
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_destrava_eixoA
   set rapid_spindle_blk {G_adjust G_motion X Y Z H M_coolant}
   set rapid_spindle_x_blk {G_adjust G_motion X H M_coolant}
   set rapid_spindle_y_blk {G_adjust G_motion Y H M_coolant}
   set rapid_spindle_z_blk {G_adjust G_motion Z H M_coolant}
   ...
   PB_CMD_verifica_travamento_eixoA
}
...
#=============================================================
   ...
   MOM_set_seq_on
   PB_CMD_cabecalho
   MOM_force Once G_cutcom G_plane G_feed G_motion G_mode G
...
set tempo_anterior   0
 ...
 MOM_output_text "O001 $co$mom_output_file_basename$ci"
 ...
 MOM_output_literal "$co MAQUINA  : FEELER 4E - COMANDO: MITSUBISHI MELDAS M65 $ci"
...
#=============================================================
proc PB_CMD_destrava_eixoA { } {
#=============================================================
global lock_A  mom_rotary_delta_4th

if {$lock_A == "ON" && $mom_rotary_delta_4th != 0} {
...
#=============================================================
proc PB_CMD_force_circular_coord { } {
#=============================================================
...
#==============================================================================
# 10/AGO/05     A    Kleber        - Acrescentada na proc PB_CMD_before_motion
#                    verificacao de componente do TLAXIS em X.
#                    Se houver, mostra janela de mensagem e em
#                    seguida aborta o pos-processamento;
#                    - Inserido arquivo de janela de mensagem
#                    "feeler_FV1300_meldasM65_4e_t_user.tcl";
#------------------------------------------------------------------------------
# 06/DEZ/06     B    Kleber        - Removido o arquivo de janela de mensagem
#                    "feeler_FV1300_meldasM65_4e_t_user.tcl";
#                    - Criada a proc PB_CMD_verif_tlaxis que,
#                    se houver componente do TLAXIS em X, exibe
#                    uma mensagem na tela com os valores dos
#                    componentes do TLAXIS e o nome da operação.
#                    Em seguida aborta o pós-processamento.
#                    - Acresc. home position no final do prog.
#------------------------------------------------------------------------------
# 18/SET/07     C    Kleber        - Acresc. M26 e M25 na proc PB_CMD_fourth_axis_rotate_move
#------------------------------------------------------------------------------
}
...
#--- verifica se eh usinagem 4 eixos simultaneos ou nao ----
global mom_operation_type
global op_type

set op_lst    [split $mom_operation_type " "]
set op_type    [lindex $op_lst    0]
#---------------------------------------------------------
}
...
#=============================================================
proc PB_CMD_verif_cutcom { } {
#=============================================================
global mom_cutcom_status  mom_sys_cutcom_code
global mom_tool_number Tnumber

if {$mom_cutcom_status == "OFF" || $mom_cutcom_status == "UNDEFINED"} {
    set Tnumber    ""
} else {
        if {$Tnumber == ""} {MOM_force once D}
        set Tnumber    $mom_tool_number
}
}


#=============================================================
proc PB_CMD_verif_tlaxis { } {
#=============================================================
 ...
 global mom_tool_axis mom_path_name

 #-- Verifica se o tool axis tem componente em X
 set cx    0.001
 if {$mom_tool_axis(0) > $cx} {
     MOM_close_output_file $ptp_file_name

     MOM_output_to_listing_device "  "
     MOM_output_to_listing_device "  "
     MOM_output_to_listing_device ";===> ------------------------------------------"
     MOM_output_to_listing_device ";===>  NOME DA OPERACAO: $mom_path_name "
     MOM_output_to_listing_device ";===> ------------------------------------------"
     MOM_output_to_listing_device ";===>   ATENCAO: TLAXIS TEM COMPONENTE EM X  "
     MOM_output_to_listing_device ";===>            MAIOR QUE LIMITE ( $cx )"
     MOM_output_to_listing_device ";===> ------------------------------------------"
     MOM_output_to_listing_device ";===>  TLAXIS : I =$mom_tool_axis(0) <=="
     MOM_output_to_listing_device ";===>           J =$mom_tool_axis(1) "
     MOM_output_to_listing_device ";===>           K =$mom_tool_axis(2) "
     MOM_output_to_listing_device ";===> ------------------------------------------"
     MOM_output_to_listing_device ";===>     ABORTANDO POS-PROCESSAMENTO !!! "
     MOM_output_to_listing_device ";===> ------------------------------------------"
     MOM_output_to_listing_device "  "

  #--- Aborta o posprocessadmento
  MOM_abort ";===> ATENCAO: TLAXIS TEM COMPONENTE EM X!! <==="
}
}


#=============================================================
proc PB_CMD_verifica_travamento_eixoA { } {
#=============================================================
global op_type  lock_A

if {$lock_A == "OFF"} {
    if {$op_type != "Sequential" && $op_type != "Variable-axis"} {
        PB_CMD_clamp_fourth_axis
    }
} elseif {$lock_A == "ON"} {
       if {$op_type == "Sequential" || $op_type == "Variable-axis"} {

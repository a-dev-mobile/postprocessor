########################## TCL Event Handlers ##########################
#
#  MAZAK_QTN350_MATRIX.tcl -
#
#  Created by ronconi  @  segunda-feira, 8 de outubro de 2012 09:22:17 E. South America Standard Time
...
#=============================================================
   ...
   MOM_do_template tap
   MOM_force Once G_motion X Z F_tap M_spindle
...
#=============================================================
   ...
   MOM_set_seq_off
   PB_CMD_init_var
   MOM_output_literal "(MAZAK QTN350 MATRIX - REV. 01 - 08OUT12)"
   MOM_set_seq_on
   MOM_force Once G_cutcom G_plane G_return G_motion G_mode G
...
#=============================================================
proc PB_CMD_check_home { } {
#=============================================================
global XHome_pos ZHome_pos usr_msg1
global mom_XHome_pos mom_ZHome_pos

if {![info exists flag_home]} {
    set flag_home    0
    set usr_msg1 ""
}

if {[info exists mom_XHome_pos]} {
    set XHome_pos    $mom_XHome_pos
    set ZHome_pos    $mom_ZHome_pos
    set flag_home    1
    set usr_msg1 " (ATENCAO: NOVO HOME DEFINIDO)"
} else {set flag_home    0}
}
...
set tap_rpm   $mom_sys_spindle_direction_code($mom_spindle_direction)
...
#=============================================================
proc PB_CMD_conta_peca { } {
#=============================================================
MOM_output_text "\#3901=\#3901+1"
MOM_output_text "M99"
MOM_output_text "N90000"
}


#=============================================================
proc PB_CMD_define_zero_peca { } {
#=============================================================
...
if {$mom_fixture_offset_value > 1} {
    set zero_peca    "G53.5 \#Z[expr 99 + $mom_fixture_offset_value]"
}
}
...
#
# Mazak QT-20 QT-40 ST-60 / Mazatrol.
#
#
# Revisions
#
...
global mom_part_name mom_program_name
global mom_logname  mom_group_name
...
set mom_machine_name qtn150

if {![info exists mom_group_name]} {
    set descr1    " "
} else {set descr1    $mom_group_name}

 #MOM_output_literal "$mom_sys_rewind_stop_code"
 MOM_output_literal " "
 MOM_output_literal "O[string toupper $mom_output_file_basename]($descr1)"
 MOM_output_literal "(ARQUIVO     : [string toupper $mom_part_name])"
 MOM_output_literal "(PROGRAMADOR : [string toupper $mom_logname])"
 MOM_output_literal "(MAQUINA     : MAZAK QTN350 MATRIX)"
 MOM_output_literal "(REV.PP      : 01 - 08/OUT/12)"
 MOM_output_literal "(DATA        : [string toupper $user_date])"

# MOM_output_text "IF\[\#3901GE\#3902\]GOTO 90000"
# MOM_output_text "M58"
}


#=============================================================
proc PB_CMD_init_var { } {
#=============================================================
global XHome_pos ZHome_pos
global mom_sys_home_pos

set XHome_pos    $mom_sys_home_pos(0)
set ZHome_pos    $mom_sys_home_pos(2)
}
...
#=============================================================
  ...
  global udc_spindle_range_code
  global mom_spindle_speed

#  if {$mom_spindle_speed <= 50} {
#     set udc_spindle_range_code 41
#  } elseif {$mom_spindle_speed > 50} {
#     set udc_spindle_range_code 42
#  }

  if {$mom_spindle_mode == "RPM"} {
     MOM_force once M_spindle S G_spin M_range
     MOM_do_template spindle_rpm
  } elseif {$mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM"} {
     MOM_force once M_spindle S G G_spin M_range
...
MOM_output_literal "(FERRAMENTA: T$mom_tool_number $mom_tool_name)"
MOM_output_literal "  "
...
#=============================================================
proc PB_CMD_thread_motion { } {
#=============================================================
global mom_operation_type

if {$mom_operation_type == "Turn Threading"} {
MOM_suppress once F G_motion
...
#=============================================================
proc PB_CMD_tool_check { } {
#=============================================================
global mom_tool_number    last_tool_number

if {![info exists last_tool_number]} {
    set last_tool_number    9999999999
}

if {$mom_tool_number != $last_tool_number} {
    MOM_force once G_motion G W U
    MOM_do_template end_of_path_1
}

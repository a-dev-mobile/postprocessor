########################## TCL Event Handlers ##########################
#
#  romi_centur_30D_fanuc.tcl - lathe
#
#    This is a 2-Axis Horizontal Lathe Machine.
#
#  Created by ronconi @ sexta-feira, 5 de janeiro de 2018 10:56:34 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "romi_centur_30D_fanuc__IN.pui"
...
#=========================
 ...
 set mom_sys_master_head                       "TURN_XZ"

 set mom_sys_postname(TURN_XZ)                 "$mom_sys_master_post"
 set mom_sys_postname(MILL)                    "Tacchi_HD3_120L-mill_Fanuc31i_in"
 set mom_sys_postname(TURN_UW)                 "Tacchi_HD3_120L-UW_Fanuc31i_in"
...
#=============================================================
   MOM_do_template start_of_HEAD__MILL
}


#=============================================================
proc PB_start_of_HEAD__TURN_UW { } {
#=============================================================
   MOM_do_template start_of_HEAD__TURN_UW
}


#=============================================================
proc PB_start_of_HEAD__TURN_XZ { } {
#=============================================================
   MOM_do_template start_of_HEAD__TURN_XZ
}
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   global mom_operation_name
   MOM_output_literal "(OPER: $mom_operation_name)"
   global mom_tool_description
   MOM_output_literal "(FERR: $mom_tool_description)"
   PB_CMD_spindle_sfm_prestart
   PB_CMD_coolant_check
   PB_CMD_force_adress_ini_oper
...
proc PB_CMD_TLength { } {
#=============================================================
# Força sair troca de ferramenta
PB_auto_tool_change
}
...
#=============================================================
global mom_coolant_status mom_operation_type mom_coolant_mode

if {$mom_operation_type == "Lathe Machine Control"} {
    set mom_coolant_status    "UNDEFINED"
    set mom_coolant_mode     "UNDEFINED"
    return
}
...
if {$mom_coolant_status == "UNDEFINED"} {
    ...
    MOM_reload_variable mom_coolant_status
}
...
#=============================================================
proc PB_CMD_final_oper { } {
#=============================================================
global usr_mach_time mom_machine_time usr_tool_time
global mom_next_oper_has_tool_change

set oper_mach_time    [expr $mom_machine_time - $usr_mach_time]
set usr_mach_time    $mom_machine_time

if {![info exists usr_tool_time]} {set usr_tool_time    0}
set usr_tool_time    [expr $usr_tool_time + $oper_mach_time]

if {$oper_mach_time < 0.01} {return}

if {$mom_next_oper_has_tool_change == "YES" } {
    MOM_output_literal "(----------------------------------------------------)"
    MOM_output_literal "( TEMPO DESTA FERRAMENTA: [format %.1f $usr_tool_time]MIN.)"
    MOM_output_literal "(----------------------------------------------------)"
...
#=============================================================
proc PB_CMD_force_adress_ini_oper { } {
#=============================================================
...
#=============================================================
#
#  Gera cabecalho do programa com as seguintes informações:
#
#        - Nome do programa:  Oxxxx -> usa o nome do programa do NX;
#        - Revisão do pós-processador;
#        - Nome do arquivo PRT e Versão do NX;
#        - Data e hora;
#        -

global mom_date mom_output_file_basename
global mom_attr_PROGRAMVIEW_PART_NAME
global mom_group_name mom_part_name mom_ug_version
global mom_parent_group_name
global usr_mach_time

set usr_mach_time    0

#--- Define nome do programa -----------------------------------------
set usr_group_name "O0001"
set usr_group_name "O$mom_output_file_basename"
if {[info exists mom_parent_group_name]} {
    set ini_name    [string range $mom_parent_group_name 0 0]
    if {$ini_name == "O"} {
    set usr_group_name    "$mom_parent_group_name"
    }
}

#------ Define Nome do arquivo NX -------------------------------------
set usr_partname    [lindex [split $mom_part_name "\\"] end]

#------ Define Data e Hora --------------------------------------------
set usr_data        "DATA. "
append usr_data        [lindex [split $mom_date " "] 2]
append usr_data        "/"
append usr_data        [string toupper [lindex [split $mom_date " "] 1]]
append usr_data        "/"
append usr_data        [lindex [split $mom_date " "] 4]
append usr_data        " - HORA: "
append usr_data        [lindex [split $mom_date " "] 3]

#---- Header Output ---------------------------------------------------
MOM_output_literal "$usr_group_name"
MOM_output_literal "(MAQUINA. ROMI CENTUR 30D - FANUC )"
MOM_output_literal "(PP. REV. 02 - 07/NOV/12 - KLEBER)"
#--- REV ANTERIORES ---------------------------------------------------
#MOM_output_literal "(PP. REV. 01 - 30/OUT/12 - KLEBER)"
#MOM_output_literal "(PP. REV. 00 - 28/SET/12 - KLEBER)"
#


#kr MOM_output_literal "(ARQUIVO. [string toupper $usr_partname] - $mom_ug_version)"

if {![info exists mom_attr_PROGRAMVIEW_PART_NAME]} {set mom_attr_PROGRAMVIEW_PART_NAME $usr_partname}
MOM_output_literal "($mom_attr_PROGRAMVIEW_PART_NAME)"

MOM_output_literal "($usr_data)"
}
...
#=============================================================
proc PB_CMD_restore_seqnum { } {
#=============================================================
global mom_seqnum save_mom_seqnum

set mom_seqnum $save_mom_seqnum


MOM_force once G_motion X Z F
}
...
#=============================================================
proc PB_CMD_set_seqnum { } {
#=============================================================
global mom_seqnum save_mom_seqnum
global pop_tool_change_n_code

if {![info exists mom_seqnum]} {set mom_seqnum    1}

set save_mom_seqnum $mom_seqnum
set mom_seqnum $pop_tool_change_n_code

#--- Gera numero de bloco de Referencia de troca de ferramenta
if {$mom_seqnum < 10} {
    MOM_output_text "N00$mom_seqnum"
} else {MOM_output_text "N0$mom_seqnum"}

incr pop_tool_change_n_code
}


#=============================================================
proc PB_CMD_spindle_range { } {
#=============================================================
global mom_spindle_range

if {![info exists mom_spindle_range] || $mom_spindle_range < 41} {
      set mom_spindle_range    42
}
}
...
#=============================================================
  global mom_spindle_mode mom_spindle_rpm spindle_is_out
   ...
   if { ![string compare "SFM" $mom_spindle_mode] || ![string compare "SMM" $mom_spindle_mode] } {
      ...
      if {$mom_spindle_rpm == 0} {
          MOM_output_literal "(===> ROTACAO MAX. NAO DEFINIDA <===)"
          return
      }
      #MOM_output_literal "########$mom_spindle_mode##########"
      MOM_do_template spindle_rpm_preset M_spindle
...
#=============================================================
proc PB_CMD_tempo_final { } {
#=============================================================
global usr_mach_time mom_machine_time  usr_tool_time

if {[format %.2f $usr_tool_time] != [format %.2f $mom_machine_time]} {
    MOM_output_literal "(---------------------------------------------------------)"
    MOM_output_literal "( TEMPO DESTA FERRAMENTA: [format %.1f $usr_tool_time]MIN.)"
}

if {$mom_machine_time > 60} {
    set time    [expr $mom_machine_time / 60]

    MOM_output_literal "(====================================================)"
    MOM_output_literal "( TEMPO TOTAL de USINAGEM: [format %.1f $time]H.)"
    MOM_output_literal "(====================================================)"
} else {
    MOM_output_literal "(====================================================)"
    MOM_output_literal "( TEMPO TOTAL de USINAGEM: [format %.2f $mom_machine_time]MIN.)"
    MOM_output_literal "(====================================================)"
}
}
...
#=============================================================
proc PB_CMD_thread_type { } {
#=============================================================
global mom_total_depth mom_total_depth_tolerance  cycle_init_flag
global mom_prev_pos mom_pos usr_conic usr_Q

#MOM_output_literal "==> $mom_total_depth | $mom_total_depth_tolerance"
#-----------------------------------------------------------------------------
 if {$mom_total_depth_tolerance  == 0} {
     MOM_force once G_motion X Z F
     MOM_do_template thread_move_G32
 } else {
         MOM_do_template thread_move_G76_UN
         set usr_conic    [expr [format %.3f $mom_pos(0)] - [format %.3f $mom_prev_pos(0)]]
         set usr_Q        [expr ($mom_total_depth * 1000) / sqrt($mom_total_depth/$mom_total_depth_tolerance)]

#MOM_output_literal "==> usr_Q=$usr_Q"
         MOM_force once G_motion X Z user_thread_depth user_thread_ini_pass F
         MOM_do_template thread_move_G76
 }
...
#    MOM_do_template thread_move_G32
#} else {
#        if {$mom_total_depth_tolerance == $mom_total_depth} {
#             MOM_do_template thread_move_G76_ACME
#        } else {
#                MOM_do_template thread_move_G76_UN
#        }
#       MOM_force once G_motion X Z user_thread_depth user_thread_ini_pass F
...
#=============================================================
proc PB_CMD_tool_change_n_code { } {
#=============================================================
global pop_tool_change_n_code
set pop_tool_change_n_code 1
}


#=============================================================
proc PB_CMD_usr_coolant_off { } {
#=============================================================

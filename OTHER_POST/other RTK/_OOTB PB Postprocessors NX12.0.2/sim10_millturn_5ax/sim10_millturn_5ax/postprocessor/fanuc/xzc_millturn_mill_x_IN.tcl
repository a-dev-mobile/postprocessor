########################## TCL Event Handlers ##########################
#
#  xzc_millturn_mill_x_IN.tcl - 3_axis_mill_turn
#
#    This is a 3-Axis Milling Machine.
#
#  Created by w5ygvn @ Friday, March 03, 2017 2:13:30 PM China Standard Time
...
# 01-06-2017 szl - Resaved with PB v11.0.1.
  ...
  set mom_sys_alt_unit_post_name                "xzc_millturn_mill_x_IN__MM.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_cycle_bore_code                   "87"
  ...
  set mom_sys_lathe_postname                    "mori_millturn_lathe"
...
#=============================================================
   ...
   MOM_force Once F X Z fourth_axis
   MOM_do_template cycle_bore

   MOM_do_template cycle_parameters_3
...
#=============================================================
   ...
   PB_CMD_go_home_end_of_op

   MOM_do_template end_of_path_4
}
...
#=============================================================
proc PB_CMD_go_home_end_of_op { } {
#=============================================================
MOM_output_text "G28 X0.0"
MOM_output_text "G28 Z0.0"
}
...
#=============================================================
proc PB_CMD_select_spindle { } {
#=============================================================
global mom_select_head
# use  program group name as program ID
if { [info exists mom_select_head] } {
  set spndl M98P8200
  } else {
  set spndl M98P8300
  }
...
#
   ...
   if {[info exists mom_sys_rotary_error] && [info exists mom_motion_event]} {
     if {$mom_sys_rotary_error != 0 && $mom_motion_event == "linear_move"} {
         global mom_sys_retract_plane mom_sys_reengage_distance
         global mom_sys_rotary_reengage_feedrate
        ...
        global mom_sys_retract_type
        global mom_sys_retract_distance
         switch $mom_sys_retract_type {
             DISTANCE {
            set mom_pos(0) [expr $mom_prev_pos(0)+$mom_sys_retract_distance*$mom_sys_spindle_axis(0)]
            set mom_pos(1) [expr $mom_prev_pos(1)+$mom_sys_retract_distance*$mom_sys_spindle_axis(1)]
            set mom_pos(2) [expr $mom_prev_pos(2)+$mom_sys_retract_distance*$mom_sys_spindle_axis(2)]
               set num_sol 1
             }
             PLANE -
             default {
               ...
               if {$mom_kin_4th_axis_type == "Table"} {
                  set num_sol [CALC_CYLINDRICAL_RETRACT_POINT mom_prev_pos mom_kin_spindle_axis $mom_sys_retract_plane ret_pt]
               } else {
                  set num_sol [CALC_SPHERICAL_RETRACT_POINT mom_prev_pos mom_prev_tool_axis cen $mom_sys_retract_plane ret_pt]
               }
         ...
         set dist [expr $mom_sys_reengage_distance*2.0]
...
#
         ...
         #
            GET_SPINDLE_AXIS mom_prev_tool_axis
            for {set i 0} {$i < 3} {incr i} {
               set mom_pos($i) [expr $mom_prev_pos($i) + $mom_sys_reengage_distance*$mom_sys_spindle_axis($i)]
            }
            set mom_feed_rate $mom_feed_approach_value
            if {[EQ_is_equal $mom_feed_rate 0.0]} {set mom_feed_rate $mom_kin_rapid_feed_rate}
            set dist [expr $dist - $mom_sys_reengage_distance]
         ...
         #
            ...
            FEEDRATE_SET
            VEC3_sub mom_pos mom_prev_pos del_pos
            set mom_feed_rate_number [expr ($mom_sys_frn_factor*$mom_feed_rate) / $mom_sys_reengage_distance ]
...
#
         ...
         #
            ...
            for {set i 0} {$i < 3} {incr i} {
               set mom_pos($i) [expr $mom_prev_alt_pos($i) + $mom_sys_reengage_distance * $mom_sys_spindle_axis($i)]
            }
            MOM_suppress once fourth_axis fifth_axis
            set mom_feed_rate $mom_feed_approach_value
            if {[EQ_is_equal $mom_feed_rate 0.0]} {set mom_feed_rate $mom_kin_rapid_feed_rate}
            set rdist [expr $dist - $mom_sys_reengage_distance]
            if {$rdist <= 0.0} {set rdist $mom_sys_reengage_distance}
         ...
         #
            ...
            VEC3_sub mom_pos save_pos del_pos
            set dist [VEC3_mag del_pos]
            if {$dist <= 0.0} {set dist $mom_sys_reengage_distance}
            set mom_feed_rate_number [expr ($mom_sys_frn_factor*$mom_feed_rate)/ $dist]
            FEEDRATE_SET
...
global mom_sys_lock_arc_save
global mom_kin_arc_output_mode
if {![info exists mom_sys_lock_arc_save]} {set mom_sys_lock_arc_save $mom_kin_arc_output_mode}

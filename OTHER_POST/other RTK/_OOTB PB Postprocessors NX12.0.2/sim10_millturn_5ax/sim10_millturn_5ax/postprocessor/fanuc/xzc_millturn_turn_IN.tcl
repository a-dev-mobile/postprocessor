########################## TCL Event Handlers ##########################
#
#  xzc_millturn_turn_IN.tcl - lathe
#
#    
#
#  Created by w5ygvn @ Friday, March 03, 2017 2:13:40 PM China Standard Time
...
# 01-06-2017 szl - Resaved with PB v11.0.1.
  ...
  set mom_sys_alt_unit_post_name                "xzc_millturn_turn_IN__MM.pui"
...
#=============================================================
   MOM_do_template cutcom_off
   PB_CMD_spindle_rpm_reset

   MOM_force Once G
   MOM_do_template x_axis_home_g28
...
#=============================================================
  ...
  global mom_sys_machine_time mom_machine_time
   ...
   MOM_force Once Text G_cutcom G_plane G_motion
...
proc PB_CMD_gohome { } {
#=============================================================
global mom_gohome_status

if { $mom_gohome_status == 0 } {

            # only put this out if the gohome event has been forgotten and the

            # tool is actually changed



            if { [string toupper $mom_next_oper_has_tool_change] == "YES" } {

                        MOM_gohome_move ;# Force out the gohome even if it is missing

                        set mom_gohome_status 1
...
#=============================================================
proc PB_CMD_init_turn { } {
#=============================================================
  global force_G92_block_once
  set    force_G92_block_once 0

  global force_thread_taper_direction
  set force_thread_taper_direction 0
...
global mom_spindle_direction_code

if { [info exists mom_head_name] } {

    if { $mom_head_name == "TURN_LOWER_MAIN" } {
        set mom_spindle_direction_code(CW)  3
        set mom_spindle_direction_code(CCW) 4
        set mom_spindle_direction_code(OFF) 5
    }

  if { $mom_head_name == "TURN_LOWER_SUB" } {
        set mom_spindle_direction_code(CW)  103
        set mom_spindle_direction_code(CCW) 104
        set mom_spindle_direction_code(OFF) 105
    }
...
#=============================================================
proc PB_CMD_spindle_rpm_reset { } {
#=============================================================
global mom_spindle_mode

if [ info exists mom_spindle_mode ] {
   if { $mom_spindle_mode != "RPM" } {
     MOM_do_template spindle_rpm_reset
     }
  }
}
...
if { $mom_head_name == "TURN_LOWER_SUB" } {
    ...
    set mom_sys_spindle_direction_code(OFF) 104
}
}
...
global mom_thread_taper_direction
set mom_thread_taper_direction [expr $mom_prev_pos(0)-$mom_pos(0)]

#MOM_ouput_literal "$mom_thread_taper_direction"

if { [EQ_is_equal $mom_thread_taper_direction 0 ] } {
     MOM_suppress once thread_taper_direction
}

########################## TCL Event Handlers ##########################
#
#  sim15_millturn_9ax_sinumerik_mm.tcl - 5_axis_head_table
#
#    This is a 5-Axis Milling Machine With
#    Rotary Head and Table.
#
#  Created by lili @ Thursday, November 16, 2017 10:37:51 AM China Standard Time
...
# 17-Jul-2014 Allen - Enhance CYCLE97:Add marco CYLCE97.
# 30-Mar-2015 Lili  - Add Drilling operation type check in PB_CMD_control_soft_limit.
# 27-Apr-2015 Lili  - Enhance operation type check in PB_CMD_control_soft_limit.
# 22-Dec-2015 szl   - Add PB_CMD_spindle_off and PB_CMD_coolant_on to enhance back counter sinking and deep hole drilling.
# 29-Mar-2016 szl   - Updated PB_CMD_coolant_on. Update mom_coolant_mode if operation doesn't attach UDE, otherwise it won't work in turbo mode.
...
# 01-Jul-2016 szl   - Update PB_CMD_detect_operation_mode to avoid the regression -- only in channel 1 need to check if operation is a 5 axis operation
#                     Add missing backslashs for Macro COUPDEF in PB_DEFINE_MACROS.
...
#                     3. Adjust some codes format in this TCL file to make the logic and description more clear.
#
}
  ...
  set mom_sys_alt_unit_post_name                "sim15_millturn_9ax_sinumerik_mm__IN.pui"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   if { [PB_CMD__check_block_spindle_off_end_path] } {
      MOM_do_template spindle_off_end_path
   }
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   if { [PB_CMD__check_block_Tool_change_head] } {
      PB_call_macro Tool_change_head
   }

   MOM_do_template initial_move
   PB_CMD_set_transmit_mode
   ...
   if { [PB_CMD__check_block_g59] } {
      MOM_force Once Text X Z
      MOM_do_template g59
   }
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   if { [PB_CMD__check_block_tool_change_turret] } {
...
#=============================================================
   PB_CMD_handle_subspindle_motion
}
...
#=============================================================
proc MOM_ude_cancel_spindle_sync { } {
#=============================================================
   PB_CMD_cancle_spindle_synchronous
}


#=============================================================
proc MOM_ude_set_spindle_sync { } {
#=============================================================
...
#=============================================================
   ...
   MOM_do_template workpiece_takeover
   PB_CMD_calculate_R200

   MOM_do_template workpiece_takeover_4

   MOM_do_template workpiece_catch_1

   MOM_do_template workpiece_catch_3

   MOM_force Once G_motion Text Text
   MOM_do_template workpiece_catch

   MOM_force Once G_motion G part_transfer_approach
   MOM_do_template workpiece_takeover_1

   MOM_force Once G_motion G F fxs fxst fxsw
   MOM_do_template workpiece_takeover_2
   PB_CMD_catch_sub_spindle

   MOM_do_template workpiece_catch_2
}
...
#=============================================================
   ...
   MOM_do_template workpiece_transfer
   PB_CMD_reset_part_transfer_flag

   MOM_force Once G_motion G z3puls F fxs
   MOM_do_template workpiece_transfer_7

   MOM_force Once G_motion G z3
   MOM_do_template workpiece_transfer_8
}
...
return
  ...
  if {[info exist mom_coordinate_output_mode] && $mom_coordinate_output_mode == "TRANSMIT"} {
     global mom_channel_id mom_fixture_offset_value
     if {$mom_channel_id == 2} {
        if {$mom_fixture_offset_value == 2} {
           MOM_output_literal "TRANS C3=0"
        } else {
           MOM_output_literal "TRANS C4=0"
        }
     }
...
return
 ...
 #Reset C axis leader for milling and drilling operations
   if {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value == 2 } {
       if { [string match "Point to Point" $mom_operation_type] || [string match "Hole Making" $mom_operation_type] || [string match "Drilling" $mom_operation_type] } {
           ...
           set mom_sys_leader(fifth_axis) "C3="
           set mom_sys_trailer(fifth_axis) ""
       }
   } else {
       if { [string match "Point to Point" $mom_operation_type] || [string match "Hole Making" $mom_operation_type]  || [string match "Drilling" $mom_operation_type] } {
          ...
          set mom_sys_leader(fifth_axis) "C4="
          set mom_sys_trailer(fifth_axis) ""
       }
   }
  ...
  if {![string compare "V5" $sinumerik_version] || ![string compare "V6" $sinumerik_version] || ![string compare "V7" $sinumerik_version] } {
     ...
     if {[string match "ON" $mom_siemens_smoothing]} {
        ...
        switch $sinumerik_version {
           ...
           "V5" -
           "V6" -
           "V7"   {set mom_siemens_smoothing "G64"}
        }
     }
...
#=============================================================
  ...
  global mom_machine_mode
  global dpp_prev_channel_id
  global mom_channel_id spindle
  global mom_operation_name
  global mom_warning_info
  if {[string compare $mom_machine_mode "TURN"]} {
return
   ...
   if { ![info exists spindle_is_out] } {
      if { $mom_spindle_mode == "RPM" } {
         ...
         if { [info exists mom_spindle_maximum_rpm] && [expr $mom_spindle_maximum_rpm > 0] } {
           ##<2013-2-19 Allen> Be used to force addresses LIMS if the variable mom_channel_id is changed.
           if { ![info exists dpp_prev_channel_id] || ($dpp_prev_channel_id != $mom_channel_id) } {
            MOM_force once S_limit
            set dpp_prev_channel_id $mom_channel_id
           }


         } else {
           ##<2013-2-19 Allen> Check if the Maximum limit is set in operation, if not a default value will be used and post will write warning message into listing window
             set mom_warning_info "$mom_operation_name: maximun Spindle speed limit is not set,or not equal to 0. Please set the value, or system will give it a default value 1000."
             MOM_catch_warning
             set mom_spindle_maximum_rpm 1000
         }
...
#=============================================================
proc PB_CMD__check_block_CYCLE832_v6 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
   ...
   if { [info exists sinumerik_version] && [string match "V6" $sinumerik_version] } {
      #If tolerance is redefined in UDE, output again as User Defined
      if { ![info exists mom_siemens_tol_status] } {
         set mom_siemens_tol_status "System"
      }
...
#=============================================================
proc PB_CMD__check_block_Tool_change_head { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
  ...
  global mom_out_angle_pos
  global prev_tool_change_head_number

  if {$mom_channel_id==1} {
     if {![info exists tool_holder_spindle_axis]} {
        set tool_holder_spindle_axis 0
     }
     PB_CMD__check_block_TURN_set_angle
 return 1
  } else {
     if {[info exists prev_tool_change_head_number]} {
        unset prev_tool_change_head_number
     }
 return 0
  }
...
#Force output of GETD()
   MOM_force Once getd

   if {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value == 2 } {
       set millturn_spindle_name "C3"
 return 1
   } elseif {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value == 1 } {
       set millturn_spindle_name "C4"
 return 1
   }
...
#=============================================================
proc PB_CMD__check_block_initial_move_1_1 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#=============================================================
proc PB_CMD__check_block_spindle_off_end_path { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
   ...
   if {(![string compare $mom_machine_mode "TURN"] && [info exists mom_next_machine_mode] && [string compare $mom_machine_mode $mom_next_machine_mode]) || \
       ([info exists mom_current_oper_is_last_oper_in_program] && $mom_current_oper_is_last_oper_in_program == "YES")} {
 return 1
   } elseif {![string compare $mom_machine_mode "MILL"]} {
 return 1
   } else {
 return 0
   }
...
#=============================================================
proc PB_CMD__check_block_tool_change_turret { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#   0 : No output
  ...
  global mom_sys_lathe_x_factor mom_sys_lathe_i_factor
...
#=============================================================
  ...
  global mom_fixture_offset_value
  global MCS_z_offset
  global mom_part_transfer_feedrate
  global mom_part_transfer_spindle_speed
  global mom_part_unit
  global mom_output_unit
  ...
  ##<2012-11-05 JSF>code to deal with part_unit and output_unit
  set mom_part_transfer_contact_position [expr $mom_part_transfer_contact_point_pos(2)-$mom_ude_spindles_distance + $main_offset]

  if { ![string match $mom_part_unit $mom_output_unit] } {
      switch $mom_part_unit {
      IN {
         set mom_part_transfer_contact_position [expr $mom_part_transfer_contact_point_pos(2)*25.4 - $mom_ude_spindles_distance*25.4 + $main_offset]
         }
      MM {
         set mom_part_transfer_contact_position [expr $mom_part_transfer_contact_point_pos(2)/25.4 - $mom_ude_spindles_distance/25.4 + $main_offset]
         }
  ...
  if {![info exists mom_part_transfer_feedrate]} {
     set mom_part_transfer_feedrate 100
  }

  if {![info exists mom_part_transfer_spindle_speed]} {
     set mom_part_transfer_spindle_speed 0
  }
}
...
#=============================================================
proc PB_CMD_cancle_spindle_synchronous { } {
#=============================================================
...
#
  ...
  if {[string match "3_axis_mill" $mom_kin_machine_type]} {
     ...
     if { [string match "*ROTARY*" $mom_siemens_ori_def] } {
        MOM_suppress Once A3 B3 C3
  #<2012-10-30 lili> Fix axial drilling C angle not output issue. Remove following detection
...
return
 ...
 if {!([string match $mom_current_motion "circular_move"] && $mom_pos_arc_plane == "XY")} {
  ...
  #<2015-03-30 lili> Add Drilling operation type check.
      if {[string match "Point to Point" $mom_operation_type] || [string match "Hole Making" $mom_operation_type] || \
          [string match "Drilling" $mom_operation_type] || [string match "Chamfer Milling" $mom_operation_type] || \
          [string match "Radial Groove Milling" $mom_operation_type] || [string match "Generic Motion" $mom_operation_type]} {
      ...
      } elseif {[string match $mom_current_motion "initial_move"] || [string match $mom_current_motion "first_move"]} {
  #<2012-11-05 lili> soft limit control only works at first motion for milling operations
         set mom_pos(0) [expr -1*$mom_pos(0)]
         set mom_pos(1) [expr -1*$mom_pos(1)]
         set ang [expr $mom_pos(4)+180]
         if {![info exists mom_prev_out_angle_pos(1)]} {set mom_prev_out_angle_pos(1) 0}
         set mom_out_angle_pos(1) [ROTSET $ang $mom_prev_out_angle_pos(1) $mom_kin_5th_axis_direction\
                                               $mom_kin_5th_axis_leader mom_sys_leader(fifth_axis)\
                                               $mom_kin_5th_axis_min_limit $mom_kin_5th_axis_max_limit]
  ...
  #<2015-03-30 lili> Add Drilling operation type check.
      if {[string match "Point to Point" $mom_operation_type] || [string match "Hole Making" $mom_operation_type] || \
         ...
         PB_CMD_set_principal_axis
         set mom_out_angle_pos(1) [LIMIT_ANGLE [expr $mom_pos(4)+180]]
         global mom_motion_type
         if {$mom_cycle_spindle_axis == 0 && [string compare $mom_current_motion "initial_move"] \
             && [string compare $mom_current_motion "first_move"] && ![string compare $mom_motion_type "CYCLE"] } {
             set mom_pos(0) [expr -1*$mom_pos(0)]
         }
      } elseif {[string match $mom_current_motion "initial_move"] || [string match $mom_current_motion "first_move"]} {
          set mom_pos(0) [expr -1*$mom_pos(0)]
...
return
  ...
  # <szl 2016-07-01> Only check if operation is a 5 axis operation in channel 1.
  if {![info exists mom_channel_id] || $mom_channel_id == 1} {
     ...
     if {($mom_tool_axis_type >=2 && [string match "Variable-axis *" $mom_operation_type ])  || \
         ...
         ([string match "variable_axis" $mom_tool_path_type] && ![string match "Variable-axis *" $mom_operation_type] && \
          ...
          [string compare "Radial Groove Milling" $mom_operation_type] && [string compare "Generic Motion" $mom_operation_type])} {
  ...
  if {[string match "Point to Point" $mom_operation_type] || [string match "Hole Making" $mom_operation_type] || \
      [string match "Drilling" $mom_operation_type] || [string match "Chamfer Milling" $mom_operation_type] || \
      [string match "Radial Groove Milling" $mom_operation_type] || [string match "Generic Motion" $mom_operation_type]} {
     if {[EQ_is_equal [expr abs($mom_tool_axis(2))] 1.0] && (![info exists mom_siemens_coord_rotation] || $mom_siemens_coord_rotation != 1)} {
        PB_CMD_set_XZC_mode
     }
  }
...
return
  ...
  if { [info exists mom_siemens_program_control] && [string match "TRUE" $mom_siemens_program_control] } {
     ...
     set subroutine_name "${mom_output_file_directory}${mom_operation_name}${output_extn}"
            #<2011-07-20 lili> output to subprog folder for CSE simulaiton.
...
#=============================================================
proc PB_CMD_handle_subspindle_motion { } {
#=============================================================
  ...
  if {![string compare $mom_move_type_name "Chuck_Motion"]} {
     # M120 - Open chuck on main spindle
     # M130 - Open chuck on sub spindle
     # M121 - Close chuck on main spindle
     # M131 - Close chuck on sub spindle
  ...
  if {![string compare $mom_move_type_name "Subspindle_Motion"]} {
     ...
     if {[info exists mom_ssm_move_to_home] && $mom_ssm_move_to_home == 1} {
        MOM_output_literal "G90 G53 G0 Z3=0"
     } else {
...
#                   folder which located in same directory of \postprocessor as
#                   machine kits.
 ...
 if {$cycle_name == "THREAD"} {
    ...
    if {[info exists mom_turn_thread_end_line]} {
       ...
       if {[EQ_is_equal $yee $yes]} {
          set thread_end_pos(2) $mom_turn_thread_end_line($ze)
       } else {
...
#=============================================================
  ...
  if {![string compare $mom_operation_type "Generic Motion"]} {
 return
  }
  if {$mom_sys_lathe_x_double == 1} {
...
#=============================================================
proc PB_CMD_output_feed_definition { } {
#=============================================================
  global mom_siemens_feed_definition
  global flag

  if {[info exist flag(feed_definition)]} {
return
  }

  if { [info exists mom_siemens_feed_definition] && $mom_siemens_feed_definition == "ON" } {
     MOM_output_literal "DEF REAL _F_CUTTING, _F_ENGAGE, _F_RETRACT"
     set flag(feed_definition) 1
  }
}
...
#=============================================================
proc PB_CMD_output_turn_subprogram { } {
#=============================================================
 global mom_channel_id

 if {[info exists mom_channel_id] && $mom_channel_id==1} {
    MOM_output_literal "TURN_UPPER"
 } elseif {[info exists mom_channel_id] && $mom_channel_id==2} {
    MOM_output_literal "TURN_LOWER"
 }
}
...
#=============================================================
proc PB_CMD_reverse_cutcom { } {
#=============================================================
# milling at sub-spindle face requires reverse cutcom codes
     global mom_tool_axis
     ...
     if { [string toupper $mom_machine_mode] == "MILL" } {


        # if mom_tool_axis vector is -1 reverse codes
        if {[EQ_is_equal $mom_tool_axis(2) -1.0]} {
        ...
        if {[info exists mom_channel_id] && $mom_channel_id == 2} {
    ...
    # Output coordinate rotation code
      if {![EQ_is_equal $coord_ang_A $save_coord_ang_A] || ![EQ_is_equal $coord_ang_B $save_coord_ang_B] || ![EQ_is_equal $coord_ang_C $save_coord_ang_C]} {
         if { ![string match "SWIVELING" $mom_siemens_5axis_mode] } {
            ...
            if { [PB_CMD__check_block_CYCLE800_powerline] } {
               PB_call_macro CYCLE800
            }
            if { [PB_CMD__check_block_CYCLE800_solutionline] } {
               PB_call_macro CYCLE800_sl
            }
         }
      }
...
#
  ...
  set mom_siemens_ori_coord          "ORIMKS" ;      # ORIWKS / ORIMKS
...
#=============================================================
proc PB_CMD_set_transmit_mode { } {
#=============================================================
     global mom_mcs_goto mom_kin_4th_axis_rotation
     global mom_tool_axis mom_pos mom_kin_4th_axis_ang_offset
     global RAD2DEG mom_sys_lathe_x_factor mom_sys_spindle_axis
     global mom_out_angle_pos
 ...
 return
     ...
     if {[info exist mom_siemens_transmit_mode] && $mom_siemens_transmit_mode == "ON"} {
        if {[EQ_is_equal [expr abs($mom_tool_axis(2))] 1.0]} {
           ...
           if { ![string compare "reverse" $mom_kin_4th_axis_rotation] } { set transmit_pos(4) [expr -1*$transmit_pos(4)] }
           set transmit_pos(4) [expr $transmit_pos(4) + $mom_kin_4th_axis_ang_offset]
           set transmit_pos(4) [LIMIT_ANGLE $transmit_pos(4)]
           set mom_out_angle_pos(1) $transmit_pos(4)
        }
        if {$mom_channel_id == 2} {
           if {$mom_fixture_offset_value == 2} {
              MOM_output_literal "ATRANS C3=180"
           } else {
              MOM_output_literal "ATRANS C4=180"
           }
        }
     }
...
#=============================================================
proc PB_CMD_sync_spindle_output { } {
#=============================================================
# Output following spindle speed preset value and spindle synchronize code when sync spindle UDE is used.
# If one spindle synchronize coupling has been defined, can't specify the other one.
#
# 2014-3-5 Initial version - Levi.
...
#=============================================================
   ...
   set mom_pb_macro_arr(COUPDEF) \
       [list {COUPDEF ( , ) 0 {}} \
        {{{$mom_ude_following_spindle_name} 0} \
         {{$mom_ude_master_spindle_name} 0} \
         {{$mom_ude_speed_ratio_numerator} 1 4 5 1 1 9 4} \
         {{$mom_ude_speed_ratio_denominator} 1 4 5 1 1 9 4} \
         {{\"$mom_ude_block_behavior\"} 0} \
         {{\"$mom_ude_coupl_type\"} 0}}]

   set mom_pb_macro_arr(COUPON) \
       [list {COUPON ( , ) 0 {}} \
        {{{$mom_ude_following_spindle_name} 0} \
         {{$mom_ude_master_spindle_name} 0} \
         {{$mom_ude_angle_offset} 1 4 2 1 1 6 4}}]

   set mom_pb_macro_arr(COUPOF) \
       [list {COUPOF ( , ) 0 {}} \
        {{{$mom_ude_following_spindle_name} 0} \
         {{$mom_ude_master_spindle_name} 0}}]

   set mom_pb_macro_arr(COUPDEL) \
       [list {COUPDEL ( , ) 0 {}} \
        {{{$mom_ude_following_spindle_name} 0} \
         {{$mom_ude_master_spindle_name} 0}}]

   set mom_pb_macro_arr(Tool_change_head) \
       [list {TC ( , ) 0 {}} \
        {{{$mom_tool_number} 1 0 5 0 0 5} \
         {{$mom_out_angle_pos(0)} 1 4 3 1 1 7 4} \
         {{$tool_holder_spindle_axis} 1 4 3 1 1 7 4}}]
}

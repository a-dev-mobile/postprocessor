########################## TCL Event Handlers ##########################
#
#  sim15_millturn_9ax_fanuc_mm.tcl - 5_axis_head_table
#
#    This is a 5-Axis Milling Machine With
#     Rotary Head and Table.
#
#  Created by lili @ Thursday, November 16, 2017 10:37:42 AM China Standard Time
...
# 09-12-2016 lili - Update channel 1 tool change code from G361 to fanuc macro G65 P0361.
...
# 15-Dec-2016 szl - Bug fix PR7894933: mirror circular output with switching arc_direction for the turning operations are in channel 2.
#                   Update the command PB_CMD_millturn_init_lathe_machine and PB_CMD_millturn_init_mill_machine.
...
#
  ...
  set mom_sys_alt_unit_post_name                "sim15_millturn_9ax_fanuc_mm__IN.pui"
...
#=============================================================
proc MOM_Set_coord_transform { } {
#=============================================================
   global mom_ude_coord_transformation
   PB_CMD_ude_set_coord_transform
}
...
#=============================================================
   ...
   if { [PB_CMD__check_block_mill_default_linear_move] } {
      MOM_do_template circular_move
   }

   if { [PB_CMD__check_block_polar_output] } {
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   if { [PB_CMD__check_block_sim15_G52_cancel] } {
      MOM_do_template sim15_G52_cancel
   }
   PB_CMD_reset_output_mode
   PB_CMD_release_spindle
   PB_CMD_end_of_path
   PB_CMD_return_home
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   PB_CMD_spindle_prestart
   PB_CMD_detect_tool_path_type
   PB_CMD_detect_output_type
   PB_CMD_detect_cycle_clearance_plane
   PB_CMD_check_output_mode_validity
   PB_CMD_head_setting

   MOM_do_template auto_tool_change

   if { [PB_CMD__check_block_sim15_tool_change] } {
      MOM_do_template sim15_tool_change
   }

   if { [PB_CMD__check_block_tool_change_1] } {
      MOM_do_template tool_change_1
   }
   PB_CMD_output_mainspinle

   if { [PB_CMD__check_block_initial_move_BC] } {
      MOM_do_template initial_move_BC
   }

   if { [PB_CMD__check_block_sim15_G52] } {
      MOM_do_template sim15_G52
   }
   PB_CMD_output_coord_rotation
   PB_CMD_set_polar_mode
   PB_CMD_set_xzc_mode
   PB_CMD_set_tcp_code
   PB_CMD_TURN_output_spindle_speed
   PB_CMD_recal_cycle_rapid_pos
   PB_CMD_force_output
   PB_CMD_coolant_on
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   if { [PB_CMD__check_block_sim15_tool_change] } {
      MOM_force Once B_toolchange D_toolchange
      MOM_do_template sim15_tool_change
   }
...
#=============================================================
  ...
  global mom_motion_event
   ...
   if { [PB_CMD__check_block_cycle_no_clearanc_plane_rapid_move] } {
      MOM_do_template rapid_move_3
   }
}
...
#=============================================================
  ...
  global mom_sys_machine_time mom_machine_time
   ...
   PB_CMD_millturn_init_mill_machine
   PB_CMD_millturn_init_lathe_machine
...
#=============================================================
   ...
   MOM_do_template cycle_rigid_tap
   set cycle_init_flag FALSE
}
...
#=============================================================
proc MOM_workpiece_catch { } {
#=============================================================
   global mom_part_transfer_contact_point
   global mom_ude_spindles_distance
   global mom_part_transfer_approach
   global mom_part_transfer_fxst
   global mom_part_transfer_fxsw
   PB_CMD_output_ude_workpiece_catch
}
...
#=============================================================
proc MOM_workpiece_transfer { } {
#=============================================================
   global mom_part_transfer_retract
   global mom_spindle_2_position
   PB_CMD_output_part_transfer_subspindle
}
...
#=============================================================
   ...
   if { [PB_CMD__check_block_mill_work_plane] } {
      MOM_do_template mill_work_plane
   }

   if { [PB_CMD__check_block_turn_output] } {
...
#=============================================================
   ...
   PB_CMD_uplevel_new_tap

   MOM_force Once G_cutcom G_mode
...
return
   ...
   if { ![info exists spindle_is_out] } {
      if { $mom_spindle_mode == "RPM" } {
         ...
         MOM_suppress once M_spindle S G_spin
         }
...
#=============================================================
proc PB_CMD_TURN_output_spindle_speed { } {
#=============================================================
# Command to set spindle speed mode for turning
...
return
  ...
  if { $mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM" } {
     MOM_force once G_spin G92 M_spindle S
...
# Deal with the condition about G43.5 that using vector to output
   ...
   if {![string compare $mom_machine_mode "MILL"]} {
       if {$dpp_tcp_tool_axis_output_mode == "VECTOR"&&$dpp_tool_path_type=="5AXIS"} {
       return 1
       } else {
       return 0
       }
...
#=============================================================
proc PB_CMD__check_block_cycle_no_clearanc_plane_rapid_move { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
# Deal with initial rapid move of cycle operation that has no clearance plane or start point
  ...
  if {$dpp_cycle_clearance_plane == "FALSE" && $mom_current_motion == "initial_move"} {
return 1
  } else {
return 0
  }
...
#=============================================================
proc PB_CMD__check_block_cycle_parameters_2 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
# Record the previous mom_cycle_retract_to_pos(2)
 global dpp_prev_cycle_retract_to_pos
 global mom_cycle_retract_to_pos
 set dpp_prev_cycle_retract_to_pos $mom_cycle_retract_to_pos(2)
 return 1
...
#=============================================================
proc PB_CMD__check_block_mill_default_linear_move { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
# Deal with the default condition
  ...
  global mom_machine_mode
  global dpp_tcp_tool_axis_output_mode
  global mom_motion_type dpp_cal_coord_rot_type
  global dpp_cycle_clearance_plane
  global mom_current_motion
  global dpp_tool_path_type
  global dpp_coord_transformation
...
if {![string compare $mom_machine_mode "MILL"]} {
    if {$dpp_coord_transformation == "POLAR" || ($dpp_tool_path_type=="5AXIS" && $dpp_tcp_tool_axis_output_mode == "VECTOR")||($dpp_cycle_clearance_plane == "FALSE" && $mom_current_motion == "initial_move")} {
        return 0
    } else {
         return 1
    }
...
#=============================================================
proc PB_CMD__check_block_polar_output { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
# Deal with the polar case
...
if {![string compare $mom_machine_mode "MILL"]} {
    ...
    if {$dpp_coord_transformation == "POLAR"} {
       if {$mom_channel_id == 2} {
          set mom_pos(1) [expr -$mom_pos(1)]
          if {[info exists mom_motion_event] && $mom_motion_event == "circular_move"} {
             set mom_pos_arc_center(1) [expr -$mom_pos_arc_center(1)]
             set mom_prev_pos(1) [expr -$mom_prev_pos(1)]
          }
       }
    return 1
    } else {
...
#=============================================================
   ...
   if { [info exists dpp_return_motion_start] && $dpp_return_motion_start } {
      ...
      set o_buffer [MOM_do_template rapid_move_1 CREATE]
...
#=============================================================
proc PB_CMD__check_block_sim15_G52 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
   ...
   global sim15_G52_active
  MOM_ask_mcs_info

  set sim15_G52_active "inactive"
...
#=============================================================
proc PB_CMD__check_block_sim15_G52_cancel { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#=============================================================
proc PB_CMD__check_block_sim15_tool_change { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
## condition:
#   1. mom_channel_id ==1
#   2. if $dpp_coord_rotation == "ROTATION" and $dpp_coord_rotation_output_type == "SWIVELING"(G68.2) set B_axis_angle to 0.0
#
  ...
  if {$mom_channel_id==1} {
      if { $dpp_coord_rotation == "ROTATION" && $dpp_coord_rotation_output_type == "SWIVELING" } {
          set B_axis_angle 0.0
      } else {
          set B_axis_angle $mom_out_angle_pos(0)
}
 return 1
  } else {
 return 0
...
#=============================================================
   ...
   return 0
      ...
      if {$mom_motion_event == "linear_move"} {
         PB_CMD_check_first_buffer_for_return_motion
         set o_buffer [MOM_do_template linear_move_2 CREATE]
         lappend dpp_return_motion_list $o_buffer
      } elseif {$mom_motion_event == "circular_move"} {
         PB_CMD_check_first_buffer_for_return_motion
         set o_buffer [MOM_do_template circular_move_2 CREATE]
         lappend dpp_return_motion_list $o_buffer
      }
   return 0
   } else {
...
#=============================================================
proc PB_CMD__check_block_tool_change_1 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#=============================================================
proc PB_CMD__check_block_turn_output { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
...
#
 ...
 CALCULATE_ANGLE "ZXZ" rotation_matrix coord_rotation
 }
...
if {[array exists mom_parent_csys_matrix]} {
 ...
 # Calculate euler angles , rotation order is X->Y->Z
 if {$dpp_coord_rotation_output_type == "SWIVELING"} {
 CALCULATE_ANGLE "ZXZ" csys_matrix coord_rotation
 }
 if {$dpp_coord_rotation_output_type == "WCS_ROTATION"} {
 CALCULATE_ANGLE "XYZ" csys_matrix coord_rotation
 }
...
#=============================================================
# Command to check validity of output mode
# Used in initial move and first move
# 03-12-12 yaoz - Initial version
#
# parameters:
...
#
# return:
#   dpp_output_coord_mode
#   dpp_coord_rotation
#   dpp_coord_transformation
#
  global mom_machine_mode
  if {![string compare $mom_machine_mode "TURN"]} {
    return
  ...
  if {$dpp_tool_path_type == "5axis"} {
    if {$dpp_coord_transformation != "OFF"} {
      MOM_output_to_listing_device "5 axis cannot be outputed with polar or XZC transformation.It is set to OFF internally"
      set dpp_coord_transformation "OFF"
    }
  }

if {$dpp_coord_transformation == "POLAR" &&\
    $dpp_tool_path_type == "3AXIS" &&\
    $dpp_cal_coord_rot_type == "AUTO_3D" } {
    set dpp_coord_rotation "OFF"
    return
   }

if {$dpp_tool_path_type == "3AXIS" &&\
     $dpp_coord_rotation == "ROTATION" &&\
     $dpp_cal_coord_rot_type == "AUTO_3D" &&\
     $dpp_coord_rotation_output_type == "WCS_ROTATION"} {
     set dpp_coord_rotation "OFF"
  } elseif {$dpp_tool_path_type == "3AXIS" &&\
     $dpp_coord_rotation == "ROTATION" &&\
     $dpp_cal_coord_rot_type == "AUTO_3D" &&\
     $dpp_coord_rotation_output_type == "SWIVELING"} {
...
#=============================================================
proc PB_CMD_connect_spindle { } {
#=============================================================
global mom_machine_mode
global mom_channel_id
global mom_fixture_offset_value
global dpp_main_spindle_connect
global dpp_sub_spindle_connect

  if {![string compare $mom_machine_mode "MILL"]} {
   if {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value == 2 } {
   MOM_output_literal "M245"
   set dpp_sub_spindle_connect "TRUE"
   } elseif {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value == 1 } {
   MOM_output_literal "M45"
   set dpp_main_spindle_connect "TRUE"
   }
  }
...
return
  ...
  if { [string compare "AUTO_3D" $dpp_cal_coord_rot_type] || $dpp_coord_rotation=="OFF"} {
return
  }
  ...
  if {![EQ_is_equal $coord_rotation(0) $dpp_prev_coord_rotation(0)]||\
     ![EQ_is_equal $coord_rotation(1) $dpp_prev_coord_rotation(1)]||\
     ![EQ_is_equal $coord_rotation(2) $dpp_prev_coord_rotation(2)]} {
     set dpp_prev_coord_rotation(0) $coord_rotation(0)
     set dpp_prev_coord_rotation(1) $coord_rotation(1)
     set dpp_prev_coord_rotation(2) $coord_rotation(2)
...
return
   ...
   if {$mom_cycle_tool_axis_change ==0 && $mom_cycle_clearance_plane_change == 1} {
      MOM_do_template cycle_rapidtoZ
   } elseif {$mom_cycle_tool_axis_change ==0 && $mom_cycle_clearance_plane_change == -1} {
      MOM_do_template cycle_rapidtoXY
   }
}
...
#=============================================================
# Command to set coord rotation calculation type automatically, and calculate it accordingly
# Used in initial move and first move
# 03-12-12 yaoz - Initial version
# 06-13-12 yaoz - remove "Swiveling" from dpp_coord_rotation value list
# parameter:
#   dpp_coord_rotation
#   mom_kin_coordinate_system_type
#
#
# return:
#   dpp_cal_coord_rot_type
#
#
#

  global dpp_coord_rotation
## "WCS rotation"  G68/ROT/AROT G68.2/CYCLE800/PLANE SPATIAL
## "OFF"
...
## "Auto3D"
  ...
  if {[string match $dpp_coord_rotation "OFF"]} {
    return
  }

## automatic check
  if {[string match $dpp_coord_rotation "WCS rotation"]} {
    if { [string match "CSYS" $mom_kin_coordinate_system_type] } {
      set dpp_cal_coord_rot_type "LOCAL CSYS"
      PB_CMD_cal_coord_rotation_Local_CSYS
    } else {
      set dpp_cal_coord_rot_type "Auto3D"
      PB_CMD_cal_coord_rotation_Auto3D
    }
  }
### debug info####
  global debug
  if {$debug} {
MOM_output_to_listing_device "detect coord rotation:"
MOM_output_to_listing_device "dpp_coord_rotation: $dpp_coord_rotation"
MOM_output_to_listing_device "dpp_cal_coord_rot_type: $dpp_cal_coord_rot_type"
}
####
...
#=============================================================
# Detect whether the drilling operation has clearance plane
# or start point
#
# 07-23-2012 ljw - Initial version
 global mom_motion_type
...
# 07-27-2012 yaoz - If mom_tool_axis(2) is 0, the coord rotation is OFF.
...
## automatic check
  switch $dpp_tool_path_type {
    ...
    "3AXIS"       {
     if {$mom_kin_machine_type == "3_axis_mill" || $mom_kin_machine_type == "3_axis_mill_turn"} {
     return
     }
     if {![EQ_is_equal $mom_tool_axis(2) 1.0]} {
        ...
        set dpp_cal_coord_rot_type "AUTO_3D"
          ...
          if { [string match "CSYS" $mom_kin_coordinate_system_type]} {
              ...
              if {![string length [string trim $parent_mcs]]} {
                  set mom_warning_info "Unable to find the parent MCS of CSYS"
                  MOM_catch_warning
                  }
 ...
 #if the operation is a general 3 axis operation, not a 3+2 axis operation
 #set dpp_prev_coord_rotation angles to 0
 global dpp_prev_coord_rotation
 if {[string match "OFF" $dpp_coord_rotation]} {
    set dpp_prev_coord_rotation(0) 0
    set dpp_prev_coord_rotation(1) 0
    set dpp_prev_coord_rotation(2) 0
  }
...
### debug info####
  global debug
  if {$debug} {
     MOM_output_to_listing_device "tool axis:$mom_tool_axis(0) $mom_tool_axis(1) $mom_tool_axis(2)"
     MOM_output_to_listing_device "detect coord rotation:"
     MOM_output_to_listing_device "dpp_coord_rotation_output_type: $dpp_coord_rotation_output_type"
     MOM_output_to_listing_device "dpp_coord_rotation: $dpp_coord_rotation"
     MOM_output_to_listing_device "dpp_cal_coord_rot_type: $dpp_cal_coord_rot_type"
     MOM_output_to_listing_device "detect TCP condition:"
     MOM_output_to_listing_device "dpp_tcp_tool_axis_output_mode: $dpp_tcp_tool_axis_output_mode"
     MOM_output_to_listing_device "dpp_output_coord_mode: $dpp_output_coord_mode"
  }
####
...
# 07-27-2012 yaoz - If channel id is 2, set tool path type to 3AXIS
  ...
  if {($mom_tool_axis_type >= 2  &&  [string match "Variable-axis *" $mom_operation_type])  || \
     ...
     ![string match "Feature Based Planar Milling" $mom_operation_type] && ![string match "Drilling" $mom_operation_type] &&\
     ![string match "Chamfer Milling" $mom_operation_type] && ![string match "Radial Groove Milling" $mom_operation_type]} {
  ...
  ## set tool path type to 3 axis if turret is used.
  global mom_channel_id
...
### debug info####
  global debug
  if {$debug} {
     MOM_output_to_listing_device "detect tool path type in $mom_operation_name"
     MOM_output_to_listing_device "mom_machine_mode: $mom_machine_mode"
     MOM_output_to_listing_device "mom_operation_type: $mom_operation_type"
     MOM_output_to_listing_device "mom_tool_axis_type: $mom_tool_axis_type"
     MOM_output_to_listing_device "dpp_tool_path_type: $dpp_tool_path_type"
  }
####
...
catch {unset dpp_ge(cycle_hole_counter)}
}
...
#=============================================================
MOM_force once G_motion G_adjust G_feed G_mode X Y Z M_spindle H
}
...
#=============================================================
proc PB_CMD_head_setting { } {
#=============================================================
# Command to calculate B head rotation.
# Used in Initial Move and first move
# 04-05-12 yaoz - Initial version
#
  ...
  if {$mom_channel_id==1} {
    ...
    if {![string compare $mom_machine_mode "TURN"]} {
      if {![array exists mom_tool_axis]} {
        ...
        #set mom_out_angle_pos(0) [expr $RAD2DEG*atan2($mom_tool_axis(0),$mom_tool_axis(2))]
        set mom_out_angle_pos(0) [ROTSET [expr $RAD2DEG*atan2($mom_tool_axis(0),$mom_tool_axis(2))] $mom_out_angle_pos(0) $mom_kin_4th_axis_direction $mom_kin_4th_axis_leader mom_sys_leader(fourth_axis)   $mom_kin_4th_axis_min_limit $mom_kin_4th_axis_max_limit]
      }
    }
  }
}
...
## set default value
  ...
  set dpp_tcp_tool_axis_output_mode "VECTOR"
  set dpp_output_coord_mode "TCP_FIX_TABLE"
  set dpp_cycle_clearance_plane "TRUE"
  set dpp_prev_coord_rotation(0) 0
  set dpp_prev_coord_rotation(1) 0
  set dpp_prev_coord_rotation(2) 0
  set dpp_cycle_hole_counter 0
  set dpp_coord_transformation "OFF"
...
#=============================================================
proc PB_CMD_millturn_init_lathe_machine { } {
#=============================================================
# Command to initialize the lathe machine kinematic
# Used in Start of Path
# 04-05-12 yaoz - Initial version
#
...
return
   ...
   # <2016-12-15 szl> mirror circular output
   if {$mom_channel_id ==1} {
...
#=============================================================
proc PB_CMD_millturn_init_mill_machine { } {
#=============================================================
# Command to initialize the milling machine kinematic
# Used in Start of Path
# 04-05-12 yaoz - Initial version
#
...
return
  ...
  # <2016-12-15 szl> reset mom_sys_circle_code for milling operations
  set mom_sys_circle_code(CLW)    "2"
  set mom_sys_circle_code(CCLW)   "3"

  if {$mom_channel_id==1} {
     set tool_holder_spindle_axis 0.0
     set spindle 1
     MOM_enable_address Y fourth_axis
     set mom_sys_lathe_x_factor "1"
     set mom_sys_lathe_i_factor "1"
     ...
     set spindle 2
     MOM_disable_address Y fourth_axis
     set mom_sys_lathe_x_factor "-1"
     set mom_sys_lathe_i_factor "-1"
  }
...
#=============================================================
   ...
   if { [info exists dpp_output_coord_mode] && [string compare "OFF" $dpp_output_coord_mode] } {
      MOM_do_template caxis_clamp
      MOM_do_template caxis_clamp_1
   }
}
...
#=============================================================
# Output G68/68.2 coordinate rotation code
# Used in initial move and first move
#
# 04-05-12 yaoz - Initial version
# 06-13-12 yaoz - Add G68.2 output support
  ...
  global mom_init_pos
  global coord_offset
  global save_coord_ang_C
  global dpp_coord_rotation
  global mom_machine_mode
  global dpp_coord_rotation_output_type
  global coord_rotation
  global mom_pos mom_cycle_rapid_to_pos mom_cycle_rapid_to
  global mom_pos mom_cycle_feed_to_pos mom_cycle_feed_to
  global mom_pos mom_cycle_retract_to_pos mom_cycle_retract_to
...
# Return when Not need to rotation
  if {![info exists dpp_coord_rotation] || [string compare $dpp_coord_rotation "ROTATION"]} {
    return }
  if {![info exists dpp_coord_rotation_output_type]} {
    return }
  ...
  global dpp_prev_coord_rotation coord_rotation
  set dpp_prev_coord_rotation(0) $coord_rotation(0)
  set dpp_prev_coord_rotation(1) $coord_rotation(1)
  set dpp_prev_coord_rotation(2) $coord_rotation(2)

  if {$dpp_cal_coord_rot_type=="AUTO_3D"} {
     VMOV 3 mom_pos mom_cycle_rapid_to_pos
     VMOV 3 mom_pos mom_cycle_feed_to_pos
     VMOV 3 mom_pos mom_cycle_retract_to_pos
     set mom_cycle_rapid_to_pos(2) [expr $mom_pos(2)+$mom_cycle_rapid_to]
  ...
  } elseif {$dpp_coord_rotation_output_type == "WCS_ROTATION"} {
  set mom_sys_adjust_code 43.1
  MOM_force once rotate_X rotate_Y rotate_Z H

  if { ![EQ_is_equal $coord_rotation(0) 0.0] && ![EQ_is_equal $coord_rotation(1) 0.0] && ![EQ_is_equal $coord_rotation(2) 0.0] } {
      set mom_warning_info "Invalid coordinate system.  Unable to determine correct rotations."
      MOM_catch_warning
  }

  if {$mom_kin_machine_type == "5_axis_dual_table"} {
      set mom_warning_info "G68 is not supported for 5 axis dual table machine"
      MOM_catch_warning
}

  if {$mom_kin_machine_type == "5_axis_head_table"} {
      if {![EQ_is_zero $mom_out_angle_pos(1)]} {
         if {![EQ_is_zero $coord_rotation(2)] || (![EQ_is_zero $coord_rotation(0)] && ![EQ_is_zero $coord_rotation(1)])} {
            set mom_warning_info "Invalid coordinate system.  Unable to determine correct rotations."
            MOM_catch_warning
         } else {
            set save_coord_ang_C $coord_rotation(2)
            set coord_rotation(2) [expr -1*$mom_out_angle_pos(1)]
            MOM_do_template coord_rot_k
           set coord_rotation(2) $save_coord_ang_C
           }
        }
    }

if { [info exists coord_rotation(0)] && ![EQ_is_zero $coord_rotation(0)] } {
      MOM_do_template coord_rot_i
}
if { [info exists coord_rotation(1)] && ![EQ_is_zero $coord_rotation(1)] } {
      MOM_do_template coord_rot_j
}
if { [info exists coord_rotation(2)] && ![EQ_is_zero $coord_rotation(2)] } {
      MOM_do_template coord_rot_k
}
...
### debug info####
  global debug
  if {$debug} {
     MOM_output_to_listing_device "output coord rotation"
     MOM_output_to_listing_device "coord_rotation(0): $coord_rotation(0)"
     MOM_output_to_listing_device "coord_rotation(1): $coord_rotation(1)"
     MOM_output_to_listing_device "coord_rotation(2): $coord_rotation(2)"
     MOM_output_to_listing_device "coord_offset(0): $coord_offset(0)"
     MOM_output_to_listing_device "coord_offset(1): $coord_offset(1)"
     MOM_output_to_listing_device "coord_offset(2): $coord_offset(2)"
     MOM_output_to_listing_device "mom_out_angle_pos(1): $mom_out_angle_pos(1)"
  }
####
}
...
#=============================================================
# Command to output the code for diameter mode
# Used in Start of Path
# 04-05-12 yaoz - Initial version
#
  global mom_channel_id
  ...
  if {$mom_sys_lathe_x_double == 1} {
       MOM_output_literal "G10.9 X0"
  } elseif {$mom_sys_lathe_x_double == 2} {
       MOM_output_literal "G10.9 X1"
  }
}
...
#=============================================================
   ...
   set dpp_output_coord_mode "OFF"

   if { [info exists mom_5axis_control_mode] } {
      if { $mom_5axis_control_mode == "TCP" } {
         set dpp_output_coord_mode "TCP"
      } elseif { $mom_5axis_control_mode == "POS" } {
         set dpp_output_coord_mode "OFF"
      }
...
#=============================================================
proc PB_CMD_output_mainspinle { } {
#=============================================================
# Command to output main spindle code and C axis connect code.
# Used in initial move and first move
# 04-05-12 yaoz - Initial version
#
  ...
  if {![string compare $mom_machine_mode "TURN"]} {
    switch $spindle {
      "3" {
            #MOM_output_literal "M245"
            MOM_output_literal "M304"
            #set dpp_sub_spindle_connect "TRUE"
          }
      "4" {
            #MOM_output_literal "M45"
            MOM_output_literal "M303"
            #set dpp_main_spindle_connect "TRUE"
          }
      default           { }
    }
...
## is it needed to output main spindle when milling.
  if {![string compare $mom_machine_mode "MILL"]} {
    switch $spindle {
      "1" {
            if {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value==2 } {
           ...
           MOM_output_literal "M301"
          }
      "2" {
            if {[info exists mom_fixture_offset_value] && $mom_fixture_offset_value==1 } {
            MOM_output_literal "M45"
            set dpp_main_spindle_connect "TRUE"
          }
...
#=============================================================
proc PB_CMD_output_part_transfer_subspindle { } {
#=============================================================
### output the code for transferring workpiece to sub spindle
    global mom_part_transfer_retract
    global mom_spindle_2_position
    global mom_part_transfer_flag
...
MOM_output_literal "(Part transfer to subspindle..)"
MOM_output_literal "M69 (Mainspindle chuck unclamp, part transfer)"
MOM_output_literal "M36 (Cancel speed synchronized operation mode)"
MOM_output_literal "G1 G91 Z3=[expr $mom_part_transfer_retract] F100"
MOM_output_literal "G0 G90 G53 Z3=[expr $mom_spindle_2_position] H0"
#MOM_output_literal "G330(Return subspindle to machine zero)"
...
#=============================================================
proc PB_CMD_output_ude_workpiece_catch { } {
#=============================================================
### output the code for catching workpiece by sub spindle
...
MOM_output_literal "(Part transfer start..)"
MOM_output_literal "M303 G96 S2000 M04"
MOM_output_literal "M35 (Specify speed synchronized operation mode)"
MOM_output_literal "M269 (Subspindle chuck unclamp)"
MOM_output_literal "G0 G53 Z3=[expr $mom_part_transfer_contact_position+$mom_part_transfer_approach]"
MOM_output_literal "G1 G53 Z3=[expr $mom_part_transfer_contact_position] F100"
#MOM_output_literal "G38 J K F Q(Detect the pushing of workpiece)"
...
#=============================================================
proc PB_CMD_recal_cycle_rapid_pos { } {
#=============================================================
   ...
   # This procedure will only be called for Drill Dwell cycle.
   if { ![info exists cycle_init_flag] } {
      return
   }
   if { !([info exists cycle_name] && [info exists mom_cycle_counter_sink_dia]) } {
      # Clear this flag for other cycles
      set skip_flag 0
      return
   }
   if { [string match "initial_move" $mom_motion_event] || [string match "first_move" $mom_motion_event] } {
      # The initial or first move is the "drill dwell" cycle, then an additional event will be inserted between
      # MOM_drill_dwell & MOM_drill_dwell_move, thus there has no need to recalculate the cycle rapid position
      # again in the next MOM_drill_dwell_move.
      set skip_flag 1
      CALCULATE_CYCLE_POS 0
   } else {
      if { [info exists skip_flag] } {
         if { $skip_flag } {
            # skip the current move
            set skip_flag 0
         } else {
            # Recalculate the cycle rapid position
            CALCULATE_CYCLE_POS 1
         }
      } else {
         CALCULATE_CYCLE_POS 1
      }
...
if { [info exists dpp_main_spindle_connect] && $dpp_main_spindle_connect == "TRUE" } {
MOM_output_literal "M46"
set dpp_main_spindle_connect "FALSE"
}

if { [info exists dpp_sub_spindle_connect] && $dpp_sub_spindle_connect == "TRUE" } {
MOM_output_literal "M246"
set dpp_sub_spindle_connect "FALSE"
}
}
...
# Used in end of path
#
# 03-14-2012 yaoz - Initial version
# 06-14-2012 yaoz - Updated to support G68.2
# 07-27-2012 yaoz - Handle turning and dpp_coord_transformation
  ...
  if {($dpp_output_coord_mode == "TCP_FIX_TABLE" || $dpp_output_coord_mode == "TCP_FIX_MACHINE")&&\
     $dpp_tool_path_type=="5AXIS"} {
     set mom_sys_adjust_code 43
  }

  switch $dpp_coord_rotation {
    "ROTATION" {   MOM_output_literal "G69"
                   set mom_sys_adjust_code 43
                   set coord_offset(0) 0.0
                   set coord_offset(1) 0.0
                   set coord_offset(2) 0.0
  ...
  switch $dpp_coord_transformation {
    "POLAR" { set dpp_coord_transformation "OFF"
                   MOM_output_literal "G113"}
    "XZC" {
      global mom_lock_axis mom_lock_axis_plane mom_lock_axis_value_defined mom_lock_axis_value
      set dpp_coord_transformation "OFF"
      ...
      MOM_lock_axis }
    "OFF" { }
    default { set dpp_coord_transformation "OFF" }
}

## reset dpp variable to default
PB_CMD_set_default_dpp_value
MOM_do_template absolute_mode
}
...
#=============================================================
  ...
  if {$mom_channel_id==1} {
     set mom_sys_home_pos(0) 598.7
     set mom_sys_home_pos(1) -108
     set mom_sys_home_pos(2) 450
     MOM_force Once X Y Z H G_motion
     MOM_do_template return_home_x
     MOM_do_template return_home_spindle_yz

  } elseif {$mom_channel_id==2} {
     set mom_sys_home_pos(0) 338
     set mom_sys_home_pos(1) 0
     set mom_sys_home_pos(2) 486
     MOM_force Once X Y Z H G_motion
     MOM_do_template return_home_turret_x
...
# 2014-04-18 levi - Bug fix PR#7128990, tcl error "dpp_ge(cycle_hole_counter) doesn't exist".
   global dpp_ge
   if {![info exists dpp_ge(cycle_hole_counter)]} {
      set dpp_ge(cycle_hole_counter) 1
   } else {
...
#=============================================================
# Command to output polar mode code
# Used in initial move and first move
# 04-05-12 yaoz - Initial version
#
  ...
  if {![string match $dpp_coord_transformation "POLAR"]} {
    return
  }
...
return
  ...
  if {![string compare $dpp_tool_path_type "2AXIS"]} {
return
  }
  if {![string compare $dpp_tool_path_type "3AXIS"] && $dpp_coord_rotation=="OFF"} {
     ...
     } elseif {![string compare "4_axis_head" $mom_kin_machine_type]} {
        if {![EQ_is_zero $mom_out_angle_pos(0)]} {
           set mom_sys_adjust_code "43.1"
        }
    }
  ...
  if {[string match $dpp_tcp_tool_axis_output_mode "AXIS"] && $dpp_tool_path_type=="5AXIS"} {
     set mom_sys_adjust_code 43.4
  } elseif {[string match $dpp_tcp_tool_axis_output_mode "VECTOR"] && $dpp_tool_path_type=="5AXIS"} {
     set mom_sys_adjust_code 43.5
     set dpp_output_coord_mode "TCP_FIX_TABLE"
  }

  if {[string match $dpp_output_coord_mode "TCP_FIX_TABLE"]&& $dpp_tool_path_type=="5AXIS"} {
     VMOV 3 mom_mcs_goto mom_pos
     VMOV 3 mom_prev_mcs_goto mom_prev_pos
     VMOV 3 mom_arc_center mom_pos_arc_center
  }
...
## return if NOT need set XZC mode
  if {![string compare $mom_machine_mode "TURN"]} {
    return
  }

  if {![string match $dpp_coord_transformation "XZC"]} {
    return
  }
...
#=============================================================
proc PB_CMD_ude_set_coord_transform { } {
#=============================================================
global mom_ude_coord_transformation
global dpp_coord_transformation
  global debug
  if {$debug} {
MOM_output_to_listing_device "get in ude set coord transform"
MOM_output_to_listing_device "$mom_ude_coord_transformation"
}
set dpp_coord_transformation $mom_ude_coord_transformation
}


#=============================================================
proc PB_CMD_ude_set_transform { } {
#=============================================================
global mom_ude_coord_transformation
global dpp_coord_transformation


mom_ude_coord_transformation
MOM_output_literal "$mom_ude_coord_transformation"
set dpp_coord_transformation $mom_ude_coord_transformation
}
...
#=============================================================
# <lili 09-12-2016> Update channel 1 tool change code to G65 P0361.

  global mom_channel_id

  if {[info exists mom_channel_id] && $mom_channel_id==1}  {
     MOM_output_literal "T0"
     # MOM_output_literal "G361 B0 D0"
     MOM_output_literal "G65 P0361 B0 D0"
...
#=============================================================
proc PB_CMD_unset_option_variable { } {
#=============================================================
# Make sure mom_cycle_option is unset to output float tapping code, mom_cycle_option
# is used in PB_CMD_tapping_g_code_string_determine
...
proc CALCULATE_CYCLE_POS { cycle_flag } {
   global mom_cycle_hole_dia mom_cycle_tool_angle
   global mom_pos mom_cycle_rapid_to_pos mom_cycle_feed_to_pos
   global mom_sys_spindle_axis
   global mom_cycle_clearance_plane mom_cycle_rapid_to
   global mom_tool_tip_diameter
   global DEG2RAD

   if { ![info exists mom_cycle_hole_dia] || ![info exists mom_cycle_tool_angle] } {
      return
   }

   if { [EQ_is_zero $mom_cycle_tool_angle] } {
      return
   }

   if {![info exists mom_tool_tip_diameter]} {
      set tool_tip_diameter 0
   } else {
      set tool_tip_diameter $mom_tool_tip_diameter
   }

   set ang_radians [expr $mom_cycle_tool_angle * $DEG2RAD / 2.0]
   set offset_len  [expr [expr $tool_tip_diameter / 2.0] / tan($ang_radians)]

   if { $cycle_flag } {
      for { set i 0 } { $i < 3 } { incr i } {
         set mom_cycle_rapid_to_pos($i) [expr $mom_pos($i) + $mom_cycle_clearance_plane * $mom_sys_spindle_axis($i)]
         set mom_cycle_feed_to_pos($i)  [expr $mom_cycle_feed_to_pos($i) + $offset_len * $mom_sys_spindle_axis($i)]
      }
   } else {
      for { set i 0 } { $i < 3 } { incr i } {
         set mom_pos($i) [expr $mom_pos($i) + ($mom_cycle_clearance_plane-$mom_cycle_rapid_to) * $mom_sys_spindle_axis($i)]
         set mom_cycle_rapid_to_pos($i) $mom_pos($i)
         set mom_cycle_feed_to_pos($i)  [expr $mom_cycle_feed_to_pos($i) + $offset_len * $mom_sys_spindle_axis($i)]
      }
   }
}

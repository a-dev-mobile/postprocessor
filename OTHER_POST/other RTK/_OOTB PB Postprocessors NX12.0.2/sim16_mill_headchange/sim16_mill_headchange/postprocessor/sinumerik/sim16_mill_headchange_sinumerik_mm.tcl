########################## TCL Event Handlers ##########################
#
#  sim16_mill_headchange_sinumerik_mm.tcl - 5_axis_dual_head
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Heads.
#
#  Created by lili @ Thursday, November 16, 2017 10:38:03 AM China Standard Time
...
# 02-23-2016 Lili - Update Headchange output and CYCLE800 output.
...
#                     PB_CMD_rotate_rapid_coordinate and PB_CMD_rotate_cycle_coordinate.
  ...
  set mom_sys_alt_unit_post_name                "sim16_mill_headchange_sinumerik_mm__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_home_pos(0)                       "800"
  set mom_sys_home_pos(1)                       "-1000"
  set mom_sys_home_pos(2)                       "1200"
  set mom_sys_zero                              "0"
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   if { [PB_CMD__check_block_headchange] } {
      PB_call_macro headchange
   }
...
#   1 : Output BLOCK
#   0 : No output
# <lili 02-23-2016> Add cycle800 output for simple vertical head.
   ...
   global coord_angle_1 coord_angle_2 coord_angle_3
...
#For sim16 headchange machine
   global head_number

   if {[info exists head_number] && $head_number == 2} {
      set cycle800_tc "\"RA_HEAD\""
   } elseif {[info exists head_number] && $head_number == 3} {
      set cycle800_tc "\"AC_HEAD\""
   } elseif {[info exists head_number] && $head_number == 1} {
      #<lili 02-23-16> add tcarr name for vert_head.
      set cycle800_tc "\"VERT_HEAD\""
      #<TJ 09-28-16> return 0, because Cycle800 for this head is not needed
      return 0
   } else {
return 0
   }
...
return 1
       ...
       # <lili 02-23-16> output cycle800 for vert_head in initial move.
      set level [info level]
      set upper_proc [info level [expr $level-1]]
      if {![string compare $upper_proc "MOM_initial_move"]} {
         set cycle800_dir 1
         set cycle800_st 0
         set cycle800_mode 57
...
# 30-Nov-17 zhx    - fix PR8993565 drilling operation issue
...
# PB_CMD_init_cycle_parameters deals with auto mode, cycle_init_flag = TRUE
  ...
  if { [string match "Point to Point" $mom_operation_type] ||\
       ...
       ![string match "DISTANCE" $mom_cycle_return_type]) } {
...
#=============================================================
proc PB_CMD__check_block_headchange { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
   ...
   global prev_head_number
   global prev_tool_name
   global mom_out_angle_pos
   global prev_head_angle_pos
   global head_out_angle_pos
   global mom_siemens_coord_rotation
   global head_swivel
   global mom_init_pos
   ...
   if {[info exists mom_tool_name]} {
      if {[regsub {_[0-9]$} $mom_tool_name "" new]} {
         set mom_tool_name $new
      }
   }

   if {![info exists prev_head_number]} {
      set prev_head_number 0
   }

   if {![info exists prev_tool_name]} {
      set prev_tool_name ""
   }

   if {![info exists prev_head_angle_pos]} {
      set prev_head_angle_pos(0) 0
      set prev_head_angle_pos(1) 0
   }
   ...
   if {[info exists mom_siemens_coord_rotation] && $mom_siemens_coord_rotation == 1} {
      ...
      set head_swivel 0
      set head_out_angle_pos(0) $mom_init_pos(3)
      set head_out_angle_pos(1) $mom_init_pos(4)
      ...
      set head_out_angle_pos(0) [ROTSET $mom_init_pos(3) $mom_prev_out_angle_pos(0)  $mom_kin_4th_axis_direction  $mom_kin_4th_axis_leader  mom_sys_leader(fourth_axis)  $mom_kin_4th_axis_min_limit  $mom_kin_4th_axis_max_limit]

      set head_out_angle_pos(1) [ROTSET $mom_init_pos(4) $mom_prev_out_angle_pos(1)  $mom_kin_5th_axis_direction   $mom_kin_5th_axis_leader   mom_sys_leader(fifth_axis)  $mom_kin_5th_axis_min_limit  $mom_kin_5th_axis_max_limit]

   } else {
      set head_swivel 1
      set head_out_angle_pos(0) $mom_out_angle_pos(0)
      set head_out_angle_pos(1) $mom_out_angle_pos(1)
      if {[info exists mom_siemens_coord_rotation] && $mom_siemens_coord_rotation == 2} {
         set head_out_angle_pos(0) $mom_out_angle_pos(1)
         set head_out_angle_pos(1) $mom_out_angle_pos(0)
      }

   }

   if {$head_number ==2} {
      MOM_disable_address fourth_axis fifth_axis fifth_axis_home
      MOM_enable_address fourth_axis_home
      if {[EQ_is_equal $head_out_angle_pos(1) 90] && $head_swivel == 1} {
          set head_out_angle_pos(0) [LIMIT_ANGLE [expr 180+$head_out_angle_pos(0)]]
          set mom_out_angle_pos(0) $head_out_angle_pos(0)
          set mom_out_angle_pos(1) -90
          MOM_reload_variable -a mom_out_angle_pos
      }
      set head_out_angle_pos(1) 0
   } elseif {$head_number ==1} {
      MOM_disable_address fourth_axis fifth_axis fourth_axis_home fifth_axis_home
   } else {
      MOM_enable_address fourth_axis fifth_axis fourth_axis_home fifth_axis_home
   }

# <lili 02-22-16> update following codes to set head_out_angle_pos to 0
if {0} {
   if {$head_number != $prev_head_number || [string compare $mom_tool_name $prev_tool_name]} {
      set prev_tool_name $mom_tool_name
      set prev_head_number $head_number
      set prev_head_angle_pos(0) $head_out_angle_pos(0)
      set prev_head_angle_pos(1) $head_out_angle_pos(1)
 return 1
   } else {
      if {[EQ_is_equal $prev_head_angle_pos(0) $head_out_angle_pos(0)] && [EQ_is_equal $prev_head_angle_pos(1) $head_out_angle_pos(1)]} {
 return 0
      } else {
      set prev_head_angle_pos(0) $head_out_angle_pos(0)
      set prev_head_angle_pos(1) $head_out_angle_pos(1)
...
#=============================================================
proc PB_CMD__check_block_headchange_1 { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output
# 2014-07-22 Allen - The command is deal with cycle plane change and tool axis changes in rapid move.
   ...
   if {$head_number ==1} { return 0 }
   if {[info exists mom_siemens_coord_rotation] && $mom_siemens_coord_rotation == 1} { return 0}
   ...
   if {[string match "5_axis_dual_head" $save_mom_kin_machine_type] && [string match "5_axis_dual_table" $mom_kin_machine_type]} {
      set head_out_angle_pos(0) $mom_out_angle_pos(1)
      set head_out_angle_pos(1) $mom_out_angle_pos(0)
   } else {
      set head_out_angle_pos(0) $mom_out_angle_pos(0)
   ...
   if {$head_number ==2} {
      MOM_disable_address fourth_axis fifth_axis fifth_axis_home
      MOM_enable_address fourth_axis_home
      if {[EQ_is_equal $head_out_angle_pos(1) 90]} {
          set head_out_angle_pos(0) [LIMIT_ANGLE [expr 180+$head_out_angle_pos(0)]]
          set mom_out_angle_pos(1) $head_out_angle_pos(0)
...
# 10-15-13 Allen- restore kinematics in 3+2 axis SWIVELING mode.
  ...
  if {[info exists head_swivel] && $head_swivel == 1} {
  return 0; #same as cycle800 need detect last parameter of headchange macro
  }
...
#   1 : Output BLOCK
#   0 : No output
#<lili 02-22-16> Do not output rotation axes due to cycle800 will take care to position the rotatry axes.
  global mom_operation_type
...
return 0
  ...
  if { $mom_siemens_coord_rotation != 0 && [string match "SWIVELING" $mom_siemens_5axis_mode] } {
# for sim16 mill headchange machine to comment out following return
#return 0
  }
...
# mom_siemens_coord_rotation "0"/"1"/"2"
  ...
  if { $mom_siemens_milling_setting == "Default"} {
     if {$dpp_ge(toolpath_axis_num)==5} {
       ...
       # for sim16 mill headchange machine
...
# 10-15-13 Allen - Rewrite the command for that if the tool vector turn to be along Z axis,table axis will not return to zero.
# 09-14-15 Lili - Remove head_change call, use cycle800 output instead
...
#=============================================================
   ...
   set mom_pb_macro_arr(headchange) \
       [list {HEADCHANGE ( , ) 0 {}} \
        {{{\"$mom_tool_name\"} 0} \
         {{$head_number} 1 0 2 0 0 2} \
         {{$head_out_angle_pos(0)} 1 4 3 1 1 7 4} \
         {{$head_out_angle_pos(1)} 1 4 3 1 1 7 4}}]
}
...
set cam_post_user_tcl "sim16_simulation_ini.tcl"

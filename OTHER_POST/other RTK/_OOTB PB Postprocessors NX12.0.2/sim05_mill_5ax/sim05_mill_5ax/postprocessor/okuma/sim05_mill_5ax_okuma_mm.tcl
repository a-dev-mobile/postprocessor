########################## TCL Event Handlers ##########################
#
#  sim05_mill_5ax_okuma_mm.tcl - 5_axis_dual_head
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Heads.
#
#  Created by lili @ Thursday, November 16, 2017 10:27:01 AM China Standard Time
...
# 01-06-2017 szl - Resaved with PB v11.0.1.
  ...
  set mom_sys_alt_unit_post_name                "sim05_mill_5ax_okuma_mm__IN.pui"
...
return
   ...
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE initial_move"
   puts $tmp_file "  \{"
   ...
   puts $tmp_file "      dwell\[\$mom_delay_value\]\\opt"
   ...
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE cycle_retract"
   puts $tmp_file "  \{"
   puts $tmp_file "      G_cutcom\[\$mom_sys_cutcom_code(OFF)\]"
   puts $tmp_file "      G_motion\[\$mom_sys_rapid_code\]"
   puts $tmp_file "      X\[\$mom_cycle_clearance_pos(0)\]"
   puts $tmp_file "      Y\[\$mom_cycle_clearance_pos(1)\]"
   puts $tmp_file "      Z\[\$mom_cycle_clearance_pos(2)\]"
   puts $tmp_file "      fifth_axis\[\$mom_out_angle_pos(1)\]"
   puts $tmp_file "      fourth_axis\[\$mom_out_angle_pos(0)\]"
   puts $tmp_file "  \}"
   puts $tmp_file ""
   ...
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE rapid_move_1"
   puts $tmp_file "  \{"
   puts $tmp_file "      tcp\[169\]"
   puts $tmp_file "      G_feed\[\$mom_sys_feed_rate_mode_code(\$feed_mode)\]\\opt"
   puts $tmp_file "      G_motion\[\$mom_sys_rapid_code\]"
   ...
   puts $tmp_file "  \{"
   puts $tmp_file "      G_plane\[\$mom_sys_cutcom_plane_code(\$mom_cutcom_plane)\]\\opt"
   puts $tmp_file "      G_motion\[\$mom_sys_cycle_tap_code\]"
   puts $tmp_file "      X\[\$mom_cycle_feed_to_pos(0)\]"
   puts $tmp_file "      Y\[\$mom_cycle_feed_to_pos(1)\]"
   puts $tmp_file "      Z\[\$mom_cycle_feed_to_pos(2)\]"
   puts $tmp_file "      fifth_axis\[\$mom_out_angle_pos(1)\]"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   if { [PB_CMD__check_block_tool_length_offset_cancel] } {
      MOM_do_template tool_length_offset_cancel
   }
   ...
   if { [PB_CMD__check_block_return_to_reference_point] } {
      MOM_force Once H
      MOM_do_template return_to_reference_rotation_axis
   }
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   MOM_force Once wcs_fixture_offset
   MOM_do_template wcs_selection

   if { [PB_CMD__check_block_fouth_fifth_axis_move] } {
...
#=============================================================
  ...
  # First tool only gets handled once
   ...
   MOM_do_template return_to_reference
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   if { [PB_CMD__check_block_wcs_selection] } {
      MOM_force Once wcs_fixture_offset
      MOM_do_template wcs_selection
   }

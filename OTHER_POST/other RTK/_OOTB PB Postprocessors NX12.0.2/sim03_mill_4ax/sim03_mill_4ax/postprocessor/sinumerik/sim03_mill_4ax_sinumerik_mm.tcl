########################## TCL Event Handlers ##########################
#
#  sim03_mill_4ax_sinumerik_mm.tcl - 4_axis_table
#
#    This is a 4-Axis Milling Machine With
#     Rotary Table.
#
#  Created by lili @ Thursday, November 16, 2017 10:25:16 AM China Standard Time
...
# 26-Apr-2017 szl   - Update PB_CMD__check_block_rotation_axes, regression fix(PR8816483) for sim03_tombstone: the fixture cannot get the correct rotation angle.
}
  ...
  set mom_sys_alt_unit_post_name                "sim03_mill_4ax_sinumerik_mm__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_home_pos(1)                       "616.5"
  set mom_sys_home_pos(2)                       "790"
  set mom_sys_zero                              "0"
...
#=============================================================
proc MOM_subprogram_for_tool_path { } {
#=============================================================
   global mom_ude_output_subprogram
}
...
#
  ...
  #<2014-03-25 lili> Fix following condition to allow local CSYS rotation coordinate
  if {[info exists dpp_fixture_rotation($mom_fixture_offset_value)] && $mom_siemens_coord_rotation != 1} {
     set mom_out_angle_pos(0) $dpp_fixture_rotation($mom_fixture_offset_value)
     MOM_force Once G_motion fourth_axis
     MOM_do_template rotation_axes
     set mom_out_angle_pos(0) 0
     MOM_do_template rotation_axes CREATE
...
#=============================================================
proc PB_CMD_end_subop { } {
#=============================================================
  global dpp_sub_name
  global mom_operation_name
  global ptp_file_name
  global dpp_save_seqnum
  global mom_sequence_increment mom_sequence_frequency
  global mom_ude_output_subprogram

  if {![info exists mom_ude_output_subprogram] || $mom_ude_output_subprogram != "ON"} {
return
  }

  if {[info exists dpp_sub_name($mom_operation_name)] && [file exists $dpp_sub_name($mom_operation_name)]} {

        MOM_output_literal "M17"
        global feedrate_file_name
        set feedrate_file_name $dpp_sub_name($mom_operation_name)
        if {[llength [info commands PB_CMD_output_feedrate_variables]]} {
        PB_CMD_output_feedrate_variables
        }
        MOM_close_output_file $dpp_sub_name($mom_operation_name)

  }
      MOM_end_subroutine
      MOM_open_output_file $ptp_file_name
      SEQNO_RESET
      MOM_reset_sequence $dpp_save_seqnum $mom_sequence_increment $mom_sequence_frequency

      MOM_set_seq_on

      if {![regsub {_[0-9]$} $mom_operation_name "" operation_name]} {
         set operation_name $mom_operation_name
      }
      MOM_output_literal "$operation_name"
}
...
#=============================================================
  ...
  if {![info exists mom_prev_out_angle_pos(0)]} {set mom_prev_out_angle_pos(0) 0}
  if {[info exists dpp_fixture_rotation($mom_fixture_offset_value)]} {
     set dpp_fixture_rotation($mom_fixture_offset_value) [ROTSET $dpp_fixture_rotation($mom_fixture_offset_value) $mom_prev_out_angle_pos(0)  $mom_kin_4th_axis_direction  $mom_kin_4th_axis_leader  mom_sys_leader(fourth_axis)  $mom_kin_4th_axis_min_limit  $mom_kin_4th_axis_max_limit]
  }
}
...
#=============================================================
proc PB_CMD_start_subop { } {
#=============================================================
   global mom_output_file_directory mom_operation_name
   global mom_sequence_number mom_sequence_increment mom_sequence_frequency
   global dpp_subop
   global dpp_sub_name
   global ptp_file_name
   ...
   global dpp_save_seqnum
   global mom_ude_output_subprogram
   global mom_copy_source_operation
...
return
   ...
   MOM_close_output_file $ptp_file_name
   if {[info exists mom_copy_source_operation]} {
      if {[string match "" $mom_copy_source_operation]} {
         set operation_name $mom_operation_name
      } else {
         set operation_name $mom_copy_source_operation
      }
   } elseif {![regsub {_[0-9]$} $mom_operation_name "" operation_name]} {
      set operation_name $mom_operation_name
   }

   set output_extn ".spf"
   set subroutine_name "${mom_output_file_directory}${operation_name}${output_extn}"
   global mom_post_in_simulation
   if {[info exists mom_post_in_simulation] && [string match "CSE" $mom_post_in_simulation]} {
      global mom_part_name
      set part_dir "[file dirname $mom_part_name]/"
      if {[catch {file mkdir ${part_dir}cse_files/subprog/}]} {
         ...
         set subprog_dir ${part_dir}cse_files/subprog/
         if {[file isdirectory $subprog_dir]} {
            set subroutine_name [file nativename "${subprog_dir}${operation_name}${output_extn}"]
         }
      }
   }

   set dpp_save_seqnum $mom_seqnum
   if { ![info exists dpp_subop($operation_name)] } {
      if {[file exists $subroutine_name]} {
         MOM_remove_file $subroutine_name
      }
      set dpp_subop($operation_name) 1
      set dpp_sub_name($mom_operation_name) $subroutine_name
...
set cam_post_user_tcl "simulation_ini_subprogram.tcl"

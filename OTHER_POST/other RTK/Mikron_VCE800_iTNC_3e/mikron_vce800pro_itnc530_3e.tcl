########################## TCL Event Handlers ##########################
#
#  mikron_vce800pro_itnc530_3e.tcl - 3_axis_mill
#
#    This is a 3-Axis Milling Machine.
#
#  Created by ronconi @ terça-feira, 9 de janeiro de 2018 13:42:05 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "mikron_vce800pro_itnc530_3e__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_coolant_code(MIST)                "25"
  set mom_sys_coolant_code(THRU)                "28"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "15000"
  set mom_kin_4th_axis_min_incr                 "0.001"
  set mom_kin_4th_axis_min_limit                "-15000"
  ...
  set mom_kin_rapid_feed_rate                   "20000"
...
#=============================================================
   ...
   CIRCLE_SET
   PB_CMD_check_arc_radius
   MOM_force Once Text X Y R circle_direction Text
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_motion X Y Z G_cutcom Text M_coolant}
   set rapid_spindle_x_blk {G_motion X G_cutcom Text M_coolant}
   set rapid_spindle_y_blk {G_motion Y G_cutcom Text M_coolant}
   set rapid_spindle_z_blk {G_motion Z G_cutcom Text M_coolant}
   set rapid_traverse_blk {G_motion X Y Z G_cutcom user_add M_spindle}
   set rapid_traverse_xy_blk {G_motion X Y G_cutcom user_add M_spindle}
   set rapid_traverse_yz_blk {G_motion Y Z G_cutcom user_add M_spindle}
   set rapid_traverse_xz_blk {G_motion X Z G_cutcom user_add M_spindle}
   set rapid_traverse_mod {G_motion user_add}
   set rapid_spindle_mod {G_motion Text}
...
###############################################################
global mom_tool_axis mom_path_name mom_out_angle_pos

if {[string range $mom_tool_axis(0) 0 5] != 0} {
    MOM_output_literal "###################################################"
    MOM_output_literal "#  FERRAMENTA NAO ESTA PERPENDICULAR AO EIXO X !  #"
    MOM_output_literal "   OPERACAO: $mom_path_name : "
    MOM_output_literal "   TLAXIS  : $mom_tool_axis(0),$mom_tool_axis(1),$mom_tool_axis(2)"
    MOM_output_literal "###################################################"

    MOM_output_to_listing_device "#  FERRAMENTA NAO ESTA PERPENDICULAR AO EIXO X !  #"
}

if {$mom_out_angle_pos(0) > 15000 || $mom_out_angle_pos(0) < -15000} {
    MOM_output_literal "###################################################"
    MOM_output_literal "#  ANGULO 'A' EXCEDEU LIMITE DE +/- 15000 GRAUS ! #"
    MOM_output_literal "    OPERACAO: $mom_path_name  "
    MOM_output_literal "    ANGULO  : $mom_out_angle_pos(0)"
    MOM_output_literal "###################################################"

    MOM_output_to_listing_device "#  ANGULO 'A' EXCEDEU LIMITE DE +/- 15000 GRAUS ! #"
}
}
...
#=============================================================
global mom_event_handler_file_name mom_part_name mom_fixture_offset_value
...
global co ci
 ...
 MOM_output_literal "$co FEITO POR: $mom_logname   $ci"
 MOM_output_literal "$co REV-01   : $mom_date   $ci"
 MOM_output_literal "$co MAQUINA  : MIKRON VCE800 PRO - COMANDO: ITNC530 $ci"
 MOM_output_literal "$co --------------------------------------------------------------  $ci"
# MOM_output_literal "$co POSPROCESSADOR: [ string toupper $mom_event_handler_file_name ] $ci"
 MOM_output_literal "$co Rev.: $usr_controle_rev    Data: $usr_data_rev   $ci"
# MOM_output_literal "$co --------------------------------------------------------------  $ci"

 MOM_output_literal "CYCL DEF 32.0 TOLERANCE  "
 MOM_output_literal "CYCL DEF 32.1 T0.05  "
 MOM_output_literal "$co --------------------------------------------------------------  $ci"

 MOM_output_literal "BLK FORM 0.1 Z  X-0  Y-0  Z-0 $ci"
 MOM_output_literal "BLK FORM 0.2    X+0  Y+0  Z+0 $ci"
 MOM_output_literal "$co --------------------------------------------------------------  $ci"

# MOM_output_literal "$co --- DATUM SETTING ---$ci"
...
#=============================================================
proc PB_CMD_check_arc_radius { } {
#=============================================================
global mom_arc_radius mom_arc_angle   usr_rad

if {$mom_arc_angle > 180} {
    set usr_rad  [expr (-1) * $mom_arc_radius]
} else {set usr_rad  $mom_arc_radius}
}
...
#=============================================================
  global cycle_init_flag    mom_cycle_delay  mom_path_name
  global mom_motion_event   mom_cycle_feed_to
  global mom_tool_diameter  mom_cycle_step1  mom_cycle_step2  mom_cycle_step3
  global mom_cycle_retract_mode   mom_cycle_rapid_to  mom_cycle_retract_to
  global mom_cycle_feed_rate_per_rev   tap_direction  mom_spindle_direction
  global cycle_peck_decr  cycle_peck_min
 ...
 #----- Heidenhain cycle types -------------------------------------------------
  if {$mom_motion_event == "bore_move"} {
      set cycle_type_number 201    ;# bore
  } elseif {$mom_motion_event == "bore_drag_move" || $mom_motion_event == "bore_no_drag_move"} {
            set cycle_type_number 202    ;# bore_grag
  } elseif {$mom_motion_event == "bore_back_move"} {
           MOM_output_to_listing_device " "
           MOM_output_to_listing_device "   ======================================="
           MOM_output_to_listing_device "    ATENCAO!!! OPERACAO: $mom_path_name"
           MOM_output_to_listing_device "    ERRO: CICLO BORE BACK NAO SUPORTADO!!!!"
           MOM_output_to_listing_device "   - - - - - - - - - - - - - - - - - - - -"
           MOM_output_to_listing_device "   => POS-PROCESSAMENTO INTERROMPIDO !!!!"
           MOM_output_to_listing_device "   ======================================="
           MOM_abort "CICLO BORE BACK NAO SUPORTADO!"  ;# não suportado

  } elseif {$mom_motion_event == "tap_move"} {
            set cycle_type_number 207    ;# rigid tapping
  } else {
          #set cycle_type_number 200     ;# drill & peck
          set cycle_type_number 203     ;# drill & peck
  }
 #---------------------------------------------------------------------------------
  ...
  if {![info exists tap_direction]} {set tap_direction    $mom_spindle_direction}

  if {[info exists mom_cycle_feed_rate_per_rev]} {
      if {$tap_direction == "CCLW"} {
 ...
 #---- peck sizes -----------------------------------------------------------------------------
  ...
  set cycle_peck_decr    0
  set cycle_peck_min    0
  ...
  if { $mom_motion_event == "drill_deep_move" || $mom_motion_event == "drill_break_chip_move"} {
      ...
      if {($mom_cycle_step2 != 0)} {set cycle_peck_decr  $mom_cycle_step2}
      if {($mom_cycle_step3 != 0)} {set cycle_peck_min   $mom_cycle_step3}
  }
 #-----------------------------------------------------------------------------------------------
 ...
 # to ensure the hole Z height Q203 is set correctly.
  ...
  if {$cycle_init_flag == "TRUE"} {
      MOM_force once cycle_rapid_to cycle_feed_to   cycle_plunge       cycle_surface cycle_pullback
      MOM_force once cycle_min_step cycle_feedrate  cycle_retract_feed cycle_dwell_top
      MOM_force once cycle_dwell    cycle_decr_step cycle_tap_lead     cycle_orient

      switch $cycle_type_number {
    200 {
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_200 CREATE]       ;# Q200 setup ( engage ) ht
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_201 CREATE]    ;# Q201 hole depth ( incremental )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_206 CREATE]    ;# Q206 feedrate units/min
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_202 CREATE]    ;# Q202 peck size
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_210 CREATE]    ;# Q210 dwell above hole hardwired to zero
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_203 CREATE]    ;# Q203 Z of top of hole ( absolute )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_204 CREATE]    ;# Q204 Zreturn after cycle ( incremental )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_211 CREATE]    ;# Q211 cycle dwell]
        ...
        201 {
              ...
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_206 CREATE]    ;# Q206 feedrate units/min
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_211 CREATE]    ;# Q211 cycle dwell
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_208 CREATE]    ;# Q208 retraction feed rate
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_203 CREATE]    ;# Q203 Z of top of hole ( absolute )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_204 CREATE]    ;# Q204 Zreturn after cycle ( incremental )
              MOM_do_template cycle_block
            }
        202 {
              ...
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_203 CREATE]    ;# Q203 Z of top of hole ( absolute )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_204 CREATE]    ;# Q204 Zreturn after cycle ( incremental )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_214 CREATE]    ;# Q214 Disengaging direction
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_336 CREATE]    ;# Q336 Angle of spindle
              MOM_do_template cycle_block
            }
    203 {
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_200 CREATE] ;# Q200 setup ( engage ) ht
              ...
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_203 CREATE] ;# Q203 Z of top of hole ( absolute )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_204 CREATE] ;# Q204 Zreturn after cycle ( incremental )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_212 CREATE] ;# Q212
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_213 CREATE] ;# Q213
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_205 CREATE] ;# Q205
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_211 CREATE] ;# Q211 cycle dwell
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_208 CREATE] ;# Q208
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_256 CREATE] ;# Q256
              MOM_do_template cycle_block
            }
        207 {
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_200 CREATE]    ;# Q200 setup ( engage ) ht
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_201 CREATE]    ;# Q201 hole depth ( incremental )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_239 CREATE]    ;# Q239 pitch
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_203 CREATE]    ;# Q203 Z of top of hole ( absolute )
              MOM_add_to_line_buffer end " " [MOM_do_template cycle_block_204 CREATE]    ;# Q204 Zreturn after cycle ( incremental )
              MOM_do_template cycle_block
            }
...
if {[string range $mom_operation_type 0 4] == "Point"} {
    set c32_flag    0
} else {set c32_flag    1}
}
...
#-- Atualizar estas variáveis com a letra e data da ultima revisao.... --------
 set usr_controle_rev    "J"
 set usr_data_rev    "17/JUL/09"
#------------------------------------------------------------------------------
...
#==============================================================================
# 22/ABR/08    ---    Kleber        - Liberacao inicial;
#------------------------------------------------------------------------------
# 06/MAI/08     A    Kleber        - Alterado FMAX para MAX no bloco de
#                    retraçao do eixo Z;
#------------------------------------------------------------------------------
# 24/JUL/08     B    Kleber        - Alterado ciclo para operação STD_BORE
#                    de 202 para 201.
#                    - Interp Circular: Rmin=1mm, Rmax=2000mm
#                    ArcLength=1mm
#------------------------------------------------------------------------------
# 01/OUT/08     C    Kleber        - Acresc. proc PB_CMD_init_helix
#------------------------------------------------------------------------------
# 03/DEZ/08     D    Kleber        - Acresc bloco "TOOL CALL Z S" no evento
#                    MOM_first_move.
#------------------------------------------------------------------------------
# 16/ABR/09     E    Kleber        - Alterado valor do parametro de ciclo
#                    Q208 para zero (avanco de saida = entrada)
#------------------------------------------------------------------------------
# 28/ABR/09     F    Kleber        - Removido "R0" dos ciclos de furação;
#                    - Alterado código da refrigração THRU
#                    de 7 para 28;
#                    - Acresc. "TA2" no CYCL 32.1 .
#------------------------------------------------------------------------------
# 14/MAI/09     G    Kleber        - Alterada variavel "hsc" para valor 1 (fixo)
#------------------------------------------------------------------------------
# 16/JUN/09     H    Kleber        - Alterada posição "R0" /"FMAX"
#------------------------------------------------------------------------------
# 08/JUL/09     I    William        - Ajustado conforme solicitação Jupitrans
#                                       Arquivo doc no dir do cliente ...
#------------------------------------------------------------------------------
# 17/JUL/09     J    Kleber        - ALtersdo raio max para 3000mm e
#                    raio min. para 0.8mm;
#                    - Alterada interpolação circular do
#                    formato "CC/C" para "CR/R"
#                    - Alterado limite do parametro Q208 para 10000
#                    - Removido ";" da linha "BLK FORM"
#------------------------------------------------------------------------------
# 23/SET/09     M    William        - Ajustado p/ empresa SCHEUERMANN_HEILIG
#------------------------------------------------------------------------------
# 05/NOV/09     N    William        - Ajustado p/ empresa TECNOL
#------------------------------------------------------------------------------
...
#=============================================================
global mom_path_name  mom_tool_corner1_radius
...
set diam        [format %.1f "$mom_tool_diameter"]

if {[info exists mom_tool_corner1_radius]} {
    set rc  [format %.1f "$mom_tool_corner1_radius"]
} else {set rc    0}

#MOM_output_literal "$co M00 $ci"
#MOM_output_literal "$co (!!!Verificar_Ferramenta_Desenho_Processo!!!!) $ci"
#MOM_output_literal "$co $ci"

MOM_output_literal "$co OPERACAO   : $mom_path_name $ci"
MOM_output_literal "$co FERRAMENTA : $mom_tool_name - T$mom_tool_number  D$diam x RC$rc $ci"
}
...
proc MOM_helix_move { } {
#=============================================================
   global mom_pos_arc_plane mom_pos
...
#------------------------------------------------------------------------
#    Interpolação Helicoidal padrao HEIDENHAIN
#------------------------------------------------------------------------
# 1. Criar uma WORD "angle_IPA" e WORD "incr_IZ" com formato 'coordinate_signed';
# 2. Criar um BLOCK TEMPLATE de nome "helix_move" conforme abaixo:
#     CP IPA($mom_arc_angle) IZ($dlt_Z) DR F
#
 global dlt_Z

 if {$mom_prev_pos(2) > $mom_pos(2)} {
     set dlt_Z    [expr $mom_pos(2) - $mom_prev_pos(2)]
 } else {
         set dlt_Z [expr $mom_prev_pos(2) - $mom_pos(2)]
}

 MOM_suppress once Z
 MOM_do_template circular_move
 MOM_force once G_motion angle_IPA incr_IZ circle_direction
...
#______________________________________________________
 ...
 if { ![info exists tnum]} {set tnum    1}

 if {$tnum == 1} {
     set tn($tnum)    $mom_tool_number
     incr tnum
 } else {
        set rep_tool    0
        for {set xx 1} {$xx < $tnum} {incr xx} {
             if {$mom_tool_number == $tn($xx)} {set rep_tool    1}
    }
    if {$rep_tool == 0} {
            set tn($tnum)    $mom_tool_number
            incr tnum
...
#=============================================================
global tpcm_mode  ang_type
global flag_cb  ang_A  ang_C  reset_flag
global mom_kin_arc_output_mode  arc_mode_default
global flag_cb
...
set arc_mode_default $mom_kin_arc_output_mode
}
...
#=============================================================
proc PB_CMD_pos_angle_in_cycle { } {
#=============================================================
...
#MOM_output_literal "$mom_out_angle_pos(0) ; $mom_prev_out_angle_pos(0)"

if {$mom_out_angle_pos(0) != $mom_prev_out_angle_pos(0)} {
    set ang_A    [format %.3f $mom_out_angle_pos(0)]
    PB_CMD_rapid_A
}
}
...
#=============================================================
proc PB_CMD_reduz_avanco { } {
#=============================================================
global mom_motion_event feed_txt

MOM_output_text ";==> $mom_motion_event"
...
#=============================================================
proc PB_CMD_tolerancia { } {
#=============================================================
proc MOM_Tolerancia {} {
#------------------------------------------
global mom_usr_toler  mom_usr_value_defined
global mom_usr_value

    if {$mom_usr_toler == "DESBASTE"} {
        set usr_toler    [format %.3f 0.07]
    } elseif {$mom_usr_toler == "PRE-ACAB"} {
              set usr_toler    [format %.3f 0.05]
    } elseif {$mom_usr_toler == "ACABAMENTO"} {
              set usr_toler    [format %.3f 0.02]
    }
...
MOM_output_literal "CYCL DEF 32.0 TOLERANCIA"
MOM_output_literal "CYCL DEF 32.1 T$usr_toler"
#MOM_output_literal "CYCL DEF 32.2 HSC-MODE:$hsc TA5"
MOM_output_literal ";"
}
}
...
#=============================================================
proc PB_CMD_tool_check_program { } {
#=============================================================
 ...
 #------- abre arquivo resumo e coloca a relacao de arquivos gerados ---------------------
 MOM_set_seq_off
 set fname "${mom_output_file_basename}_tc"
 set list_file_name "${mom_output_file_directory}${fname}.h"

 MOM_remove_file $list_file_name
 MOM_open_output_file $list_file_name

 MOM_output_literal "0 BEGIN PGM $fname MM"
 MOM_output_literal "; =============================="
 MOM_output_literal "; TOOL SETTING L,R"
 MOM_output_literal ";"
 MOM_output_literal "; SET PARA. L,DL,R,DR,LTOL,RTOL,"
 MOM_output_literal "; LOFFS,ROFFS,CUT,RBREAK FOR T#"
 MOM_output_literal "; IN TOOL.T TO SUITABLE VALUES"
 MOM_output_literal "; =============================="
 MOM_output_literal ";"

 for {set zz 1} {$zz < $tnum} {incr zz} {
      MOM_output_literal " TOOL CALL $tn($zz) Z S3000.  "
      MOM_output_literal " ;"
      MOM_output_literal " TCH PROBE 584 MEDICAO COMPR E RAIO ~"
      MOM_output_literal "    Q350=+0    ;TIPO DE MEDICAO ~"
      MOM_output_literal "    Q351=+0    ;AMBITO DE MEDICAO ~"
      MOM_output_literal "    Q352=+0    ;CONTR. ARESTAS CORTE ~"
      MOM_output_literal "    Q355=-1    ;SPECIAL FUNCTION ~"
      MOM_output_literal "    Q361=+3    ;QUANTIDADE MEDICOES ~"
      MOM_output_literal "    Q362=+0.01 ;VARIACAO ADMITIDA ~"
      MOM_output_literal "    Q359=+0    ;CORR. ADITIVA COMPR. ~"
      MOM_output_literal "    Q360=+0    ;CORR. ADITIVA RAIO"
      MOM_output_literal " FN 9: IF +Q199 EQU +0 GOTO LBL $zz"
      MOM_output_literal " STOP"
      MOM_output_literal " ; =============================="
      MOM_output_literal " ; EXCEEDING LTOL/RTOL/RBREAK"
      MOM_output_literal " ; TOOL FAULTY/LOCKED"
      MOM_output_literal " ; =============================="
      MOM_output_literal " ;"
      MOM_output_literal " LBL $zz"
 }

 MOM_output_literal "END PGM $fname MM"

 MOM_close_output_file $list_file_name
}
...
#=============================================================
proc PB_CMD_verif_angulos { } {
#=============================================================
global mom_out_angle_pos mom_prev_out_angle_pos
global mom_rotary_delta_4th mom_rotary_delta_5th
global flag_cb  ang_A  ang_C tpcm_mode
global feed_m128 proc_name mom_operation_type
global ang_type  reset_flag
global ini_op_flag

if {![info exists mom_prev_out_angle_pos(0)]} {
     set mom_prev_out_angle_pos(0)    " "
}

if {$mom_out_angle_pos(0) != $mom_prev_out_angle_pos(0) || $ini_op_flag == 1} {
    set ang_A    [format %.3f $mom_out_angle_pos(0)]
    set flag_cb    2
    PB_CMD_rapid_A
    set reset_flag    0
...
#=============================================================
proc PB_CMD_verif_rotacao { } {
#=============================================================
global mom_spindle_speed mom_path_name mom_tool_number
global mom_motion_event last_rpm
...
#--- Gera mudanca de RPM -----
if {![info exists last_rpm]} {set last_rpm    -1}
if {$mom_motion_event != "initial_move" } {
    MOM_force once S
    MOM_do_template rpm_chg
}
}


#=============================================================
proc PB_CMD_verif_rpm { } {
#=============================================================
}

########################## TCL Event Handlers ##########################
#
#  Romi_Multiplic_60B_Fanuc21IT.tcl - lathe
#
#    
#
#  Created by ronconi @ sexta-feira, 5 de janeiro de 2018 10:34:23 E. South America Daylight Time
...
#
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_list_file_suffix                  "M60"
     set mom_sys_output_file_suffix                "M60"
  ...
  set mom_sys_alt_unit_post_name                "Romi_Multiplic_60B_Fanuc21IT__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_lathe_thread_advance_type(1)      "76"
  ...
  set mom_sys_cycle_drill_break_chip_code       "74"
  ...
  set mom_sys_home_pos(0)                       "700"
  set mom_sys_home_pos(1)                       "0"
  set mom_sys_home_pos(2)                       "700"
  set mom_sys_zero                              "0"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_x_axis_limit                      "850"
  set mom_kin_y_axis_limit                      "1000"
  set mom_kin_z_axis_limit                      "3600"
...
#=============================================================
   ...
   PB_LATHE_THREAD_SET
   MOM_force Once G_motion Text h_last_pass h_min
   MOM_do_template lathe_thread
   MOM_force Once G_motion X Z conicidade P Q passo
...
#=============================================================
   ...
   MOM_output_literal "(TROCAR FERRAMENTA $mom_tool_name)"
   MOM_force Once T
   MOM_do_template manual_tool_change1
   MOM_do_template G54
}
...
#if {$mom_tool_number == 7} {
#   set prog_loop $mom_tool_number
#} else {
#   if {$prog_start == "yes"} {
...
#=============================================================
 ...
 global mom_oper_program mom_logname
 global mom_logname mom_output_file_basename mom_output_file_suffix
 global mom_sys_control_out mom_sys_control_in
 global master_rev  master_rev_date
 global co ci tempo_anterior

 global mom_attr_PROGRAMVIEW_1_usr_desenho
 global mom_attr_PROGRAMVIEW_2_usr_item
 global mom_attr_PROGRAMVIEW_3_usr_rev_des
 global mom_attr_PROGRAMVIEW_4_usr_descricao
 global mom_attr_PROGRAMVIEW_5_usr_fase
 global mom_attr_PROGRAMVIEW_6_usr_material
 global mom_attr_PROGRAMVIEW_7_usr_proc_rev
 ...
 set part1   [split $mom_part_name "\\"]

 set usr_maquina    "TORNO ROMI MULTIPLIC 60B - FANUC 21IT"
 #kr- ---------------------------------------------------------------------------------------------------
 if {[info exists mom_attr_PROGRAMVIEW_1_usr_desenho]} {

         set usr_desenho    $mom_attr_PROGRAMVIEW_1_usr_desenho
         set usr_item       $mom_attr_PROGRAMVIEW_2_usr_item
         set usr_rev_des    $mom_attr_PROGRAMVIEW_3_usr_rev_des
         set usr_descricao  $mom_attr_PROGRAMVIEW_4_usr_descricao
         set usr_fase       $mom_attr_PROGRAMVIEW_5_usr_fase
         set usr_material   $mom_attr_PROGRAMVIEW_6_usr_material
         set usr_proc_rev   $mom_attr_PROGRAMVIEW_7_usr_proc_rev
 } else {
         set usr_desenho    ""
         set usr_item       ""
         set usr_rev_des    ""
 ...
 set L(1)  "(   )"
 set L(2)  "(PROGRAMA: [string toupper $mom_output_file_basename.$mom_output_file_suffix]"
 set L(3)  "(DESENHO: $usr_desenho - REV.: $usr_rev_des - ITEM: $usr_item"
 set L(4)  "(DESCRICAO: [string toupper $usr_descricao]"
 set L(5)  "(FASE: [string toupper $usr_fase] - MATERIAL: [string toupper $usr_material]"
 set L(6)  "(REV PROC.: $usr_proc_rev"
 set L(7)  "(PROGRAMADO POR: [string toupper $mom_logname]  - $usr_data"
 set L(8)  "(USINADO/TESTADO POR: "
 set L(9)  "(MAQUINA: $usr_maquina"
 set L(10) "(PP. REV.: $master_rev - $master_rev_date"

 #kr- -------------------------------------------------------------------------

 MOM_output_text ":$mom_output_file_basename"
 #MOM_output_text $L(1)
 for {set j 2} {$j<9} {incr j} {
      set i    [expr 50 - [string length $L($j)]]
...
#=============================================================
proc PB_CMD_cycle_force_address { } {
#=============================================================
global mom_dwell mom_dwell_toggle

if {$mom_dwell_toggle == 1} {
    MOM_force once cycle_dwell
} else {
    MOM_suppress once cycle_dwell
}

#MOM_output_literal "DWELL=$mom_dwell"
}
...
#=============================================================
  global cycle_name udv_s

if {[info exist cycle_name]} {
   if {$cycle_name == "LATHE_THREAD"} {
      LATHE_THREAD_SET
      MOM_force once saida_rosca ang_ferr P
      MOM_do_template thread_move_1
      MOM_force once G_motion F P
      MOM_do_template thread_move
   } else {
...
set tempo_anterior   $mom_machine_time
 ...
 if {$mom_next_oper_has_tool_change == "YES" || $mom_current_oper_is_last_oper_in_program  == "YES"} {
     ...
     #kr- MOM_do_template coolant_off
     MOM_output_literal "G0X700.Z700.T00"
     #MOM_output_literal " "
 }
...
global co ci
global prog_loop
global udv_tope_flag

 #MOM_output_literal "$co FIM DE PROGRAMA $ci"
...
#=============================================================
proc PB_CMD_historico_revisoes { } {
#=============================================================
global master_rev_date master_rev

set master_rev        ".00"
set master_rev_date    "05/AGO/15"

#==============================================================================
#  DATA        Revisao        Resp      Descricao
#==============================================================================
# 05/AGO/15    .00            ERC        - Liberação inicial
#------------------------------------------------------------------------------
#
}
...
MOM_output_literal "$co $mom_path_name - $mom_tool_name $ci"
}
...
#=============================================================
proc PB_CMD_old_tope { } {
#=============================================================
global prog_loop
global prog_start
global mom_turn_workpiece_diameter
global mom_path_name udv_tope udv_tope_flag

if {![info exists udv_tope]} {
      set udv_tope     -1
      set udv_tope_flag -1
}

if {$udv_tope == "TOPE" && [info exists mom_turn_workpiece_diameter] == 1} {

      set udv_diam [expr ($mom_turn_workpiece_diameter - 8)]

      #MOM_output_literal "M9"
      MOM_output_literal "M98 P2222"
      MOM_output_literal "T0707 M05 (TOPE)"
      MOM_output_literal "G0 X[format "%.2f" $udv_diam] Z0.5"
...
#=============================================================
  global mom_spindle_mode mom_spindle_maximum_rpm mom_spindle_speed

if {$mom_spindle_mode == "RPM"} {
    if {$mom_spindle_speed >= 210} {
        set usr_range 12
    } else {set usr_range 11}
    MOM_output_literal "M$usr_range "
     MOM_force once M_spindle S G_spin
     MOM_do_template spindle_rpm
  } elseif {$mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM"} {
     if {$mom_spindle_maximum_rpm >= 210} {
         set usr_range 12
     } else {set usr_range 11}
     MOM_output_literal "M$usr_range "
...
#=============================================================
proc PB_CMD_proc_ude { } {
#=============================================================
proc MOM_contra_ponto {} {
   PB_CMD_ude
}
...
#=============================================================
proc PB_CMD_redef_lathe_thread_set { } {
#=============================================================
#=============================================================
proc  LATHE_THREAD_SET { } {
#=============================================================
  global udv_m udv_s udv_a
  global udv_p udv_q udv_f udv_x
  global mom_total_depth_increment_type
  global mom_total_depth_finish_passes_number_of_passes
  global mom_machine_control_additional_commands
  global mom_turn_thread_pitch_lead
  global thread_type thread_increment feed_rate_mode
  global mom_total_depth_constant_increment mom_total_depth
  global mom_turn_thread_crest_line
  global mom_total_depth_finish_passes_increment udv_r

  switch $mom_total_depth_increment_type {
    0 {set thread_type CONSTANT}
    1 {set thread_type VARIABLE}
    2 {set thread_type PERCENTAGE}
  }
  set thread_type CONSTANT

  if {$thread_type == "CONSTANT"} {
      set udv_m $mom_total_depth_finish_passes_number_of_passes(0)
      set udv_s [expr (($mom_machine_control_additional_commands / $mom_turn_thread_pitch_lead) * 10)]
      set udv_a 60

      set udv_p [expr ($mom_total_depth * 1000)]
      set udv_q [expr ($mom_total_depth_constant_increment * 1000)]
      set udv_f $mom_turn_thread_pitch_lead

      set udv_x [expr ($mom_turn_thread_crest_line(0) - $mom_total_depth)]

      set udv_r $mom_total_depth_finish_passes_increment(0)
  }

    MOM_output_literal "(==> $udv_x)"
...
#=============================================================
proc PB_CMD_set_tope { } {
#=============================================================
global mom_path_name
global udv_tope

if {$mom_path_name == "TOPE"} {
    set udv_tope $mom_path_name
}
}
...
#=============================================================
 ...
 MOM_output_literal "M98 P2222"
 MOM_output_literal "T0707 M05 (TOPE)"
 MOM_output_literal "G0 X21 Z0.5"
 MOM_output_literal "M36"
 MOM_output_literal "M0"
 MOM_output_literal "M37"
}

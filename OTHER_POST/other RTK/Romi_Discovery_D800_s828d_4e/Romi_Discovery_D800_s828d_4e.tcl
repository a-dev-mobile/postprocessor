########################## TCL Event Handlers ##########################
#
#  Romi_Discovery_D800_s828d_4e.tcl - 4_axis_table
#
#    This is a 4-Axis Milling Machine With
#     Rotary Table.
#
#  Created by ronconi @ sexta-feira, 5 de janeiro de 2018 16:17:21 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "Romi_Discovery_D800_s828d_4e__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_coolant_code(THRU)                "38"
  set mom_sys_coolant_code(TAP)                 "38"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "900"
  set mom_kin_4th_axis_min_incr                 "0.001"
  set mom_kin_4th_axis_min_limit                "-900"
  ...
  set mom_kin_max_frn                           "14400"
  set mom_kin_min_arc_length                    "1"
  set mom_kin_min_arc_radius                    "1"
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   PB_CMD_force_address_rapid_move
   PB_CMD_spindle_on
   PB_CMD_detect_tollpath_type
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_unclamp_check
   MOM_do_template rapid_move
   PB_CMD_check_clamp_status
   set rapid_spindle_blk {G_motion G G_mode G_feed X Y Z D M_coolant}
   set rapid_spindle_x_blk {G_motion G G_mode G_feed X D M_coolant}
   set rapid_spindle_y_blk {G_motion G G_mode G_feed Y D M_coolant}
   set rapid_spindle_z_blk {G_motion G G_mode G_feed Z D M_coolant}
   set rapid_traverse_blk {G_motion G G_mode G_feed X Y Z M_coolant}
   set rapid_traverse_xy_blk {G_motion G G_mode G_feed X Y M_coolant}
   set rapid_traverse_yz_blk {G_motion G G_mode G_feed Y Z M_coolant}
   set rapid_traverse_xz_blk {G_motion G G_mode G_feed X Z M_coolant}
...
#=============================================================
   ...
   PB_CMD_cabecalho
   MOM_set_seq_on
   MOM_force Once G G_mode G_plane G_feed Text
...
#=============================================================
proc PB_CMD__check_block_rapid_traverse { } {
#=============================================================
# This custom command should return
#   1 : Output BLOCK
#   0 : No output

 global usr_oper_type

INFO --->>> usr_oper_type=$usr_oper_type
...
#=============================================================
global a_flag
global usr_B_angle  mom_msys_origin

set usr_arot    [expr 1 * $usr_B_angle]
set Ty          [expr 1 * [format %.3f $mom_msys_origin(1)]]
set Tz          [expr 1 * [format %.3f $mom_msys_origin(2)]]
...
if {$a_flag == 1 } {
    ...
    if {$usr_arot == 0} {
       #MOM_output_literal "TRANS X0 Y0 Z0"
            ...
            #MOM_output_literal "TRANS X0 Y$Ty Z$Tz"
    }
    #MOM_output_literal "AROT X$usr_arot"
}
}
...
#=============================================================
 ...
 set fname    [lindex [split $mom_part_name "\\"] end]
 ...
 #MOM_output_literal "$co PN:$fname OP:020 REV:01 SEG:A1 $mom_logname $ci"
 #MOM_output_literal "$co DMF220 - SIEMENS S840D 4E $ci"
 ...
 MOM_output_literal "$co DISCOVERY D800 - SIEMENS S828D 4E $ci"
...
#=============================================================
proc PB_CMD_check_clamp_status { } {
#=============================================================
 global usr_oper_type usr_clamp_status

 if {$usr_oper_type == 3 && $usr_clamp_status == "off"} {
     PB_CMD_clamp_fourth_axis
     set usr_clamp_status    "on"
 } elseif {$usr_oper_type == 5} {
     set usr_clamp_status    "off"
 }
}
...
if {$cycle_flag == 0} {
    ...
    MOM_force once X Y fourth_axis
...
if {$cycle_flag == 0} {
    ...
    MOM_force once X Y fourth_axis
        ...
        if {$usr_FDPR != $mom_cycle_step1} {
            set usr_FDPR  $mom_cycle_step1
            set cycle_flag    0
        }
        #if {$usr_DAM != $mom_cycle_step2} {
...
global usr_RPO usr_RPAP usr_POSS
global mom_cycle_delay_mode
...
#=============================================================
proc PB_CMD_check_feed_type { } {
#=============================================================
global mom_sys_feed_rate_mode_code feed_mode

if {$mom_sys_feed_rate_mode_code($feed_mode) == 93} {
    MOM_force once X Y Z fourth_axis F
}
}
...
#=============================================================
proc PB_CMD_detect_tollpath_type { } {
#=============================================================
 global usr_oper_type usr_clamp_status

 set usr_clamp_status    "undefined"
...
MOM_force once G_motion X Y Z fourth_axis

#INFO ===>> [DPP_GE_DETECT_5AXIS_TOOL_PATH] | $usr_oper_type
}
...
#=============================================================
proc PB_CMD_force_address_rapid_move { } {
#=============================================================
MOM_force once G_motion X Y fourth_axis D F
...
MOM_output_literal ";"
MOM_output_literal "SUPA X=GD_TC_POS\[0\] Y=GD_TC_POS\[1\] Z=GD_WPC_POS\[2\]"
...
#-- Atualizar estas variáveis com a letra e data da ultima revisao.... --------
 set usr_controle_rev    ".00"
 set usr_data_rev    "20/MAI/16"
#------------------------------------------------------------------------------
...
#==============================================================================
# 20/MAI/16    .00    WILLIAM        - Liberacao inicial ;
#------------------------------------------------------------------------------
#
}
...
global co ci
 ...
 #MOM_output_literal "$co OPERACAO: $mom_path_name $ci"
...
if {$usr_B_angle != $last_usr_B_angle} {
...
#MOM_output_literal ";|$mom_csys_matrix(0)|$mom_csys_matrix(1)|$mom_csys_matrix(2)|"
#MOM_output_literal ";|$mom_csys_matrix(3)|$mom_csys_matrix(4)|$mom_csys_matrix(5)|"
#MOM_output_literal ";|$mom_csys_matrix(6)|$mom_csys_matrix(7)|$mom_csys_matrix(8)|"

#--- Inicializa variaveis do TRANS
set reset_TRANS "TRANS X0.0 Y0.0 Z0.0"

if {![info exists g_lista]}  {lappend g_lista    "\$P_UIFR\[0\]=CTRANS(X,0.0,Y,0.0,Z,0.0,B,0); G53"}
...
set new_TRANS   "TRANS X[format %.3f $TXX] Y[format %.3f $TYY] Z[format %.3f $TZZ]"
...
#MOM_output_literal "==> $mom_csys_matrix(8) | $usr_fixture_offset "
...
if {$tr_flag == 0} {
    if {$mom_csys_matrix(8) == 1 && $usr_fixture_offset == 54} {
        ...
        } elseif {$last_TXX != $TXX || $last_TYY != $TYY || $last_TZZ != $TZZ} {
                  set out_line_0 $reset_TRANS
                  set out_line   $new_TRANS
                  set tr_flag    1
                  if {[lsearch -exact $g_lista $new_CTRANS] == -1} {
...
#=============================================================
global mom_motion_event  mom_spindle_speed last_RPM

if {![info exists last_RPM] || $mom_motion_event == "initial_move"} {set last_RPM   0}

#if {$mom_spindle_speed != $last_RPM} {MOM_force once S M_spindle}
...
#=============================================================
proc PB_CMD_unclamp_check { } {
#=============================================================
 global usr_oper_type usr_clamp_status mom_rotary_delta_4th

 if {$usr_clamp_status  ==  "undefined" || $mom_rotary_delta_4th != 0} {
     PB_CMD_unclamp_fourth_axis
     set usr_clamp_status    "off"
 }
}
...
if {![info exists mom_init_pos(3)]} {
    set usr_B_angle    $mom_out_angle_pos(0)
} else {
        if {$mom_kin_coordinate_system_type == "LOCAL"} {set mom_init_pos(3)   0}
        set usr_B_angle    [expr $mom_init_pos(3) + $mom_out_angle_pos(0)]
}
   ...
   set mom_pb_macro_arr(CYCLE83) \
       [list {CYCLE83 ( , ) 0 =} \
        {{{$mom_cycle_retract_to_pos($mom_cycle_spindle_axis)} 1 3 5 1 1 8 3} \
         ...
         {} \
         {{$usr_FDEP} 0} \
         {{$usr_FDPR} 1 3 5 1 1 8 3} \
         {{$usr_DAM} 1 3 3 1 1 6 3} \
         {{$usr_DTB} 1 3 3 1 1 6 3} \
         {{$usr_DTS} 1 3 3 1 1 6 3} \
         {{$usr_FRF} 1 3 5 1 1 8 3} \
         {{$usr_VARI} 1 0 2 0 0 2}}]
   ...
   set mom_pb_macro_arr(CYCLE84) \
       [list {CYCLE84 ( , ) 0 =} \
        {{{$mom_cycle_retract_to_pos($mom_cycle_spindle_axis)} 1 3 5 1 1 8 3} \
         ...
         {} \
         {{$usr_DTB} 1 3 3 1 1 6 3} \
         {{$usr_SDAC} 1 0 3 0 0 3} \
         {} \
         {{$usr_PIT} 1 3 3 1 1 6 3} \
         {{$usr_POSS} 1 0 5 0 0 5} \
         {{$usr_SST} 1 3 5 1 1 8 3} \
         {{$usr_SST} 1 3 5 1 1 8 3}}]

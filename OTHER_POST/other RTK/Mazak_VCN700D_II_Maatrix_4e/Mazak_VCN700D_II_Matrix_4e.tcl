########################## TCL Event Handlers ##########################
#
#  Mazak_VCN700D_II_Matrix_4e.tcl - 4_axis_table
#
#    This is a 4-Axis Milling Machine With
#     Rotary Table.
#
#  Created by ronconi @ terça-feira, 9 de janeiro de 2018 14:07:56 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "Mazak_VCN700D_II_Matrix_4e__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_rapid_feed_mode(LINEAR_ROTARY)    "FRN"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_rapid_feed_rate                   "8001"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion G_feed X Y Z fourth_axis}
   set rapid_traverse_xy_blk {G_motion G_feed X Y fourth_axis}
   set rapid_traverse_yz_blk {G_motion G_feed Y Z fourth_axis}
   set rapid_traverse_xz_blk {G_motion G_feed X Z fourth_axis}
...
#=============================================================
   ...
   MOM_force Once G_mode G S M_spindle
...
#=============================================================
   ...
   MOM_set_seq_off
   PB_CMD_inicializacao_vars_proc
   MOM_set_seq_on
   PB_CMD_inicio_programa
   PB_CMD_Revisoes
...
#=============================================================
proc PB_CMD_HSMode_desliga { } {
#=============================================================
global HSMode  mom_current_motion
...
if {$HSMode == 1} {
    ...
    #-- Suspende HSMode para executar GO --
    set HSMode    2
}
}
...
#=============================================================
proc PB_CMD_HSMode_liga { } {
#=============================================================
global HSMode HSMrate

if {$HSMode == 1} {
    # (PARADA EXATA ON)
    MOM_output_literal "G61.1 K$HSMrate"
    # (HIGH SPEED MODE ON)
    MOM_output_literal "G05 P2 "
    MOM_suppress once D
}
}
...
if {$HSMode == 2} {
    ...
    set HSMode    1
    MOM_suppress once D
} elseif {$HSMode == 1} {MOM_suppress once D}


global mom_sys_feed_rate_mode_code  feed_mode

if {$mom_sys_feed_rate_mode_code($feed_mode) == 93} {MOM_force once F}
}


#=============================================================
proc PB_CMD_Revisoes { } {
#=============================================================
global co ci

MOM_output_literal "$co REV.PP: F - 17/NOV/10 $ci"

#
#   DATA    REV     RESP.        DESCRIÇÃO
#=============================================================================
# 17/03/10    -    William        - Liberação inicial para VSE
#-----------------------------------------------------------------------------
# 29/03/10    A    Kleber        - Retirada a "%", inicio e final;
#                    - Removidos os endereços: H,D e G43
#-----------------------------------------------------------------------------
# 26/ABR/10    B    Kleber        - G84: alterado avanço para MMPR
#-----------------------------------------------------------------------------
# 05/JUL/10    C    Kleber        - Acresc. PB_HSMode_desliga no End_of_Program;
#                    - Acresc. variável K na UDE HSMode;
#                    - Acresc. Parametro K nas procs HSM.
#-----------------------------------------------------------------------------
# 20/AGO/10    D    Kleber        - Acresc G93 a avanço inverse time
#-----------------------------------------------------------------------------
# 20/AGO/10    E    Kleber        - Alterado coolant THRU de M26 para M51
#-----------------------------------------------------------------------------
# 17/NOV/10    F    Kleber        - Alterada a rotação do eixo C para
#                    "REVERSED".
#-----------------------------------------------------------------------------
}
...
#--- Desliga HSMode no Final de cada operacao ---
global HSMode mom_High_Speed_Mode

if {$HSMode == 2} {
     set HSMode           0
     set mom_High_Speed_Mode    "OFF"
} elseif {$HSMode == 1} {
     set HSMode           0
     set mom_High_Speed_Mode    "OFF"
...
#=============================================================
proc PB_CMD_inicializacao_vars_proc { } {
#=============================================================
...
proc MOM_HSM_mode {} {
#-----------------------------------------------------
  global mom_High_Speed_mode mom_HSM_rate
  global HSMode HSMrate

  if {$mom_High_Speed_mode == "ON"} {
      set HSMode    1
      set HSMrate    $mom_HSM_rate
  }
}
#=========================================================================
}
...
#=============================================================
global mom_path_name     mom_tool_name   mom_tool_number
global mom_tool_diameter mom_tool_corner1_radius

MOM_output_literal "( OPERACAO: $mom_path_name - T$mom_tool_number $mom_tool_name)"

if { [info exists mom_tool_corner1_radius] } {
    MOM_output_literal "( DIAM. [format "%.2f" $mom_tool_diameter] RC [format "%.2f" $mom_tool_corner1_radius] )"
} else {
      MOM_output_literal "( DIAM. [format "%.2f" $mom_tool_diameter] )"
}
}
...
set ci             $mom_sys_control_in
 ...
 MOM_output_literal "$co MAQUINA  : MAZAK VCN700D_II 4 EIXOS  Comando : Matrix $ci"

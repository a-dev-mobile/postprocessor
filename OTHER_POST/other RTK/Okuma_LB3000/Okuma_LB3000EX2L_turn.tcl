########################## TCL Event Handlers ##########################
#
#  Okuma_LB3000EX2L_turn.tcl - lathe
#
#
#  Created by ronconi @ sexta-feira, 5 de janeiro de 2018 11:28:02 E. South America Daylight Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "Okuma_LB3000EX2L_turn__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_cycle_drill_deep_code             "74"
...
#=============================================================
  ...
  global turret_current
   ...
   if { [string compare $mom_tool_head "$ind_head"] && \
      ...
      CATCH_WARNING "mom_tool_head = $mom_tool_head IS INVALID, USING REAR"
   }
}
...
#=============================================================
   ...
   MOM_set_seq_off
   PB_CMD_controle_revisao
   PB_CMD_redef_MOM_head
...
if {$mom_auxfun == "TRUE"} {
   if {$mom_auxfun_text_defined == 1} {
      MOM_output_literal "M$mom_auxfun ($mom_auxfun_text)"
   } else {
      MOM_output_literal "M$mom_auxfun"
   }
}
}
...
#=============================================================
proc PB_CMD_call_sub { } {
#=============================================================
global mom_ude_turret_index
global mom_ude_oalar
global mom_ude_zero_offset
global mom_ude_ofeed
global mom_ude_osug
global mom_ude_orera_orerb

if {$mom_ude_turret_index == "A"} {
   set offset_msg "ZERO OFFSET MAIN SPINDLE TORRE A"
} elseif {$mom_ude_turret_index == "B"} {
   set offset_msg "ZERO OFFSET SUB SPINDLE TORRE B"
} else {
   set offset_msg "**********   ATENCAO: TORRE NAO DEFINIDO   **********"
}

MOM_set_seq_off
if {$mom_ude_oalar == "TRUE"} {
   MOM_output_literal "CALL OALAR (VERIFICACAO DE ALARMES)"
   set mom_ude_oalar "FALSE"
}
if {$mom_ude_zero_offset == "TRUE"} {
   MOM_output_literal "CALL OZOM$mom_ude_turret_index ($offset_msg)"
   set mom_ude_zero_offset "FALSE"
}
if {$mom_ude_ofeed == "TRUE"} {
   MOM_output_literal "CALL OFEED (ALIMENTADOR DE BARRAS)"
   set mom_ude_ofeed "FALSE"
}
if {$mom_ude_osug == "TRUE"} {
   MOM_output_literal "CALL OSUG (SUGAR A PECA)"
   set mom_ude_osug "FALSE"
}
if {$mom_ude_orera_orerb == "TRUE"} {
   MOM_output_literal "CALL ORER$mom_ude_turret_index (RETIRAR RETALHO TORRE $mom_ude_turret_index)"
   set mom_ude_orera_orerb "FALSE"
}
#kr- MOM_set_seq_on
}
...
#=============================================================
proc PB_CMD_controle_revisao { } {
#=============================================================

MOM_output_literal "(PP. REV: .02 - 10/FEV/15 - Kleber)"
#-------------------------------------------------------------------
# REVISAO: "(PP. REV: .02 - 10/FEV/15 - Kleber)"
# - Foi removido cabecalho do programa;
# - Foi invertido M3 com M4;
# - Acresc. comandos M90 e M157 no inicio do programa;
# - Acresc. redefinicao da proc MOM_head para controlar o contra-ponto;
# - Acresc. seq. de blocos no final do programa:
#    * M05
#    * T1212 (PGM TOPE)
#    * G0 X50. Z.5
#    * M91
#===================================================================
# REVISAO: "(PP. REV: .01 - 06/MAI/14)"
# - Ajustes gerais.
#-------------------------------------------------------------------
}
...
global mom_machine_time
   ...
   if {$mom_machine_time >= "1"} {
       ...
       MOM_output_literal "(TEMPO TOTAL DE USINAGEM: $min\' $seg\")"
   } elseif {$mom_machine_time < "1"} {
       set seg [expr int($mom_machine_time*60)]
       MOM_output_literal "(TEMPO TOTAL DE USINAGEM: 0\' $seg\")"
   }

#kr- MOM_output_literal "\x04"
}


#=============================================================
proc PB_CMD_force_block { } {
#=============================================================
  MOM_force once X Z M_spindle M_coolant F G_feed
}
...
#=============================================================
proc PB_CMD_ini_G71_thread_vars { } {
#=============================================================
global mom_tool_left_angle mom_tool_right_angle
global mom_total_depth_angle RAD2DEG
global thread_angle tool_angle

set tool_angle      [expr ($mom_tool_left_angle + $mom_tool_right_angle) * $RAD2DEG]
set thread_angle    [expr $mom_total_depth_angle * $RAD2DEG]


#INFO ==> $thread_angle Ta=tool_angle
...
#=============================================================
  ...
  if {$mom_spindle_mode == "RPM"} {
     MOM_force once M_spindle S G_spin G_motion
     MOM_do_template spindle_rpm
  } elseif {$mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM"} {
     MOM_force once M_spindle S G G_spin G_motion
...
global mom_prefun_text_defined

if {$mom_prefun == "TRUE"} {
   if {$mom_prefun_text_defined == 1} {
      MOM_output_literal "G$mom_prefun ($mom_prefun_text)"
   } else {
      MOM_output_literal "G$mom_prefun"
   }
}
}
...
#---------------------------------
 ...
 if {$mom_head_name == "M56"} {
     MOM_force once G_motion X Z
     MOM_do_template gohome
     MOM_output_literal "M56"
 } elseif {$mom_head_name == "M55"} {
     MOM_force once G_motion X Z
     MOM_do_template gohome
     MOM_output_literal "M55"
     MOM_output_literal "M847"
 }
...
global mom_date
global mom_sys_spindle_max_rpm_code
global mom_ude_turret_type
global mom_ude_turret_index
global mom_ude_spindle_type
global mom_auxfun_text_defined
global mom_prefun
global mom_prefun_text_defined
global mom_ude_oalar
...
set mom_machine_name "OKUMA LB3000 EX2 L"
set mom_sys_spindle_max_rpm_code "50"
set mom_ude_turret_type "NONE"
set mom_ude_turret_index "NONE"
set mom_ude_spindle_type "NONE"
set mom_auxfun_text_defined "FALSE"
set mom_prefun "FALSE"
set mom_prefun_text_defined "FALSE"
set mom_ude_oalar "FALSE"
set mom_ude_zero_offset "FALSE"
set mom_ude_ofeed "FALSE"
set mom_ude_osug "FALSE"
set mom_ude_orera_orerb "FALSE"
   ...
   #kr- MOM_output_literal "O[string toupper $mom_output_file_basename]"
   #kr- MOM_output_literal "(ARQUIVO: [string toupper $mom_part_name)]"
   #kr- MOM_output_literal "(PROGRAMADOR: [string toupper $mom_logname)]"
   #kr- MOM_output_literal "(MAQUINA: [string toupper $mom_machine_name)]"
   #kr- MOM_output_literal "(DATA: [string toupper $mom_date)]"

MOM_output_literal "M90 (FECHA A PORTA)"
MOM_output_literal "M157 (LIBERA CONTRA PONTA)"
MOM_output_literal " "
}
...
global mom_ude_dpin
global mom_ude_roro
global mom_ude_ps_rc
global mom_ude_turret_index

MOM_set_seq_off

 if {$mom_ude_turret_type != "NONE"} {

    if {$mom_ude_turret_type == "A"} {
        MOM_output_literal "G13 (TORRE $mom_ude_turret_type)"
        set mom_ude_turret_index "A"
    } elseif {$mom_ude_turret_type == "B"} {
        MOM_output_literal "G14 (TORRE $mom_ude_turret_type)"
        set mom_ude_turret_index "B"
    }

    if {$mom_ude_spindle_type != "NONE"} {
       if {$mom_ude_spindle_type == "MAIN_SPINDLE"} {
           MOM_output_literal "G140 (ATIVA MODO MAIN SPINDLE)"
       } elseif {$mom_ude_spindle_type == "SUB_SPINDLE"} {
           MOM_output_literal "G141 (ATIVA MODO SUB SPINDLE)"
       }
    }

MOM_output_literal "(**********   DEFINIR ZERO OFFSET   **********)"
MOM_output_literal "COMP=[format %.3f $mom_ude_comp] (COMPRIMENTO P/ PRIMEIRA USINAGEM)"
MOM_output_literal "PECA=[format %.3f $mom_ude_peca] (COMPRIMENTO DA PECA)"
MOM_output_literal "FIXA=[format %.3f $mom_ude_fixa] (COMPRIMENTO P/ FIXAR LADO 2)"
MOM_output_literal "LBAR=[format %.3f $mom_ude_lbar] (COMPRIMENTO TOTAL DA BARRA)"
MOM_output_literal "QPEC=[format %.3f $mom_ude_qpec] (NUMERO DE PECAS POR BARRA)"
MOM_output_literal "SBMT=[format %.3f $mom_ude_sbmt] (SOBREMETAL P/ FACEAR)"
MOM_output_literal "BED=[format %.3f $mom_ude_bed] (LAGURA DO BEDAME)"
MOM_output_literal "BARR=[format %.3f $mom_ude_barr] (DIAMETRO DA BARRA EM BRUTO)"
MOM_output_literal "DCUT=[format %.3f $mom_ude_dcut] (DIAMETRO DA PECA P/ CORTAR)"
MOM_output_literal "DFUR=[format %.3f $mom_ude_dfur] (DIAMETRO MENOR P/ CORTE)"
MOM_output_literal "DPIN=[format %.3f $mom_ude_dpin] (DIAMETRO DA PINCA SUB SPINDLE)"
MOM_output_literal "RORO=[format %.3f $mom_ude_roro] (ROTACAO P/ USINAGEM DA ROSCA)"
MOM_set_seq_off
MOM_output_literal "(*********************************************)"
MOM_output_literal "CLEAR"
MOM_output_literal "DEF WORK"
MOM_output_literal "$mom_ude_ps_rc"
MOM_output_literal "END"
MOM_output_literal "DRAW"
...
#=============================================================
   ...
   set mom_pb_macro_arr(G71) \
       [list {G71 { } { } {} 1 {}} \
        {{{$mom_pos(0) * $mom_sys_lathe_x_factor * $mom_sys_lathe_x_double} 1 3 3 1 1 6 3 X} \
         {{$mom_pos(2) * $mom_sys_lathe_z_factor} 1 3 3 1 1 6 3 Z} \
         {{$thread_angle} 1 3 3 1 1 6 3 A} \
         {{$tool_angle} 1 3 3 1 1 6 3 B} \
         {{$mom_total_depth_tolerance  * $mom_sys_lathe_x_factor  * $mom_sys_lathe_x_double} 1 3 3 1 1 6 3 D} \
         {0.02 1 3 3 1 1 6 3 U} \
         {{$mom_total_depth * $mom_sys_lathe_x_factor  * $mom_sys_lathe_x_double} 1 3 3 1 1 6 3 H} \
         {{$mom_programmed_feed_rate} 1 3 3 1 1 6 3 F} \
         {2 0 M3} \
         {5 0 M7}}]
}

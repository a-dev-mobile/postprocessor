########################## TCL Event Handlers ##########################
#
#  dmu60_evo_tnc530_mod.tcl -
#
#  Created by danilov  @  4 ìàÿ 2017 ã. 15:04:17 RTZ 2 (зима)
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "dmu60_evo_tnc530_mod__IN.pui"
...
global mom_group_name modification_group
    ...
    if {[string match "*$mom_current_modification*" $mom_part_modification] } {
          ...
          return
           ...
           GROUP_end_list {RESET_modification}
}
...
#=============================================================
global mom_pos_arc_plane saved_cc
global mom_pos_arc_center

VAR_GROUP_LIST saved_cc

if { $mom_pos_arc_plane == "XY" } {
      set new_cc "I[format "%0.3f" $mom_pos_arc_center(0)]J[format "%0.3f" $mom_pos_arc_center(1)]"
      if {[info exist saved_cc] && $saved_cc == $new_cc } { return 0 }
      MOM_force once circle_center I J
      MOM_suppress once K
      return 1
} elseif { $mom_pos_arc_plane == "ZX" } {
      set new_cc "I[format "%0.3f" $mom_pos_arc_center(0)]K[format "%0.3f" $mom_pos_arc_center(2)]"
      if {[info exist saved_cc] && $saved_cc == $new_cc } { return 0 }
      MOM_force once circle_center I K
      MOM_suppress once J
      return 1
} elseif { $mom_pos_arc_plane == "YZ" } {
      set new_cc "J[format "%0.3f" $mom_pos_arc_center(1)]K[format "%0.3f" $mom_pos_arc_center(2)]"
      if {[info exist saved_cc] && $saved_cc == $new_cc } { return 0 }
...
global saved_cc
if { [info exist saved_cc]} { unset saved_cc }
...
if { [EQ_is_gt $mom_helix_angle 3600.0]} {
        set h_d [expr $mom_pos(2) - $mom_prev_pos(2)]
        set h_a $mom_arc_angle
     while { $h_a > 3600.0 } {
             set h_a [expr $h_a - 3600.0]
             set h_d [expr $h_d - ($mom_helix_pitch * 10.0)]
             set mom_helix_angle [expr 3600.0 * $helix_mod]
             set mom_helix_depth [expr $mom_helix_pitch * 10.0]

...
#  dmu60mb_hhain.tcl -
#
#  Created by danilov  @  25 èþëÿ 2017 ã. 9:45:22 RTZ 2 (зима)
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "dmu60mb_hhain__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_post_description                  "DMU 60 monoBlock iTNC530"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_rapid_feed_rate                   "50000"
...
#=============================================================
   ...
   PB_CMD_cycle_reposition
   PB_CMD_cycle_user_reaming
...
#=============================================================
  global cycle_init_flag
   ...
   PB_CMD_cycle_reposition
   PB_CMD_TNC_CYCLE257
   MOM_force Once G_motion Z Y X
   MOM_do_template drill_boss_milling
   set cycle_init_flag FALSE
}
...
#=============================================================
  global cycle_init_flag
   ...
   PB_CMD_TNC_CYCLE267
   MOM_force Once G_motion X Y Z
   MOM_do_template drill_boss_thread_milling
   set cycle_init_flag FALSE
}
...
#=============================================================
   ...
   PB_CMD_cycle_reposition
   PB_CMD_set_cycle_pos_list
   PB_CMD_cycle_user_drill_break_chip
   MOM_force Once G_motion X Y Z
...
#=============================================================
  global cycle_init_flag
   ...
   global mom_bmg_diameter


   PB_CMD_cycle_reposition
   PB_CMD_TNC_CYCLE_map
   PB_CMD_TNC_CYCLE208
   MOM_force Once G_motion X Y Z f_max
   MOM_do_template cycle_bore_milling
   PB_CMD_cycle_time
   set cycle_init_flag FALSE
}
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   PB_CMD_path_end_command
   MOM_do_template spindle_and_coolant
   PB_CMD_path_end_5ax
...
#=============================================================
  ...
  global cycle_init_flag
   ...
   set cycle_init_flag TRUE
   set cycle_name HH_CHAMFER
   CYCLE_SET
}


#=============================================================
proc MOM_hh_chamfer_move { } {
#=============================================================
  global cycle_init_flag
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_user_rapid_move
   set rapid_traverse_blk {G_motion X Y Z fourth_axis fifth_axis f_max M_spindle air_coolant}
...
if {[info exist mom_cutter_data_output_indicator] && $mom_cutter_data_output_indicator == 1} { return }
    ...
    if { [info exists mom_pb_operation] && $mom_pb_operation == "TRAORI" } {

         if {[info exists mom_tool_axis_max_change_at_corner] && \
               ![EQ_is_equal $mom_tool_axis_max_change_at_corner 0] } {
                       set hsc_angle_ [format %0.1f [expr $mom_tool_axis_max_change_at_corner * $RAD2DEG]]
         } else { set hsc_angle_ "1" }
    } else { set hsc_angle_ "1" }
...
if {![info exist mom_tnc_cycle_q257]} {
     ...
     } else { set mom_tnc_cycle_q257 [expr $mom_tnc_cycle_q201/2] }
} ; # Q257
if {![info exist mom_tnc_cycle_q256]} {
...
global mom_spindle_speed
global user_saved_spindle_speed
...
MOM_output_literal "TOOL CALL $mom_tool_number Z [format "S%0.0f" $mom_spindle_speed]"
set user_saved_spindle_speed [format "%0.0f" $mom_spindle_speed]
MOM_output_text "; ========================="
...
#MOM_output_to_listing_device " Ïðåâûøåí ïðåäåë ïîâîðîòà ïî îñè Â ñòàíêà \n\n  óãîë 104 ãðàäóñà"
#MOM_abort
...
# set source_path [MOM_ask_env_var USRPOST_SEQURE]
# source ${source_path}dmu60mb_source.tcl
...
MOM_output_text ";=============================================="
MOM_output_text "; DMU 60 MONOBLOCK iTNC530"
MOM_output_text "; DATE [string toupper $mom_date] :  USER - [string toupper $mom_logname]"
MOM_set_seq_on
MOM_output_literal "DECLARE STRING QS10 = \"DMU60MB\""
MOM_output_literal "CALL PGM TNC:\\SFK_DMU60MB.H"
...
#=============================================================
proc PB_CMD_coolant_adapt { } {
#=============================================================
global mom_air_coolant_mode
global mom_coolant_mode
global mom_coolant_status
global mom_sys_coolant_code
global air_coolant_command

if { $mom_air_coolant_mode == "OFF" && $mom_coolant_status != "OFF" } {

       set air_coolant_command(AUTO)  "M$mom_sys_coolant_code($mom_coolant_status)"
       set mom_air_coolant_mode       "AUTO"

} elseif { $mom_coolant_status == "OFF" } {
       set mom_air_coolant_mode       "OFF"
}
}
...
#=============================================================
proc PB_CMD_cycle_bore_milling { } {
#=============================================================
global user_cycle_saved_param_list
...
global mom_Q334_step
global mom_Q335_diameter
global mom_Q342_start_diameter
global mom_Q351_direction

set user_cycle_z_coord [format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]
set user_cycle_param_list [format "%0.3f" $mom_cycle_rapid_to]
lappend user_cycle_param_list $user_cycle_z_coord
lappend user_cycle_param_list [format "%0.3f" $mom_cycle_feed_to]
lappend user_cycle_param_list [format "%0.3f" $mom_cycle_retract_to]
#===============================

if { ![info exist mom_Q342_start_diameter] } {
     set mom_Q342_start_diameter [format "%0.3f" [expr $mom_Q335_diameter * 0.9] ]
   }

join $user_cycle_param_list
#===============================

if { ![info exist user_cycle_saved_param_list] ||
            $user_cycle_saved_param_list != $user_cycle_param_list } {

MOM_output_literal "CYCL DEF 208 BORE MILLING ~"
MOM_output_text "    Q200=[format "%0.3f" $mom_cycle_rapid_to] ;SET-UP CLEARANCE ~"
MOM_output_text "    Q201=[format "%0.3f" $mom_cycle_feed_to] ;DEPTH ~"
MOM_output_text "    Q206=[format "%0.0f" $mom_cycle_feed_rate] ;FEED RATE FOR PLNGNG ~"
MOM_output_text "    Q334=[format "%0.3f" $mom_Q334_step] ;PLUNGING DEPTH ~"
MOM_output_text "    Q203=$user_cycle_z_coord ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=[format "%0.3f" $mom_cycle_retract_to] ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q335=[format "%0.3f" $mom_Q335_diameter] ;NOMINAL DIAMETER ~"
MOM_output_text "    Q342=[format "%0.3f" $mom_Q342_start_diameter] ;ROUGHING DIAMETER ~"
MOM_output_text "    Q351=$mom_Q351_direction ;"
}
set user_cycle_saved_param_list $user_cycle_param_list
#==================================
global mom_toolpath_cutting_time PI

set bmg_len [expr abs($mom_cycle_feed_to - $mom_cycle_rapid_to) / $mom_Q334_step * $PI * $mom_Q335_diameter]
set mom_toolpath_cutting_time [expr $mom_toolpath_cutting_time + $bmg_len]
...
#=============================================================
proc PB_CMD_cycle_boss_milling { } {
#=============================================================
global user_cycle_saved_param_list
...
global mom_Q202_step
global mom_Q223_diameter
global mom_Q222_start_diameter
global mom_Q368_allowance
global mom_Q370_overstep
global mom_Q351_direction
...
if { ![info exist mom_Q222_start_diameter] } {
     set mom_Q222_start_diameter [format "%0.3f" [expr $mom_Q223_diameter * 0.9] ]
   }
...
MOM_output_literal "CYCL DEF 257  ~"
MOM_output_text "    Q223=[format "%0.3f" $mom_Q223_diameter] ;DIAMETER ~"
MOM_output_text "    Q222=[format "%0.3f" $mom_Q222_start_diameter] ;ROUGH DIAMETER ~"
MOM_output_text "    Q368=[format "%0.3f" $mom_Q368_allowance] ;PRIPUSK NA STORONU ~"
MOM_output_text "    Q207=[format "%0.0f" $mom_cycle_feed_rate] ;FEED RATE ~"
MOM_output_text "    Q351=$mom_Q351_direction ;NAPRAVLENIE REZANIYA"
MOM_output_text "    Q201=[format "%0.3f" $mom_cycle_feed_to] ;DEPTH ~"
MOM_output_text "    Q202=[format "%0.3f" $mom_Q202_step] ;PLUNGING DEPTH ~"
MOM_output_text "    Q206=[format "%0.0f" $mom_cycle_feed_rate] ;FEED RATE FOR PLNGNG ~"
MOM_output_text "    Q200=[format "%0.3f" $mom_cycle_rapid_to] ;SET-UP CLEARANCE ~"
MOM_output_text "    Q203=$user_cycle_z_coord ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=[format "%0.3f" $mom_cycle_retract_to] ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q370=[format "%0.3f" $mom_Q370_overstep] ;PEREKRYTYE INSTRUMENTA "
}
set user_cycle_saved_param_list $user_cycle_param_list
#==================================
global mom_toolpath_cutting_time PI

set bmg_len [expr abs($mom_cycle_feed_to - $mom_cycle_rapid_to) / $mom_Q202_step * $PI * $mom_Q223_diameter]
set mom_toolpath_cutting_time [expr $mom_toolpath_cutting_time + $bmg_len]
...
#=============================================================
proc PB_CMD_cycle_boss_millling_data { } {
#=============================================================
global mom_hm_diameter
global mom_hm_peck
...
if { ![info exist mom_cycle_event_type] } {
   MOM_abort "NO CYCLE DATA DEFINED"
}

if { $mom_cycle_event_type == "boss_gm" } {

  set mom_Q202_step $mom_hm_peck
  set mom_Q223_diameter $mom_hm_diameter
  set mom_Q322_start_diameter $mom_hm_start_diameter

  if { ![info exist mom_hm_side_allowance] } {
     set mom_Q368_allowance 0.0
   } else { set mom_Q368_allowance $mom_hm_side_allowance }

  if { $mom_hm_direction != 1 && $mom_hm_direction != -1 } {
       set mom_Q351_direction 1
  } else { set mom_Q351_direction $mom_hm_direction }

  set mom_Q370_overstep $mom_hm_overstep

} else { MOM_abort "Operation data not completed" }

}


#=============================================================
proc PB_CMD_cycle_boss_threadmilling_data { } {
#=============================================================
global mom_thr_diameter
global mom_thr_dir
...
global mom_Q335_diameter
global mom_Q239_pitch
global mom_Q355_npitch
global mom_Q351_direction

global mom_thmg_diameter
global mom_thmg_pitch
global mom_thmg_climb
global mom_thmg_type
global mom_thmg_steps

global mom_siemens_cycle_diath
global mom_siemens_cycle_pit
global mom_siemens_cycle_cdir
global mom_siemens_cycle_typh
global mom_siemens_cycle_depth_dir

global mom_cycle_event_type

if { ![info exist mom_cycle_event_type] } { MOM_abort "NO CYCLE DATA DEFINED" }

if { $mom_cycle_event_type == "hhain_tm" } {
# Hhain threadmill data

    set mom_Q335_diameter $mom_thmg_diameter
    set mom_Q239_pitch $mom_thmg_pitch

    if { $mom_thmg_climb == "CLW" } {
         set mom_Q351_direction -1
    } else { set mom_Q351_direction 1 }

    if { [info exist mom_thmg_steps] && $mom_thmg_steps == "TRUE" } {
         set mom_Q355_npitch 1
       } else { set mom_Q355_npitch -1 }

} elseif { $mom_cycle_event_type == "cycle90_bth" } {
# Siemens threadmill data

    set mom_Q351_direction 1

if { $mom_siemens_cycle_typh == "1" } {

    if { $mom_siemens_cycle_cdir == "RIGHT" } {
        set mom_Q335_diameter [expr abs($mom_siemens_cycle_diath)]
        if { $mom_siemens_cycle_depth_dir == "UP" } { set mom_Q351_direction -1 }
    } else {
        set mom_Q335_diameter [expr -abs($mom_siemens_cycle_diath)]
        if { $mom_siemens_cycle_depth_dir == "DOWN" } { set mom_Q351_direction -1 }
     }
}

    set mom_Q335_diameter $mom_siemens_cycle_diath
    set mom_Q355_npitch -1
    set mom_Q239_pitch $mom_siemens_cycle_pit

} elseif { $mom_cycle_event_type == "general_btm" } {
# General threadmill data

   set mom_Q335_diameter [expr abs($mom_thr_diameter)]

   if { [string match "*RI*" $mom_thr_dir] } {
        set mom_Q239_pitch [expr abs($mom_thr_pitch)]
   } else { set mom_Q239_pitch [expr -abs($mom_thr_pitch)] }

   if { $mom_thr_mach_dir != -1 && $mom_thr_mach_dir != 1 } {
        set mom_Q351_direction 1
   } else { set mom_Q351_direction $mom_thr_mach_dir }

   if { ![info exist mom_thr_npitch] || $mom_thr_npitch < 0 } {
       set mom_Q355_npitch 1
   } else { set mom_Q355_npitch $mom_thr_npitch }
...
#=============================================================
proc PB_CMD_cycle_chamfer_milling { } {
#=============================================================
...
global mom_phelix_diameter
global mom_phelix_direction
global mom_phelix_depth
global mom_phelix_angle
global mom_phelix_finish_angle

global mom_ch_diam
global mom_ch_depth
global mom_ch_mach_direction
global mom_ch_pitch

if { $mom_ch_diam == 0.0 } { MOM_abort "WRONG CYCLE DATA" }
#===============================

if { ![info exist mom_tool_tip_diameter] } {
     set tool_tip_diam 0.0
 } else {
     set tool_tip_diam $mom_tool_tip_diameter }

if { [info exist mom_tool_point_angle] && $mom_tool_point_angle != 0 } {
     set faze_angle [expr $mom_tool_point_angle / 2]
} elseif { [info exist mom_tool_tip_angle] && $mom_tool_tip_angle != 0 } {
     set faze_angle [expr $mom_tool_tip_angle / 2]
} elseif { [info exist mom_tool_included_angle] && $mom_tool_included_angle != 0 } {
     set faze_angle [expr $mom_tool_included_angle / 2]
} else { MOM_abort "WRONG TOOL DATA" }

set mom_phelix_depth [expr $mom_cycle_rapid_to + $mom_ch_depth]
set mom_phelix_diameter [expr ($mom_ch_diam / 2) - ($mom_ch_depth * tan($faze_angle)) - ($tool_tip_diam / 2)]
set mom_phelix_angle [expr floor(($mom_cycle_rapid_to + $mom_ch_depth) / $mom_ch_pitch * 360.0)]

if { $mom_ch_mach_direction == "CCLW" } {
     set mom_phelix_direction "DR+"
     set mom_phelix_finish_angle 360.0
  } else {
     set mom_phelix_direction "DR-"
     set mom_phelix_angle [expr -$mom_phelix_angle]
     set mom_phelix_finish_angle -360.0
  }

set mom_pos_arc_center(0) $mom_cycle_rapid_to_pos(0)
set mom_pos_arc_center(1) $mom_cycle_rapid_to_pos(1)

MOM_force once ip_angle ip_helix ip_z ip_radius circle_direction X Y Z F

MOM_output_text "; FASKA D[format "%0.3f" $mom_ch_diam] X [format "%0.3f" $mom_ch_depth]"
#==================================
global mom_toolpath_cutting_time PI

set bmg_len [expr abs($mom_cycle_feed_to - $mom_cycle_rapid_to) / $mom_ch_pitch * $PI * 2 * $mom_phelix_diameter]
set mom_toolpath_cutting_time [expr $mom_toolpath_cutting_time + $bmg_len]
...
###  Sinumerik data
   ...
   set mom_Q335_diameter [expr $mom_siemens_cycle_prad * 2]
   set mom_Q334_step $mom_siemens_cycle_mid

   if { $mom_siemens_cycle_cdir == 2 } {
      set mom_Q351_direction -1
    } else { set mom_Q351_direction 1 }

  set mom_Q342_start_diameter [expr $mom_Q335_diameter * 0.9]

} elseif { $mom_cycle_event_type == "hhain_hm" } {
###   Hhain data

  set mom_Q334_step $mom_bmg_peck
  set mom_Q335_diameter $mom_bmg_hole_diameter
  set mom_Q342_start_diameter $mom_roughing_diameter
  ...
  set mom_Q334_step $mom_hm_peck
  set mom_Q335_diameter [expr abs($mom_hm_diameter)]
  set mom_Q342_start_diameter $mom_hm_start_diameter
...
global cycle_reposition_z
global mom_cycle_feed_rate_back
global cycle_retract_plane

if { [info exist user_cycle_saved_param_list] } {
  unset user_cycle_saved_param_list
}
if { [info exist user_saved_tool_vector] } {
     unset user_saved_tool_vector
}
if { [info exist mom_cycle_feed_rate_back] } {
     unset mom_cycle_feed_rate_back
}
if { [info exist cycle_retract_plane] } {
     unset cycle_retract_plane
}
}


#=============================================================
proc PB_CMD_cycle_out_thread_milling { } {
#=============================================================
global user_cycle_saved_param_list
...
if { $mom_tool_type != "Thread Mill" } {
MOM_output_to_listing_device "Â îïåðàöèè ôðåçåðîâàíèÿ ðåçüáû çàäàí íåâåðíûé òèï èíñòðóìåíòà !!!"
#MOM_abort
  set mom_tool_pitch $mom_Q239_pitch
}
...
set hole_deep [expr abs($mom_cycle_rapid_to) + abs($mom_cycle_feed_to)]

if { $mom_Q355_npitch == -1 } {
 if { $hole_deep < $mom_tool_flute_length } {
      set mom_Q355_npitch 0
   } else {  set mom_Q355_npitch [expr floor($mom_tool_flute_length / $mom_tool_pitch) - 1] }
}

lappend user_cycle_param_list $mom_Q355_npitch
join $user_cycle_param_list
#===============================
...
MOM_output_literal "CYCL DEF 267 ~"
MOM_output_text "    Q335=[format "%0.3f" $mom_Q335_diameter] ;NOMINAL DIAMETER ~"
MOM_output_text "    Q239=[format "%0.3f" $mom_Q239_pitch] ;THREAD PITCH ~"
MOM_output_text "    Q201=[format "%0.3f" $mom_cycle_feed_to]  ;DEPTH OF THREAD ~"
MOM_output_text "    Q355=$mom_Q355_npitch  ;THREADS PER STEP ~"
MOM_output_text "    Q253=[format "%0.0f" $mom_cycle_feed_rate]  ;F PRE-POSITIONING ~"
MOM_output_text "    Q351=$mom_Q351_direction  ;CLIMB OR UP-CUT ~"
MOM_output_text "    Q200=[format "%0.3f" $mom_cycle_rapid_to]  ;SET-UP CLEARANCE ~"
MOM_output_text "    Q358=0  ; ~"
MOM_output_text "    Q359=0  ; ~"
MOM_output_text "    Q203=$user_cycle_z_coord  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=[format "%0.3f" $mom_cycle_retract_to]  ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q254=0  ; ~"
MOM_output_text "    Q207=[format "%0.0f" $mom_cycle_feed_rate]  ;FEED RATE FOR MILLNG"
}
set user_cycle_saved_param_list $user_cycle_param_list
#==================================
global mom_toolpath_cutting_time PI
...
if { [info exist mom_cycle_tool_axis_change] && $mom_cycle_tool_axis_change == 1 } {
     ...
     if { ![EQ_is_equal $rotary_saved(0) $mom_out_angle_pos(0)] || \
          ![EQ_is_equal $rotary_saved(1) $mom_out_angle_pos(1)] } {

               PLANE_RESET "STAY"
               MOM_output_literal "L Z-20.0 FMAX M91"
               MOM_output_literal "L X0.0 Y0.0 FMAX M91"
               MOM_output_literal "L B[format "%0.3f" $mom_out_angle_pos(0)] C[format "%0.3f" $mom_out_angle_pos(1)] FMAX"

            if { [EQ_is_ge $mom_out_angle_pos(0) -90.0] } {
...
if { [EQ_is_equal $mom_feed_approach_value 0] } {
   ...
   set f_max $mom_feed_approach_value
}
MOM_set_address_format f_max $out_format

set cycle_retract_plane [format "%0.1f" $mom_cycle_retract_to_pos(2)]
}


#=============================================================
proc PB_CMD_cycle_thread_milling { } {
#=============================================================
global user_cycle_saved_param_list
...
#=============================================================
proc PB_CMD_cycle_threadmilling_data { } {
#=============================================================
global mom_thr_diameter
global mom_thr_dir
global mom_thr_cycle_typh
global mom_thr_pitch
global mom_thr_mach_dir
global mom_thr_npitch
...
} elseif { $mom_cycle_event_type == "cycle90" } {
# Siemens threadmill data

    set mom_Q351_direction 1

if { $mom_siemens_cycle_typh == "0" } {
...
} elseif { $mom_cycle_event_type == "general_tm" } {
# General threadmill data
...
#=============================================================
proc PB_CMD_cycle_user_boring { } {
#=============================================================
uplevel #0 {
set user_boring_depart_side(NO) 0
set user_boring_depart_side(X-) 1
set user_boring_depart_side(Y-) 2
set user_boring_depart_side(X+) 3
set user_boring_depart_side(Y+) 4
} ; #uplevel
...
global mom_cycle_rapid_to
global mom_cycle_rapid_to_pos
set user_cycle_clearance [format "%0.3f" $mom_cycle_rapid_to]
set user_cycle_param_list $user_cycle_clearance
#===============================

global mom_cycle_delay
if { ![info exist mom_cycle_delay] } {
   set mom_cycle_delay 0.2 }
set user_cycle_delay [format "%0.2f" $mom_cycle_delay]
lappend user_cycle_param_list $user_cycle_delay
#===============================

global mom_cycle_feed_to
set user_cycle_depth [format "%0.3f" $mom_cycle_feed_to]
lappend user_cycle_param_list $user_cycle_depth
#===============================

global mom_cycle_step1
if { [info exist mom_cycle_step1] && $mom_cycle_step1 != 0.0 } {
    set user_cycle_step  $mom_cycle_step1
} else {
        set user_cycle_step [expr ($mom_cycle_feed_to*(-1.0))]
}
set user_cycle_step [format "%0.3f" $user_cycle_step]
lappend user_cycle_param_list $user_cycle_step
#===============================

global mom_cycle_retract_to
set user_cycle_retract [format "%0.3f" $mom_cycle_retract_to]
lappend user_cycle_param_list $user_cycle_retract
#===============================

global mom_cycle_feed_rate
set user_cycle_feed [format "%0.0f" $mom_cycle_feed_rate]
lappend user_cycle_param_list $user_cycle_feed
#===============================

global mom_cycle_feed_rate_back
if { ![info exist mom_cycle_feed_rate_back] } {
     set user_cycle_feed_retract $user_cycle_feed
} else {
     set user_cycle_feed_retract [format "%0.0f" $mom_cycle_feed_rate_back]
}
lappend user_cycle_param_list $user_cycle_feed_retract
#===============================

global user_boring_depart_side
global mom_boring_retract_side
global mom_cycle_orient

if { ![info exist mom_boring_retract_side] } {
     set mom_boring_retract_side NO
}
lappend user_cycle_param_list $user_boring_depart_side($mom_boring_retract_side)

if { [info exist mom_cycle_orient] } {
     set mom_cycle_orient [format "%0.1f" $mom_cycle_orient]
} else {
     set mom_cycle_orient 0.0
}
lappend user_cycle_param_list $mom_cycle_orient
...
set user_cycle_surf [format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]


MOM_output_literal "CYCL DEF 202 BORING ~"
MOM_output_text "    Q200=$user_cycle_clearance  ;SET-UP CLEARANCE ~"
MOM_output_text "    Q201=$user_cycle_depth  ;DEPTH ~"
MOM_output_text "    Q206=$user_cycle_feed  ;FEED RATE FOR PLNGNG ~"
MOM_output_text "    Q211=$user_cycle_delay  ;DWELL TIME AT DEPTH ~"
MOM_output_text "    Q208=$user_cycle_feed_retract  ;RETRACTION FEED RATE ~"
MOM_output_text "    Q203=[format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=$user_cycle_retract  ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q214=$user_boring_depart_side($mom_boring_retract_side)  ;DISENGAGING DIRECTN ~"
MOM_output_text "    Q336=$mom_cycle_orient  ;ANGLE OF SPINDLE"
}
set user_cycle_saved_param_list $user_cycle_param_list
#==================================
}
...
#=============================================================
proc PB_CMD_cycle_user_boring_back { } {
#=============================================================
uplevel #0 {
set user_boring_depart_side(NO) 0
set user_boring_depart_side(X-) 1
...
MOM_output_text "BORING BACK 204"
...
#=============================================================
proc PB_CMD_cycle_user_defined { } {
#=============================================================
uplevel #0 {
#==========================================
proc MOM_drill_text_move {} {
...
if { $mom_cycle_text == "G208" || $mom_cycle_text == "HELIMILL" } {
      MOM_heli_mill_move
 } elseif { $mom_cycle_text == "G262" || $mom_cycle_text == "THREADMILL"} {
      MOM_thread_mill_move
 }
}
#=========================================
} ; #uplevel
...
#=============================================================
proc PB_CMD_cycle_user_drill_break_chip { } {
#=============================================================
global user_cycle_saved_param_list
...
global mom_cycle_incr_q202
global mom_cycle_incr_q212
global mom_cycle_incr_q213
global mom_cycle_incr_q205
global mom_cycle_step1

if { ![info exist mom_cycle_incr_q202] || $mom_cycle_incr_q202 <= 0.0 } {
     if { [info exist mom_cycle_step1] } {
    set mom_cycle_incr_q202 $mom_cycle_step1
       } else {
    set mom_cycle_incr_q202 [expr ($mom_cycle_feed_to*(-1.0))]
       }
}
set mom_cycle_incr_q202 [format "%0.3f" $mom_cycle_incr_q202]

if { ![info exist mom_cycle_incr_q212] || $mom_cycle_incr_q212 <= 0.0 } {
    set mom_cycle_incr_q212 0.2
}
set mom_cycle_incr_q212 [format "%0.3f" $mom_cycle_incr_q212]

if { ![info exist mom_cycle_incr_q213] } {
    set mom_cycle_incr_q213 1
}

if { ![info exist mom_cycle_incr_q205] || $mom_cycle_incr_q205 <= 0.0 } {
    set mom_cycle_incr_q205 [expr abs($mom_cycle_incr_q202 / 2)]
}
set mom_cycle_incr_q205 [format "%0.3f" $mom_cycle_incr_q205]

lappend user_cycle_param_list $mom_cycle_incr_q202
lappend user_cycle_param_list $mom_cycle_incr_q212
lappend user_cycle_param_list $mom_cycle_incr_q213
lappend user_cycle_param_list $mom_cycle_incr_q205
#===============================
...
set mom_cycle_feed_rate [format "%0.0f" $mom_cycle_feed_rate]
if { ![info exist mom_cycle_feed_rate_back] } {
      set mom_cycle_feed_rate_back $mom_cycle_feed_rate
}
lappend user_cycle_param_list $mom_cycle_feed_rate
lappend user_cycle_param_list $mom_cycle_feed_rate_back
...
MOM_output_text "    Q200=$user_cycle_clearance  ;SET-UP CLEARANCE ~"
MOM_output_text "    Q201=$user_cycle_depth ;DEPTH ~"
MOM_output_text "    Q206=$mom_cycle_feed_rate ;FEED RATE FOR PLNGNG ~"
MOM_output_text "    Q202=$mom_cycle_incr_q202  ;PLUNGING DEPTH ~"
MOM_output_text "    Q210=0  ;DWELL TIME AT TOP ~"
MOM_output_text "    Q203=[format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=$user_cycle_retract  ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q212=$mom_cycle_incr_q212  ;DECREMENT ~"
MOM_output_text "    Q213=$mom_cycle_incr_q213  ;NR OF BREAKS ~"
MOM_output_text "    Q205=$mom_cycle_incr_q205  ;MIN. PLUNGING DEPTH ~"
MOM_output_text "    Q211=$user_cycle_delay  ;DWELL TIME AT DEPTH ~"
MOM_output_text "    Q208=$mom_cycle_feed_rate_back  ;RETRACTION FEED RATE ~"
MOM_output_text "    Q256=0.2  ;DIST FOR CHIP BRKNG"
}
set user_cycle_saved_param_list $user_cycle_param_list
#=============================
}
...
#=============================================================
proc PB_CMD_cycle_user_drill_dwell { } {
#=============================================================
global user_cycle_saved_param_list
...
MOM_output_text "    Q201=$user_cycle_depth  ;DEPTH ~"
MOM_output_text "    Q206=$user_cycle_feed  ;FEED RATE FOR PLNGNG ~"
MOM_output_text "    Q202=$user_cycle_step  ;PLUNGING DEPTH ~"
MOM_output_text "    Q210=0  ;DWELL TIME AT TOP ~"
MOM_output_text "    Q203=[format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=$user_cycle_retract  ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q211=$user_cycle_delay  ;DWELL TIME AT DEPTH"
...
#=============================================================
proc PB_CMD_cycle_user_reaming { } {
#=============================================================
global user_cycle_saved_param_list
...
#=============================================================
proc PB_CMD_cycle_user_stop_tapping { } {
#=============================================================
global mom_stop_before_hole

if { [info exist mom_stop_before_hole] } {

  if { $mom_stop_before_hole == "ONE" || $mom_stop_before_hole == "BEGIN" } {
       unset mom_stop_before_hole
       MOM_output_literal "STOP;   !!! PRODUT` I SMAZAT` INSTRUMENT !!!"
     } elseif { $mom_stop_before_hole != "ALL" } {
       unset mom_stop_before_hole
     } else {
       MOM_output_literal "STOP;   !!! PRODUT` I SMAZAT` INSTRUMENT !!!"
      }
...
#=============================================================
proc PB_CMD_cycle_user_tapping { } {
#=============================================================
global user_cycle_saved_param_list
...
if { [info exist mom_tool_pitch] } {
     set user_cycle_pitch [format "%0.3f" $mom_tool_pitch]
} else {
     set user_cycle_pitch [format "%0.3f" $mom_cycle_feed_rate_per_rev]
}
global mom_spindle_direction
if { $mom_spindle_direction == "CCLW" } { set user_cycle_pitch [expr -abs($user_cycle_pitch)] }
lappend user_cycle_param_list $user_cycle_pitch
#===============================

global mom_cycle_feed_back_kf
if { ![info exist mom_cycle_feed_back_kf] || $mom_cycle_feed_back_kf <= 0.0 } {
     set mom_cycle_feed_back_kf 1
}
set mom_cycle_feed_back_kf [format "%0.1f" $mom_cycle_feed_back_kf]
# lappend user_cycle_param_list $mom_cycle_feed_back_kf
#===============================

global mom_cycle_taps_number
if { [info exist mom_cycle_taps_number] && $mom_cycle_taps_number > 0 } {
      set user_cycle_tap_step [expr abs( $user_cycle_depth / $mom_cycle_taps_number) ]
      set user_cycle_tap_step [format "%0.1f" $user_cycle_tap_step]
#      lappend user_cycle_param_list $user_cycle_tap_step
}
#===============================

global mom_tap_floating_holder
if { ![info exist mom_tap_floating_holder] } {
      set mom_tap_floating_holder FALSE
}
# lappend user_cycle_param_list $mom_tap_floating_holder
...
if { ![info exist user_cycle_saved_param_list] ||
  ...
  if { $mom_tap_floating_holder == "TRUE" } {
MOM_output_literal "CYCL DEF 206 ~"
MOM_output_text "    Q200=$user_cycle_clearance  ;SET-UP CLEARANCE ~"
MOM_output_text "    Q201=$user_cycle_depth  ;DEPTH OF THREAD ~"
MOM_output_text "    Q206=$mom_cycle_feed_rate  ; ~"
MOM_output_text "    Q211=0.5  ; ~"
MOM_output_text "    Q203=[format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=$user_cycle_retract  ;2ND SET-UP CLEARANCE"
  } elseif { [info exist mom_cycle_taps_number] && $mom_cycle_taps_number != 1 } {
MOM_output_literal "CYCL DEF 209  ~"
MOM_output_text "    Q200=$user_cycle_clearance  ;SET-UP CLEARANCE ~"
MOM_output_text "    Q201=$user_cycle_depth  ;DEPTH OF THREAD ~"
MOM_output_text "    Q239=$user_cycle_pitch  ;THREAD PITCH ~"
MOM_output_text "    Q203=[format "%0.3f" [expr (-1 * $mom_cycle_rapid_to)]]  ;SURFACE COORDINATE ~"
MOM_output_text "    Q204=$user_cycle_retract  ;2ND SET-UP CLEARANCE ~"
MOM_output_text "    Q257=$user_cycle_tap_step ; ~"
MOM_output_text "    Q256=0 ; ~"
MOM_output_text "    Q336=0 ; ~"
MOM_output_text "    Q403=$mom_cycle_feed_back_kf ; "
  } else {
...
#HOME_POSITION "B"
#MOM_output_literal "L C0.0 FMAX M94"

MOM_output_literal "PLANE RESET TURN FMAX"
...
#=============================================================
MOM_force once M_spindle air_coolant
}
...
set circle_center_block "CC "

   switch $mom_pos_arc_plane {
      XY {
        append circle_center_block " X[format "%0.3f" $mom_pos_arc_center(0)]"
        append circle_center_block " Y[format "%0.3f" $mom_pos_arc_center(1)]"
        set pitch_axes " IZ"
    }
      YZ {
        append circle_center_block " Y[format "%0.3f" $mom_pos_arc_center(1)]"
        append circle_center_block " Z[format "%0.3f" $mom_pos_arc_center(2)]"
        set pitch_axes " IX"
    }
      ZX {
        append circle_center_block " Z[format "%0.3f" $mom_pos_arc_center(2)]"
...
MOM_output_literal $circle_center_block

   for { set h_a $mom_arc_angle } {$h_a > 0.0} {set h_a [expr $h_a - 5400.0]} {
       if { $h_a < 5400.0 } {
         set helix_angle [expr $h_a * $helix_mod]
         set helix_pitch [expr $mom_helix_pitch / 360.0  * $h_a]
           } else {
         set helix_angle [expr 5400.0 * $helix_mod]
         set helix_pitch [expr $mom_helix_pitch / 360.0  * 5400.0]
                   }

set helix_block "CP"
append helix_block " IPA[format "%0.3f" $helix_angle]"
append helix_block " $pitch_axes[format "%0.3f" $helix_pitch]"
append helix_block " $helix_direction"
append helix_block " F[format "%0.0f" $feed]"
MOM_output_literal $helix_block
 }
MOM_force once F
...
#=============================================================
proc PB_CMD_initial_tool_vector { } {
#=============================================================
global mom_out_angle_pos

if { ![EQ_is_equal $mom_out_angle_pos(0) 0] || \
     ![EQ_is_equal $mom_out_angle_pos(1) 0] } {
         MOM_output_literal "L Z-20.0 FMAX M91"
         MOM_output_literal "L X0.0 Y0.0 FMAX M91"
         MOM_output_literal "L B[format "%0.3f" $mom_out_angle_pos(0)] C[format "%0.3f" $mom_out_angle_pos(1)] FMAX"
         MOM_output_literal "M140 MB MAX"
} else {
         MOM_output_literal "L Z-1.0 FMAX M91"
}
...
global mom_operation_name
MOM_output_text "; ========================="
MOM_output_text "; $mom_operation_name"
MOM_output_text "; ========================="
...
if { $mom_oper_tool == "NONE" || [string match "*CONTROL*" [string toupper $mom_operation_type]] } { MOM_abort_event }

if { ![info exist user_saved_spindle_speed] } { set user_saved_spindle_speed 0 }

if { $mom_spindle_speed > 0.0 && $mom_spindle_speed != $user_saved_spindle_speed } {

    MOM_output_text ";=========="
    MOM_output_literal "* - RPM CHANGE"
    MOM_output_literal "TOOL CALL Z [format "S%0.0f" $mom_spindle_speed]"
    MOM_output_text ";=========="
    set user_saved_spindle_speed [format "%0.0f" $mom_spindle_speed]
}
}
...
global mom_main_mcs saved_main_mcs
...
#kin_user_mcs_type = MAIN ROT HELP SHIFT
...
# 3 - rotation
#-----------------------------------------
if { $mom_main_mcs == "GEOMETRY" || $mom_coordinate_system_purpose == "1" } {
     set kin_user_mcs_type MAIN
} elseif { $mom_special_output == "1" } {
     set kin_user_mcs_type HELP
} elseif { $mom_special_output == "2" } {
     set kin_user_mcs_type SHIFT
} elseif { $mom_special_output == "3" } {
     set kin_user_mcs_type ROT
     RESET_KINEMATICS "TABLE"
}
#----------------------------------------
if { [info exist saved_main_mcs] } {
     SET_TCPM "OFF"
     PLANE_RESET "STAY"
     SET_SHIFT "OFF"
}
...
MOM_output_text "; ========================="
set saved_main_mcs $mom_main_mcs
...
if { $kin_user_mcs_type == "SHIFT" && ![info exist saved_csys_origin] } {
     SET_SHIFT "ON"
}

#------------------------------------------------------------------------

if { [string match "*VECTOR*" $user_tool_axis_type] } {
...
if { $user_tool_axis_type == "VARIABLE" } {
    MOM_enable_address fourth_axis fifth_axis
    SET_TCPM "ON"
}
...
#=============================================================
proc PB_CMD_set_tool_vector { } {
#=============================================================
global mom_out_angle_pos
global rotary_saved ; # (0) (1)
...
#=============================================================
MOM_force once G_motion X Y Z fourth_axis fifth_axis F
...
global mom_saved_sequence_number
  ...
  MOM_output_text "END PGM [string toupper $mom_sub_file_base_name] MM"
  MOM_close_output_file $mom_sub_file_name
  MOM_open_output_file $ptp_file_name
  catch [unset mom_sub_file_name]
  catch [unset mom_sub_file_base_name]
...
if { [info exist mom_subprogram_start] && $mom_subprogram_start == "TRUE" } {
  ...
  set mom_sub_file_name "${mom_output_file_directory}${mom_output_file_basename}_sub${mom_sub_counter}.H"
  set mom_sub_file_base_name "${mom_output_file_basename}_sub${mom_sub_counter}"

  if { [file exists $mom_sub_file_name] } { MOM_remove_file $mom_sub_file_name }

  MOM_output_literal "CALL PGM ${mom_sub_file_base_name}.H"
  ...
  MOM_output_text "0 BEGIN PGM [string toupper $mom_sub_file_base_name] MM"
  MOM_output_text ";=========="
  MOM_output_text "; SUBPROGRAMM $mom_sub_counter"
  MOM_output_text "; DMU60 MONOBLOCK TNC530"
  MOM_output_text "; DATE [string toupper $mom_date] :  USER - [string toupper $mom_logname]"
...
return
   ...
   if { [string match "UNDEFINED" $mom_cutcom_status] || [string match $mom_user_prev_cutcom_status $mom_cutcom_status] } {
...
#=============================================================
proc PB_CMD_tapping_cycle_call { } {
#=============================================================
return

global mom_stop_before_hole
if { [info exist mom_stop_before_hole] && $mom_stop_before_hole != "NONE" } {
    MOM_output_literal "STOP ; PRODUT I SMAZAT"
   }
}


#=============================================================
proc PB_CMD_tnc_blk_form { } {
#=============================================================
global mom_blk_x_min mom_blk_x_max
global mom_blk_y_min mom_blk_y_max
global mom_blk_z_min mom_blk_z_max

if { ![info exist mom_blk_x_min] } {
MOM_output_literal "BLK FORM 0.1 Z X-100 Y-100 Z-10"
MOM_output_literal "BLK FORM 0.2 X100 Y100 Z0"
} elseif { $mom_blk_x_min != "DONE" } {
...
# "Sequential Mill Main Operation"
# "Zig-Zag Surface"
# "Mill User-Defined"
# "Hole Making"
# "Point to Point"
...
# UF_PARAM_tlaxis_type_normal_to_first  = 3
...
# mom_tool_axis_type > 3 variable
# mom_tool_axis_type <= 3 vector, normal, +ZM
...
if { $mom_operation_type == "Hole Making" || \
       $mom_operation_type == "Point to Point" || \
          [string match "*Drill*" $mom_operation_type] } {
   #3plus2 mode
   set user_tool_axis_type VECTOR_PTP
   return
}

if { ![info exist mom_tool_axis_type] } {
   if { [string match "*Generic*" $mom_operation_type] ||
        [string match "*Cylinder*" $mom_operation_type] ||
        [string match "*Thread*" $mom_operation_type] } {
   #3plus2 mode
...
if { $mom_cycle_event_type == "hhain_hm" || $mom_cycle_event_type == "general_hm" || $mom_cycle_event_type == "pocket4" } {
     MOM_cycle_bore_milling_move
 } elseif { $mom_cycle_event_type == "hhain_tm" || $mom_cycle_event_type == "general_tm" || $mom_cycle_event_type == "cycle90" } {
     MOM_cycle_thread_milling_move
 } elseif { $mom_cycle_event_type == "boss_gm" } {
     MOM_boss_milling_move
 } elseif { $mom_cycle_event_type == "general_btm" || $mom_cycle_event_type == "cycle90_bth" } {
     MOM_boss_thread_milling_move
...
#=============================================
global mom_cycle_event_type
set mom_cycle_event_type "boss_gm"
}
...
#=============================================
global mom_cycle_event_type
set mom_cycle_event_type "general_btm"
}
...
global mom_cycle_text
 ...
 if { $mom_siemens_cycle_typh == 1 } {
    set mom_cycle_event_type "cycle90_bth"
 } else { set mom_cycle_event_type "cycle90" }
}
...
#=============================================================
proc PB_CMD_user_from_move { } {
#=============================================================
global mom_current_motion
FMAX_SET
if { $mom_current_motion == "first_move" } { return }
MOM_do_template rapid_move_5ax
}


#=============================================================
proc PB_CMD_user_kinematics { } {
#=============================================================
uplevel #0 {
set user_kinematics_type "HEAD"
set mom_sys_cycle_reps_code "CYCL CALL POS"

set mom_plane_transformation_status "OFF"
set user_saved_tool_vector "0"
set mom_auto_tcpm_mode "OFF"
set fun_tcpm(ON) "FUNCTION TCPM F TCP AXIS POS PATHCTRL AXIS"
set fun_tcpm(OFF) "FUNCTION RESET TCPM"

set air_coolant_command(OFF)                    "M09"
set air_coolant_command(WATER_OUT)              "M8"
set air_coolant_command(WATER_THRU)             "M7"
set air_coolant_command(AIR_OUT)                "M25"
set air_coolant_command(AIR_THRU)               "M26"
set air_coolant_command(AIR_THRU_WATER_OUT)     "M8 M26"
set air_coolant_command(AIR_OUT_WATER_THRU)     "M7 M25"
set air_coolant_command(AUTO)                   "M8"
set mom_air_coolant_mode                        "OFF"

#$air_coolant_command($mom_air_coolant_mode)
#=====================================================================
proc RESET_KINEMATICS { mode } {
#=====================================================================
global user_kinematics_type

if { ![info exist user_kinematics_type] } {
    set user_kinematics_type "NO"
}

if { $mode != $user_kinematics_type && $mode == "HEAD" } {
...
} elseif { $mode != $user_kinematics_type && $mode == "TABLE" } {
...
set user_kinematics_type $mode
}

#=========================================================================
proc C_MATRIX_MOVE {} {
#=========================================================================
global mom_out_angle_pos mom_mcs_goto
global DEG2RAD RAD2DEG

set trans_angle [expr $mom_out_angle_pos(1) * (-1)]
set trans_angle_90 [expr $trans_angle + 90.0]

set new_x [expr $mom_mcs_goto(0)*cos($trans_angle*$DEG2RAD) + $mom_mcs_goto(1)*cos($trans_angle_90*$DEG2RAD)]
set new_y [expr $mom_mcs_goto(0)*sin($trans_angle*$DEG2RAD) + $mom_mcs_goto(1)*sin($trans_angle_90*$DEG2RAD)]
MOM_output_literal "L B0.0 C[format "%0.3f" $mom_out_angle_pos(1)] FMAX"
MOM_output_literal "L X[format "%0.3f" $new_x] Y[format "%0.3f" $new_y] FMAX"
}
...
set new_x [expr $mom_mcs_goto(0)*cos($trans_angle*$DEG2RAD) + $mom_mcs_goto(1)*cos($trans_angle_90*$DEG2RAD)]
set new_y [expr $mom_mcs_goto(0)*sin($trans_angle*$DEG2RAD) + $mom_mcs_goto(1)*sin($trans_angle_90*$DEG2RAD)]
MOM_output_literal "L X[format "%0.3f" $new_x] Y[format "%0.3f" $new_y] FMAX M130"
# B & C position must be maden by PLANE TURN before
}
...
if { ![info exist mom_auto_tcpm_mode] } { set mom_auto_tcpm_mode "OFF" }

if { $mode == "ON" && $mom_auto_tcpm_mode == "OFF" } {
       MOM_output_literal "FUNCTION TCPM F TCP AXIS POS PATHCTRL AXIS"
       set mom_auto_tcpm_mode "ON"
 } elseif { $mode == "OFF" && $mom_auto_tcpm_mode == "ON" } {
...
if { $mode == "ON" && ![info exist saved_csys_origin] } {
    ...
    set saved_csys_origin $user_csys_origin
...
#=====================================
proc PLANE_SPATIAL { type } {
#=====================================
# type MOVE, TURN, STAY
set plane_transformation_type $type

global mom_csys_matrix
global RAD2DEG

#if Y non st XY plane abort
if { [format "%0.3f" $mom_csys_matrix(5)] != 0.0} { MOM_abort }
...
set m8 $mom_csys_matrix(8)
  ...
  set B [expr -atan2($m2,$m8)*$RAD2DEG]
  set C [expr -atan2($m3,$m4)*$RAD2DEG]
  set B [format "%.3f" $B]
  set C [format "%.3f" $C]

global saved_csys_origin
set user_csys_origin [format "X%.3f Y%.3f Z%.3f" $mom_csys_matrix(9) $mom_csys_matrix(10) $mom_csys_matrix(11)]
if { ![info exist saved_csys_origin] || $saved_csys_origin != $user_csys_origin } {
          MOM_output_literal  "TRANS DATUM AXIS $user_csys_origin"
   }
set saved_csys_origin $user_csys_origin
...
MOM_output_literal "PLANE SPATIAL SPA0.0 SPB$B SPC$C $plane_transformation_type"
}
...
MOM_output_literal "PLANE SPATIAL SPA+0.0 [format "SPB%0.3f SPC%0.3f" $mom_out_angle_pos(0) $mom_out_angle_pos(1)] $plane_transformation_type"
}
...
if { ![info exist mom_plane_transformation_status]
     || $mom_plane_transformation_status == "OFF" } { return; }

set mom_plane_transformation_status OFF

MOM_output_literal  "PLANE RESET $plane_transformation_type"

if { [string match "*TURN*" $type ] } {
# MOM_output_literal  "L C0.0 FMAX M94"
 }

global saved_csys_origin
if { [info exist saved_csys_origin] } {
    unset saved_csys_origin
    MOM_output_literal  "TRANS DATUM RESET"
   }
}
...
#=====================================
set rotary_saved() 2
set rotary_saved(0) 0
set rotary_saved(1) 0
#=====================================
...
#=====================================
global mom_out_angle_pos
global mom_saved_rotary_4th mom_saved_rotary_5th

  if {![info exist mom_saved_rotary_4th]} { set mom_saved_rotary_4th 0 }
  if {![info exist mom_saved_rotary_5th]} { set mom_saved_rotary_5th 0 }

  set ang_4th [format "%0.5f" $mom_out_angle_pos(0)]
  set ang_5th [format "%0.5f" $mom_out_angle_pos(1)]
  set answer 0

if { $mom_saved_rotary_4th != $ang_4th } {
      set answer 1
   }
if { $mom_saved_rotary_5th != $ang_5th } {
      set answer [expr $answer + 10]
   }

#  set mom_saved_rotary_4th $ang_4th
#  set mom_saved_rotary_5th $ang_5th
...
global rotary_saved ; # (0) (1)
global mom_out_angle_pos
  set rotary_saved(0) [format "%0.5f" $mom_out_angle_pos(0)]
  set rotary_saved(1) [format "%0.5f" $mom_out_angle_pos(1)]
}

#=====================================
proc HOME_POSITION { axis } {
#=====================================
global mom_sys_home_pos

set home_pos_string "L"
if { [string match "*X*" $axis] } { append home_pos_string " X[format "%0.3f" $mom_sys_home_pos(0)]" }
if { [string match "*Y*" $axis] } { append home_pos_string " Y[format "%0.3f" $mom_sys_home_pos(1)]" }
if { [string match "*Z*" $axis] } { append home_pos_string " Z[format "%0.3f" $mom_sys_home_pos(2)]" }
append home_pos_string " FMAX M91; HOME POSITION"
if { [string match "*B*" $axis] } { set home_pos_string "L B0.0 FMAX" }
MOM_output_literal $home_pos_string
}
...
global mom_feed_rate_mode
   ...
   set out_format "Feed_"
   append out_format $mom_feed_rate_mode
   MOM_set_address_format F $out_format
}
...
#=========================================================================
proc CSYS_MATRIX_MOVE {} {
#=========================================================================
#global mom_out_angle_pos mom_mcs_goto mom_pos
global mom_pos
...
set m8 $mom_csys_matrix(8)
set m9 $mom_csys_matrix(9)
set m10 $mom_csys_matrix(10)
set m11 $mom_csys_matrix(11)

set m_definitor [expr $m0*$m4*$m8+$m3*$m7*$m2+$m6*$m1*$m5-$m2*$m4*$m6-$m5*$m7*$m0-$m8*$m1*$m3]

set m_minor(0) [expr $m4*$m8-$m5*$m7]
set m_minor(1) [expr $m3*$m8-$m5*$m6]
set m_minor(2) [expr $m3*$m7-$m4*$m6]
set m_minor(3) [expr $m1*$m8-$m2*$m7]
set m_minor(4) [expr $m0*$m8-$m2*$m6]
set m_minor(5) [expr $m0*$m7-$m1*$m6]
set m_minor(6) [expr $m1*$m5-$m2*$m4]
set m_minor(7) [expr $m0*$m5-$m2*$m3]
set m_minor(8) [expr $m0*$m4-$m1*$m3]

set m_dop(0) $m_minor(0)
set m_dop(1) [expr $m_minor(1)*(-1)]
set m_dop(2) $m_minor(2)
set m_dop(3) [expr $m_minor(3)*(-1)]
set m_dop(4) $m_minor(4)
set m_dop(5) [expr $m_minor(5)*(-1)]
set m_dop(6) $m_minor(6)
set m_dop(7) [expr $m_minor(7)*(-1)]
set m_dop(8) $m_minor(8)

set m_transp(0) $m_dop(0)
set m_transp(1) $m_dop(3)
set m_transp(2) $m_dop(6)
set m_transp(3) $m_dop(1)
set m_transp(4) $m_dop(4)
set m_transp(5) $m_dop(7)
set m_transp(6) $m_dop(2)
set m_transp(7) $m_dop(5)
set m_transp(8) $m_dop(8)

# back_csys obratnaya matritsa dlya mom_csys_matrix
set r_definitor [expr 1/$m_definitor]

set back_csys(0) [expr $r_definitor * $m_transp(0)]
set back_csys(1) [expr $r_definitor * $m_transp(1)]
set back_csys(2) [expr $r_definitor * $m_transp(2)]
set back_csys(3) [expr $r_definitor * $m_transp(3)]
set back_csys(4) [expr $r_definitor * $m_transp(4)]
set back_csys(5) [expr $r_definitor * $m_transp(5)]
set back_csys(6) [expr $r_definitor * $m_transp(6)]
set back_csys(7) [expr $r_definitor * $m_transp(7)]
set back_csys(8) [expr $r_definitor * $m_transp(8)]

set trans_point(0) [expr $back_csys(0)*$mom_pos(0) + $back_csys(1)*$mom_pos(1) + $back_csys(2)*$mom_pos(2) + $m9]
set trans_point(1) [expr $back_csys(3)*$mom_pos(0) + $back_csys(4)*$mom_pos(1) + $back_csys(5)*$mom_pos(2) + $m10]
set trans_point(2) [expr $back_csys(6)*$mom_pos(0) + $back_csys(7)*$mom_pos(1) + $back_csys(8)*$mom_pos(2) + $m11]

set trans_angle [expr atan2($m3,$m4)*$RAD2DEG]
set trans_angle_90 [expr $trans_angle + 90.0]

set new_x [expr $trans_point(0)*cos($trans_angle*$DEG2RAD) + $trans_point(1)*cos($trans_angle_90*$DEG2RAD)]
set new_y [expr $trans_point(0)*sin($trans_angle*$DEG2RAD) + $trans_point(1)*sin($trans_angle_90*$DEG2RAD)]
set move_string "L X[format "%0.3f" $new_x] Y[format "%0.3f" $new_y] FMAX"
MOM_output_literal $move_string
}

#=========================================================================
proc MOM_flush_mag_door {} {
#=========================================================================
PB_CMD_cheap_flush
}
...
#=============================================================
proc PB_CMD_user_linear_move { } {
#=============================================================
global mom_auto_tcpm_mode
global mom_cutter_data_output_indicator mom_contact_status
global mom_cycle_tool_axis_change
global rotary_saved
global mom_out_angle_pos
global mom_operation_type
...
if { [info exist mom_cutter_data_output_indicator] && $mom_cutter_data_output_indicator == 2 } {
      if { [info exist mom_contact_status] && $mom_contact_status == "ON" } {
         MOM_force once G_motion X Y Z normal_x normal_y normal_z tcpm_x tcpm_y tcpm_z
         MOM_do_template linear_move_tcpm_contact
   } else {
   ...
   if {[info exist mom_cycle_tool_axis_change] && $mom_cycle_tool_axis_change == 1 } { MOM_abort_event }
   ...
   if { ![EQ_is_equal $rotary_saved(0) $mom_out_angle_pos(0)] || \
        ...
        PLANE_ANGLES "TURN FMAX"
        MOM_force once X Y Z G_motion
    }

#  ROTARY_SAVE
  MOM_force once G_motion
  MOM_do_template linear_move_3ax
}
...
#=============================================================
proc PB_CMD_user_rapid_move { } {
#=============================================================
global mom_out_angle_pos
global rotary_saved ; # (0) (1)
global mom_auto_tcpm_mode
global mom_motion_distance
global user_abort_event
global mom_tool_axis
global mom_operation_type
global mom_cycle_tool_axis_change
...
if { $mom_motion_distance <= 0.002 } { MOM_abort_event }
...
if { $mom_auto_tcpm_mode == "ON" } {
      ...
      MOM_force once G_motion X Y Z tcpm_x tcpm_y tcpm_z
      MOM_do_template rapid_move_tcpm
      ROTARY_SAVE
      MOM_abort_event

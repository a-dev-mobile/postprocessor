########################## TCL Event Handlers ##########################
#
#  mazak_mill_qt_series.tcl -
#
#  Created by danilov  @  28 èþëÿ 2015 ã. 14:40:14 RTZ 2 (зима)
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "mazak_mill_qt_series__IN.pui"
...
#=========================
 ...
 set mom_sys_postname(TURN)                    "mazak_turn_qt_series"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_feed G_motion G_mode X Y Z fifth_axis M_coolant}
   set rapid_traverse_xy_blk {G_feed G_motion G_mode X Y fifth_axis M_coolant}
   set rapid_traverse_yz_blk {G_feed G_motion G_mode Y Z fifth_axis M_coolant}
   set rapid_traverse_xz_blk {G_feed G_motion G_mode X Z fifth_axis M_coolant}
...
#=============================================================
proc PB_CMD_drill_cycle_set_abs { } {
#=============================================================
...
if { $mom_fanuc_mill_mode == "XZ" } {
     ...
     set mom_cycle_r [expr ($mom_user_cycle_clearance - $mom_cycle_rapid_to_pos(0)) * 2.0]
     set mom_cycle_deep [expr ($mom_cycle_feed_to - $mom_cycle_rapid_to) * 2.0]
...
if { ![info exist delay_first_move] } {
    UI_UNSET mom_mill_mode_status
    PB_CMD_mill_mode_on
    PB_CMD_rpm_change
} else { UI_UNSET delay_first_move }

if { $mom_motion_event == "from_move" } {
    MOM_from_move
    set delay_first_move YES
    MOM_abort_event
}
}
...
proc MOM_helix_move { } {
#=============================================================
#   global mom_pos_arc_plane mom_pos mom_prev_pos
#   global mom_helix_pitch mom_arc_angle helix_step
#   global helix_angle helix_direction helix_mod helix_pitch
#   global mom_arc_direction

#set helix_step [expr abs(($mom_prev_pos(2) - $mom_pos(2))/$mom_helix_pitch)]
...
if { $mom_operation_type == "Hole Making" || $mom_operation_type == "Point to Point" } {
...
if { [info exist mom_mill_mode_status] && $mom_mill_mode_status == "ON" } {
  MOM_output_literal "M202"
  catch [unset mom_mill_mode_status]
}
}
...
global mom_tool_axis
  if { [format "%0.7f" $mom_tool_axis(2)] == 1.0 } {
       return 1
   } else { return 0 }
}
...
if { $mom_machine_with_y != "YES" && ![ZM_AXIS] } {
...
if { $user_tool_axis_type == "VECTOR" && [ZM_AXIS] } {
  ...
  if { $mom_machine_with_y == "YES" } { MOM_output_literal "G00 G90 Y0.0" }
  MOM_output_literal "G17 X[format "%0.3f" [PB_CMD_radius]] C0.0"
  MOM_output_literal "G12.1"
  MOM_force once G_motion F G_mode
  ...
  MILL_MODE_SET "XZ"
  MOM_force once G_mode G_motion G_feed X Y Z fifth_axis F

...
#  separator.tcl -
#
#  Created by danilov  @  24 ìàðòà 2017 ã. 10:48:36 RTZ 2 (зима)
...
if { $mom_cycle_event_type == "mt_boremilling" ||
     ...
     if {[info exists mom_attr_OPER_type_ ] && $mom_attr_OPER_type_ == "boremilling" && \
          ![EQ_is_equal $mom_tool_axis(2) 0] } {

        for {set i 0} {$i < $HOLE_number} {incr i} {
             MOM_output_text "( BOREMILLING : D = [format "%0.3f" $HOLE_diameter] )"
                 ...
                 set mom_hm_diameter $HOLE_diameter
...
global cutoff_offset
  ...
  if { ![info exists mom_var511] } { set mom_var511 0 }
  ...
  if { $mom_var511 !=0 && [info exists cutoff_offset] } {
           MOM_output_literal "G52.5"
           MOM_output_literal "G53.5 Z\[\#9101+\#511[format "%+0.3f" $cutoff_offset]\]"
  } elseif { $mom_var511 != 0 } {
           MOM_output_literal "G52.5"
           MOM_output_literal "G53.5 Z\[\#9101+\#511\]"
  } elseif { [info exists cutoff_offset] } {
           MOM_output_literal "G52.5"

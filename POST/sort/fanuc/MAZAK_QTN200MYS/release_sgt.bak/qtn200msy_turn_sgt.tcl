########################## TCL Event Handlers ##########################
#
#  qtn200msy_turn_sgt.tcl -
#
#  Created by danilov  @  14 èþíÿ 2017 ã. 14:03:13 RTZ 2 (зима)
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "qtn200msy_turn_sgt__IN.pui"
...
#=============================================================
   ...
   MOM_force Once G_motion X R W_cycle_deep F Text
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_multistart_thread_reposiition
   PB_CMD_set_clearance
...
#=============================================================
   ...
   MOM_force Once Text X R W_cycle_deep cycle_step cycle_dwell F Text M_spindle
...
global SYNCHRO_status
  ...
  if { $mom_auxfun == "724" } {
         if {[info exists SYNCHRO_status] && $SYNCHRO_status == "ON" } {
               MOM_force once M
...
global SPINDLE_mode SUBSPINDLE_mode

if { [info exist SYNCHRO_mode] && $SYNCHRO_mode == "ON" } {
     MOM_output_literal "$SPINDLE_mode($mom_machine_mode) $SUBSPINDLE_mode($mom_machine_mode)"
} elseif { $mom_active_spindle == 2 } {
     MOM_output_literal "$SUBSPINDLE_mode($mom_machine_mode)"
} else {
     MOM_output_literal "$SPINDLE_mode($mom_machine_mode)"
}
...
global mom_current_motion
  ...
  MOM_force once G_mode G_motion X Z
...
proc PB_CMD_SKIP_to { } {
#=============================================================
global barfeeder_skip

if {![info exists barfeeder_skip]} {
     MOM_output_text "N99992 (LABEL TO SKIP)"
     set barfeeder_skip 1
}
}
...
#=============================================================
global mom_cycle_r mom_user_cycle_clearance mom_cycle_rapid_to_pos mom_cycle_feed_to_pos
...
MOM_output_literal "\#33=[format %0.3f $mom_cycle_xdelta] ( X axis CONE per side )"
...
set mom_kin_read_ahead_next_motion "YES"
MOM_reload_kinematics
...
#============================
global mom_pos
global mom_multithread_start_pos
global mom_number_of_starts

if {[info exist mom_number_of_starts] && $mom_number_of_starts > 1 } {
    if { ![info exist mom_multithread_start_pos] } {
           set mom_multithread_start_pos $mom_pos(2)
    } else { set mom_pos(2) $mom_multithread_start_pos }
  }
}
...
if { $mom_tool_ug_type < 3 } {
  if { [info exist mom_tool_direction] && $mom_tool_direction == 1 } {
      if { [info exist mom_work_spindle] && $mom_work_spindle == 2 } {
...
MOM_force once G_feed G_motion G_mode F M_coolant
}
...
global cutoff_offset
    ...
    if {[info exists mom_head_name_defined] && $mom_head_name_defined == 1 } {
          if {[info exists mom_head_name] && $mom_head_name == "DATUM_SHIFT" } {
                set cutoff_offset [format "%0.3f" $mom_parent_csys_origin(2)]
                unset mom_head_name
                unset mom_head_name_defined
          } else { UI_UNSET cutoff_offset }
    } else { UI_UNSET cutoff_offset }
...
global mom_next_spindle_direction_code

set mom_next_spindle_direction_code(CLW)       "303"
set mom_next_spindle_direction_code(CCLW)      "304"
set mom_next_spindle_direction_code(OFF)       "305"
...
set mom_next_spindle_direction_code(CLW)       "3"
set mom_next_spindle_direction_code(CCLW)      "4"
set mom_next_spindle_direction_code(OFF)       "5"
...
if { [info exist mom_subprogram_start] && $mom_subprogram_start == "TRUE" } {
  ...
  MOM_output_literal "G40 G99 G90 G21"
...
#=============================================================
proc PB_CMD_turn_initial { } {
#=============================================================
MOM_force_block once y0
MOM_do_template y0
...
if { $mom_oper_tool == "NONE" } { PB_CMD_goto_zero }
if { $mom_fixture_offset_value == 2 } {
     PB_CMD_set_sub_spindle
} else { PB_CMD_set_main_spindle }

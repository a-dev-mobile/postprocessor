...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "separator__IN.pui"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   PB_CMD_FIN_holes_return
   PB_CMD_FIN_holes_end
...
#=============================================================
proc PB_CMD_FIN_holes_approach { } {
#=============================================================
return
...
global mom_sys_ptp_output
  ...
  if { $mom_attr_OPER_type_ == "fin_holes" } {
       set RECORD_start 1
       MOM_set_seq_off
...
#=============================================================
proc PB_CMD_FIN_holes_end { } {
#=============================================================
global mom_attr_OPER_type_
global GOTO_mark

  if { $mom_attr_OPER_type_ == "fin_holes" } {
       MOM_output_literal "\#21=\#21+1"
       MOM_output_literal "IF \[\#21LT\#22\] GOTO $GOTO_mark (JUMP TO START)"
  }
...
#=============================================================
proc PB_CMD_FIN_holes_return { } {
#=============================================================
return

global RECORD_start RECORD_list
global ptp_file_name
global mom_sys_ptp_output
global INIT_pos mom_pos mom_out_angle_pos
global HOLE_number

  if {[info exist RECORD_start] && [info exist RECORD_list]} {
       MOM_open_output_file $ptp_file_name
       set mom_sys_ptp_output ON
       unset RECORD_start
       MOM_set_seq_on
       VMOV 3 INIT_pos mom_pos
    for { set i 0 } { $i < $HOLE_number } { incr i } {
          set mom_out_angle_pos(1) [expr $i * (360.0/$HOLE_number)]
        if { $i != 0 } {
              MOM_output_literal "(CAXIS POS=[format %0.3f $mom_out_angle_pos(1)])"
              MOM_force once G_motion X Y fifth_axis
              MOM_do_template rapid_traverse
           }
         foreach record_ $RECORD_list {
                MOM_output_literal "$record_"
         }
    }
   unset RECORD_list
  }
...
MOM_output_text "(--------------------------)"
MOM_oiutput_text "(MOVE SUBSPINDLE $takeover_depth MM OVER PART. SET V501 = Z2)"
...
# ============================================
proc MOM_option_takeover { } {
# ============================================
### Параметры перехвата задаются в начале программы.
### Перехват выводиться постпроцессором автоматически при первой смене номера СКС

## "MILL TURN : Перехват детали ( Упрощенный )"
global mom_option_takeover_mod_opt ; # "0. Команда не активна","1. Забрать деталь из ГШП"
## "2. Отломить и забрать надрезанную деталь","3. Ручной перенос" UI_LABEL "Режим перехвата"
global mom_option_part_length ; # "#1 : Длина готовой детали"
global mom_option_blank_outlet ; # "#2 : Вылет заготовки из ГШП"
global mom_option_takeover_length ; # "#3 : Длина зажима в патроне КШП при перехвате"
global mom_option_main_chuk_hei ; # Off "#4 : Длина патрона и кулачков ГШП"
global mom_option_sub_chuk_hei ; # Off "#6 : Длина патрона и кулачков КШП"
global mom_option_subspin_home ; # Off "#9 : Исходная позиция линейной оси КШП"
global mom_option_takeover_opt ; # "1. Подход до упора","2. Подход к позиции"
global mom_option_subspin_angle ; # Off "Угол КШП при перехвате.( СК КШП )"
global mom_option_subspin_pull_out ; # Off "Вытянуть заготовку перед отломом"
}

# ============================================
proc MOM_subspindle_came { } {
# ============================================
## EVENT subspindle_came "MAZAK QTN : Подход контршпинделя к детали"
global mom_subspin_came_mode_opt ; # "0. Заготовки нет","1. Заготовка в ГШП","2. Заготовка в КШП"
global mom_subspin_w_pos ; # Off "W=Машинная координата линейной оси КШП"
global mom_subspin_overpos ; # Off "Вылет заготовки из ГШП /  Длина зажима в патроне КШП"
global mom_subspin_c2_pos ; # Off "Угол установки Шп #2"
global mom_subspin_takeover_opt ; # "1. Подход до упора","2. Подход к позиции"
global mom_subspin_cls ; # Off "Безопасное расст. подхода"
global mom_subspin_press ; # Off "Поджим (натяг)"
global mom_subspin_subchuck_stat_opt ; # "1. Закрыть","2. Открыть" "Патрон КонтрШП после подхода"
global mom_subspin_mainchuck_stat_opt ; # "1. Закрыть","2. Открыть" "Патрон ГлавнШП после подхода"
}

# ============================================
proc MOM_subspindle_drive { } {
# ============================================
## EVENT subspindle_drive "MAZAK QTN : Смещение контршпинделя"
global mom_subspin_overpos_opt ; # "1. Смещение без детали","2. Смещение с деталью"
## UI_LABEL "Вылет заготовки из ГШП /  Длина зажима в патроне КШП"
global mom_subspin_overpos ; # Off "Вылет заготовки из ГШП /  Длина зажима в патроне КШП"
}

# ============================================
proc MOM_subspindle_back_home { } {
# ============================================
## EVENT subspindle_back_home "MAZAK QTN : Отвод контршпинделя"
global mom_subspin_gohome_mod_opt ; # "0. Отвод без детали","1. Забрать деталь"
## "2. Отломить и забрать надрезанную деталь","3. Забрать отрезанную деталь"
global mom_subspin_part_length ; # Off "Длина детали"
}
...
global HOLE_stock
  ...
  if {![info exists mom_attr_OPER_type_ ] } {
...
#          MOM_abort "User Abort"
    ...
    if { $mom_attr_OPER_type_ == "boremilling_1" } {
#           set HOLE_diameter [expr $HOLE_diameter - 0.05]
       }
    if {[catch {MOM_ask_ess_exp_value "boremilling_stock"}] == 0 } {
            set HOLE_stock [expr double([MOM_ask_ess_exp_value "boremilling_stock"])]
    } else { set HOLE_stock 0.02 }
...
global RECORD_start RECORD_list
  ...
  if {[info exist RECORD_start]} {
       lappend RECORD_list $mom_o_buffer
  }
...
#set mom_hm_diameter [expr $HOLE_diameter-$HOLE_stock]
...
global non_modal_cycle
   if { [info exists non_modal_cycle] } {
        if { $non_modal_cycle == 2 } { MOM_output_literal "G67" }
        unset non_modal_cycle
...
#=============================================================
global mom_attr_OPER_type_
  if {[info exists mom_attr_OPER_type_ ] } { return }
...
#=============================================================
proc PB_CMD_drill_cycle_set_attr { } {
#=============================================================
global mom_attr_OPER_type_
  if {![info exists mom_attr_OPER_type_ ] } { return }
...
global mom_pos mom_out_angle_pos mom_tool_axis
global CCR_ CCA_ CCD_ CCN_ CCF_
...
###==================================
     ...
     set mom_cycle_rapid_to_pos(1) $mom_pos(1)
     set mom_cycle_retract_to_pos(1) $mom_pos(1)
     set mom_cycle_feed_to_pos(1) $mom_pos(1)

  if {$mom_attr_OPER_type_ == "csink" } {
        set mom_cycle_deep [expr -($HOLE_diameter/2 + $HOLE_faze + $mom_cycle_rapid_to) * $mom_tool_axis(2)]
  } else {
         set mom_cycle_deep [expr $mom_cycle_feed_to_pos(2) - $mom_cycle_rapid_to_pos(2)]
    }
...
if { $mom_cycle_event_type == "mt_boremilling" ||
     ...
     if {[info exists mom_attr_OPER_type_ ] && [string match "*boremil*" $mom_attr_OPER_type_] && \
          ![EQ_is_equal $mom_tool_axis(2) 0] } {
...
#=============================================================
MOM_output_literal "(SEPARATOR POST)"
...
#=============================================================
proc PB_CMD_return_to_calling_post { } {
#=============================================================
global mom_sys_mill_postname mom_sys_postname
...
global PART_take_len
global mom_sys_leader
global HOLE_number INIT_pos
global GOTO_mark

  if {[info exists mom_attr_OPER_type_ ] && \
            $user_tool_axis_type == "PTP" && \
       [format "%0.7f" [expr abs($mom_tool_axis(2))]] == 1.0 } {
...
#              PB_CMD_reload_ptp_pos
  ...
  if { $user_tool_axis_type == "VECTOR" && $mom_attr_OPER_type_ == "fin_holes" } {
              MOM_force once Z
              MOM_do_template move_z
              set mom_sys_leader(Y) "Y"
              ## PB_CMD_kin_before_motion
              ...
              set AUTO_clamp_5th OFF
              VMOV 3 mom_pos INIT_pos

       if {![info exist GOTO_mark]} {
              set GOTO_mark 90002
       } else { incr GOTO_mark }

       MOM_output_literal "\#21=0"
       MOM_output_literal "\#22=[expr int($HOLE_number)] (NUMBER OF HOLES)"
       MOM_output_text "N$GOTO_mark (START)"
       MOM_output_literal "\#23=360/\#22*\#21"
       MOM_disable_address fifth_axis
       PB_CMD_kin_before_motion
       MOM_force_block once rapid_cvari
       MOM_do_template rapid_cvari
       MOM_abort_event
  }

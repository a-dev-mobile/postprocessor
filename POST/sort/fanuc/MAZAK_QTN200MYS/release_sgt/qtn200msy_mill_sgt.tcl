...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "qtn200msy_mill_sgt__IN.pui"
...
#=========================
 ...
 set mom_sys_postname(TURN)                    "qtn200msy_turn_sgt"
...
#=============================================================
   SPINDLE_SET
   PB_CMD_RPM_set
   MOM_force Once S M_spindle G_spin
...
#=============================================================
   ...
   PB_CMD_UPLEVEL_standart_subspindle
   PB_CMD_UPLEVEL_subspindle
   PB_CMD_PARAM_subspin
   PB_CMD_SGT_setup
   PB_CMD_SGT_subspin
...
#=============================================================
proc PB_CMD_BARFEEDER_list { } {
#=============================================================
uplevel #0 {

#=============================================
proc MOM_barfeeder_list { } {
#=============================================
##UI_LABEL "Список для барфидера"
global mom_barfeeder_list ; # s DEFVAL "BARFEEDER_on" TOGGLE Off "Список барфидера"
global PART_modification
global BARFEEDER_status

    if {![info exists mom_barfeeder_list] || \
         [string length $mom_barfeeder_list] < 1 } { return }

    if {![info exists PART_modification]} { return }

    if { [catch {MOM_ask_ess_exp_value $mom_barfeeder_list}] == 0 } {
            set BAR_list [format "%s" [MOM_ask_ess_exp_value $mom_barfeeder_list]]
            set BAR_list [string map { \{ "" \} "" \" "" , " " } $BAR_list]
            set BAR_list [split $BAR_list]
    } else { set BAR_list "0" }

    if {[set curr_search_ [lsearch $BAR_list $PART_modification]] < 0 } {
               set BARFEEDER_status OFF
    } else { set BARFEEDER_status ON }
}
...
#=============================================================
proc PB_CMD_HEAD_command { } {
#=============================================================
global mom_head_name

    if {[info exists mom_head_name]} {
          MOM_output_literal "mom_head_name = $mom_head_name"
       }
}
...
#
   ...
   if {[info exist GOTO_mark]} { return }
   if {[info exists mom_user_defined_text]} {
         MOM_output_text "${mom_user_defined_text}"
      }
   return
...
#=============================================================
proc PB_CMD_PARAM_subspin { } {
#=============================================================
uplevel #0 {

# ============================================
proc MOM_qtn_param_take_part { } {
# ============================================
global part_mod
if {[info exists part_mod] && ($part_mod == "shpilka" || $part_mod == "shtock" || $part_mod == "separator") } {
       SGT_take_part
       return
}

## "MAZAK QTN 200 MSY : Parameters. Контршпиндель : Захват детали."
global mom_qtn_takeover_opt ; # "0. Рассчитать через переменные #521 #504","1. Настроить #501 на станке. #504 справочная.","2. Использовать готовую #501."
global mom_qtn_var504 ; # TYPE s DEFVAL "#504" Off "Длина захвата в патрон КШП"
global mom_qtn_var501 ; # TYPE d DEFVAL "0.0" TOGGLE Off "#501 : Координата перехвата Z2. Взять со станка"
global mom_qtn_c2_pos ; # "Угол установки КШП"
global mom_qtn_clrs ; # "Безопасный зазор"
global mom_qtn_press ; # "Поджим (натяг)"
global mom_qtn_var505 ; # TYPE s DEFVAL "#505" TOGGLE Off "Вытянуть / задвинуть из / в  ГШП"
global mom_qtn_var506 ; # TYPE s DEFVAL "#506" TOGGLE Off "Переместить открытый КШП"

global mom_subspin_takeover_length_expr ; # "#504. Глубина захвата в КШП."
global mom_subspin_safe_length ; # "Безопасное расст. подхода"
global mom_subspin_press_length ; # "Поджим (натяг)"
global mom_subspin_pull_on_expr ; # "#505. Вытянуть / Задвинуть деталь."
global mom_subspin_move_on_expr ; # "#506. Переместить открытый КШП."
...
global SYNCHRO_status SYNCHRO_mode

    set take_mode [string index $mom_qtn_takeover_opt 0]
    # "0. Рассчитать через переменные #521 #504","1. Настроить #501 на станке. #504 справочная.","2. Использовать готовую #501."

       if {[info exists mom_qtn_var504] && \
           [string length $mom_qtn_var504] > 0 } {
                       set var504_expr [string map {+ "" - "" \# var } $mom_qtn_var504]
                 if {[catch {MOM_ask_ess_exp_value $var504_expr}] == 0 } {
                       set var504 [format "%0.4f" [MOM_ask_ess_exp_value $var504_expr]]
                    }
                 if { [string match "*+\#*" $mom_qtn_var504] } {
                       set takeover_length_expr [string map {+ ABS\[} $mom_qtn_var504]
                       append takeover_length_expr "\]"
                 } elseif { [string match "*-\#*" $mom_qtn_var504] } {
                       set takeover_length_expr [string map {- -ABS\[} $mom_qtn_var504]
                       append takeover_length_expr "\]"
                 } else { set takeover_length_expr $mom_qtn_var504 }
       } else { set takeover_length_expr "none" }

       if {[info exists mom_qtn_var505] && \
           [string length $mom_qtn_var505] > 0 } {
                       set var505_expr [string map {+ "" - "" \# var } $mom_qtn_var505]
                 if {[catch {MOM_ask_ess_exp_value $var505_expr}] == 0 } {
                       set var505 [format "%0.4f" [MOM_ask_ess_exp_value $var505_expr]]
                    }
                 if { [string match "*+\#*" $mom_qtn_var505] } {
                       set pull_on_expr [string map {+ ABS\[} $mom_qtn_var505]
                       append pull_on_expr "\]"
                 } elseif { [string match "*-\#*" $mom_qtn_var505] } {
                       set pull_on_expr [string map {- -ABS\[} $mom_qtn_var505]
                       append pull_on_expr "\]"
                 } else { set pull_on_expr $mom_qtn_var505 }
       } else { set pull_on_expr "none" }

       if {[info exists mom_qtn_var506] && \
           [string length $mom_qtn_var506] > 0 } {
                       set var506_expr [string map {+ "" - "" \# var } $mom_qtn_var506]
                 if {[catch {MOM_ask_ess_exp_value $var506_expr}] == 0 } {
                       set var506 [format "%0.4f" [MOM_ask_ess_exp_value $var506_expr]]
                    }
                 if { [string match "*+\#*" $mom_qtn_var506] } {
                       set move_on_expr [string map {+ ABS\[} $mom_qtn_var506]
                       append move_on_expr "\]"
                 } elseif { [string match "*-\#*" $mom_qtn_var506] } {
                       set move_on_expr [string map {- -ABS\[} $mom_qtn_var506]
                       append move_on_expr "\]"
                 } else { set move_on_expr $mom_qtn_var506 }
       } else { set move_on_expr "none" }

MOM_output_text "(PARAMETRIC TAKE PART)"
...
SYNCHRO_OFF
  ...
  if { $take_mode == 0 } {
           MOM_output_text "(--------------------------)"
       if {[info exists var504]} {
            MOM_output_text "\#504=$var504 (TAKEOVER LENGTH)"
          }

       if {[info exists part_mod] && $part_mod != "shpilka" } {
           if {[info exists var505]} {
                MOM_output_text "\#505=$var505 (MOVE PART IN MACHINE AREA)"
              }
           if {[info exists var506]} {
                MOM_output_text "\#506=$var506 (REPOS COUNTERSPINDLE)"
              }
          }
      }
...
MOM_output_text "(--------------------------)"
    ...
    if { $take_mode == 0 } {
          MOM_output_text "\#501=-634+\#9101+\#521-#504 (TAKEOVER POS)"
    } elseif { $take_mode == 2 } {
           if {![info exists mom_qtn_var501] } {
                PAUSE "Var 501 Is'nt Exists\nUpdate Var 501"
                MOM_abort "User Exit"
              }
          MOM_output_text "\#501=[format %0.4f $mom_qtn_var501] (TAKEOVER POS)"
    } else {
          MOM_output_text "(MOVE SUBSPINDLE $var504 MM OVER PART. SET V501 = Z2)"
...
MOM_output_literal "G110 C2(C = C2)"
    ...
    if {[info exists mom_qtn_c2_pos] } {
          MOM_output_literal "G0 G90 C[format %0.4f $mom_qtn_c2_pos](SUBSPINDLE ORIENT)"
          unset mom_qtn_c2_pos
    } else {
...
if { ![info exist mom_qtn_press] } { set mom_qtn_press 0.0 }
if { [info exist mom_qtn_clrs] && $mom_qtn_clrs != 0.0 } {
     MOM_output_literal "G53 G0 G90 B\[\#501+[format "%0.3f" $mom_qtn_clrs]\](W RAPID POS)"
     MOM_output_literal "G31 B\[\#501-[format "%0.3f" $mom_qtn_press]\] (W FEED POS)"
     unset mom_qtn_clrs
} else {
...
if { [info exist pull_on_expr] && $pull_on_expr != "none" } {
     MOM_output_literal "G04 G98 P1000(PAUSE)"
     MOM_output_literal "M206(SPIN 1 OPEN)"
     MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#501+\[$pull_on_expr\]\] F500.0(MOVING PART IN MACHINE AREA)"
     MOM_output_literal "M207(SPIN 1 CLOSE)"
     MOM_output_literal "M543(TRANSFER MODE OFF)"
     set mom_var511 "\#511+\[$pull_on_expr\] (SHIFT Z)"
     MOM_output_text "\#511=$mom_var511"
  }

if { [info exist move_on_expr] && $move_on_expr != "none" } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025+\[$move_on_expr\]\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
...
SYNC_mode ON
  ...
  MOM_output_text "(PARAMETRIC TAKE PART COMPLETED)"
  MOM_force once G_mode G_motion G_feed

  if {[info exists mom_qtn_var504] } { unset mom_qtn_var504 }
  if {[info exists mom_qtn_var505] } { unset mom_qtn_var505 }
  if {[info exists mom_qtn_var506] } { unset mom_qtn_var506 }
  if {[info exists mom_qtn_var501] } { unset mom_qtn_var501 }
}

# ============================================
proc MOM_qtn_param_subspindle_move { } {
# ============================================
global part_mod
if {[info exists part_mod] && ($part_mod == "shpilka" || $part_mod == "shtock" || $part_mod == "separator") } {
       SGT_subspindle_move
...
## "MAZAK QTN 200 MSY : Parameters. Контршпиндель : Переместить"
global mom_pull_free_1_expr ; # TYPE s DEFVAL "#531" TOGGLE Off "Смещение без детали."
global mom_pull_detal_1_expr ; # "#532 Смещение с деталью."
global mom_pull_free_2_expr ; # "#533 Смещение без детали."
global mom_pull_detal_2_expr ; # "#534 Смещение с деталью."
...
global SYNCHRO_status SYNCHRO_mode
       ...
       if {[info exists mom_pull_free_1_expr] && \
           [string length $mom_pull_free_1_expr] > 0 } {
                       set free_1_expr [string map {+ "" - "" \# var } $mom_pull_free_1_expr]
                 if {[catch {MOM_ask_ess_exp_value $free_1_expr}] == 0 } {
                       set var531 [format "%0.4f" [MOM_ask_ess_exp_value $free_1_expr]]
                    }
                 if { [string match "*+\#*" $mom_pull_free_1_expr] } {
                       set free_1_expr [string map {+ ABS\[} $mom_pull_free_1_expr]
                       append free_1_expr "\]"
                 } elseif { [string match "*-\#*" $mom_pull_free_1_expr] } {
                       set free_1_expr [string map {- -ABS\[} $mom_pull_free_1_expr]
                       append free_1_expr "\]"
                 }
      ...
      if {[info exists mom_pull_detal_1_expr] && \
          [string length $mom_pull_detal_1_expr] > 0 } {
                       set detal_1_expr [string map {+ "" - "" \# var } $mom_pull_detal_1_expr]
                 if {[catch {MOM_ask_ess_exp_value $detal_1_expr}] == 0 } {
                       set var532 [format "%0.4f" [MOM_ask_ess_exp_value $detal_1_expr]]
                    }
                 if { [string match "*+\#*" $mom_pull_detal_1_expr] } {
                       set detal_1_expr [string map {+ ABS\[} $mom_pull_detal_1_expr]
                       append detal_1_expr "\]"
                 } elseif { [string match "*-\#*" $mom_pull_detal_1_expr] } {
                       set detal_1_expr [string map {- -ABS\[} $mom_pull_detal_1_expr]
                       append detal_1_expr "\]"
                 }
       ...
       if {[info exists mom_pull_free_2_expr] && \
           [string length $mom_pull_free_2_expr] > 0 } {
                       set free_2_expr [string map {+ "" - "" \# var } $mom_pull_free_2_expr]
                 if {[catch {MOM_ask_ess_exp_value $free_2_expr}] == 0 } {
                       set var533 [format "%0.4f" [MOM_ask_ess_exp_value $free_2_expr]]
                    }
                 if { [string match "*+\#*" $mom_pull_free_2_expr] } {
                       set free_2_expr [string map {+ ABS\[} $mom_pull_free_2_expr]
                       append free_2_expr "\]"
                 } elseif { [string match "*-\#*" $mom_pull_free_2_expr] } {
                       set free_2_expr [string map {- -ABS\[} $mom_pull_free_2_expr]
                       append free_2_expr "\]"
                 }
      ...
      if {[info exists mom_pull_detal_2_expr] && \
          [string length $mom_pull_detal_2_expr] > 0 } {
                       set detal_2_expr [string map {+ "" - "" \# var } $mom_pull_detal_2_expr]
                 if {[catch {MOM_ask_ess_exp_value $detal_2_expr}] == 0 } {
                       set var534 [format "%0.4f" [MOM_ask_ess_exp_value $detal_2_expr]]
                    }
                 if { [string match "*+\#*" $mom_pull_detal_2_expr] } {
                       set detal_2_expr [string map {+ ABS\[} $mom_pull_detal_2_expr]
                       append detal_2_expr "\]"
                 } elseif { [string match "*-\#*" $mom_pull_detal_2_expr] } {
                       set detal_2_expr [string map {- -ABS\[} $mom_pull_detal_2_expr]
                       append detal_2_expr "\]"
                 }
...
MOM_output_text "(--------------------------)"
    if {[info exists var531]} {
          MOM_output_text "\#531=$var531 (MOVE SUBSPINDLE)"
    }
    if {[info exists var532]} {
          MOM_output_text "\#532=$var532 (MOVE PART)"
    }
    if {[info exists var533]} {
          MOM_output_text "\#533=$var533 (MOVE SUBSPINDLE)"
    }
    if {[info exists var534]} {
          MOM_output_text "\#534=$var534 (MOVE PART)"
    }
MOM_output_literal "G28 U0.0(X GO HOME)"
MOM_output_literal "M200(SPIN 1 MILL MODE)"
MOM_output_literal "M300(SPIN 2 MILL MODE)"
...
if { [info exists free_1_expr] && $free_1_expr != "none" } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025+\[$free_1_expr\]\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
     MOM_output_literal "G04 G98 P1000(PAUSE)"
   }
if { [info exists detal_1_expr] && $detal_1_expr != "none" } {
     MOM_output_literal "M206(SPIN 1 OPEN)"
     MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#5025+\[$detal_1_expr\]\] F500.0(MOVING PART IN MACHINE AREA)"
     MOM_output_literal "M207(SPIN 1 CLOSE)"
     MOM_output_literal "M543(TRANSFER MODE OFF)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
     set mom_var511 "\#511+\[$detal_1_expr\](SHIFT Z)"
     MOM_output_text "\#511=$mom_var511"
   }
if { [info exists free_2_expr] && $free_2_expr != "none" } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025+\[$free_2_expr\]\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
   }
if { [info exists detal_2_expr] && $detal_2_expr != "none" } {
     MOM_output_literal "M206(SPIN 1 OPEN)"
     MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#5025+\[$detal_2_expr\]\] F500.0(MOVING PART IN MACHINE AREA)"
     MOM_output_literal "M207(SPIN 1 CLOSE)"
     MOM_output_literal "M543(TRANSFER MODE OFF)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
     set mom_var511 "\#511+\[$detal_2_expr\](SHIFT Z)"
     MOM_output_text "\#511=$mom_var511"
   }
...
# ============================================
proc MOM_qtn_param_subspindle_gohome { } {
# ============================================
global part_mod
if {[info exists part_mod] && ($part_mod == "shpilka" || $part_mod == "shtock" || $part_mod == "separator") } {
       SGT_subspindle_gohome
       return
}

## "MAZAK QTN 200 MSY  : Parameters. Отвод шпинделя #2 "
global mom_qtn_subspin_gohome_opt ; # "0. Отвести пустой","1. Забрать деталь","2. Отломить и забрать надрезанную деталь"
global mom_qtn_var505 ; # TYPE s DEFVAL "#505" TOGGLE Off "+ вытянуть  / - задвинуть заготовку."
...
global SYNCHRO_mode SYNCHRO_status
  ...
  if {[info exists mom_qtn_subspin_gohome_opt] } {
         set subspin_opt [string index $mom_qtn_subspin_gohome_opt 0]
       ...
       if {[info exists mom_qtn_var505] && \
           [string length $mom_qtn_var505] > 0 } {
                 ...
                 if { [string match "*+\#*" $mom_qtn_var505] } {
                       set pull_out_expr [string map {+ ABS\[} $mom_qtn_var505]
                       append pull_out_expr "\]"
                 } elseif { [string match "*-\#*" $mom_qtn_var505] } {
                       set pull_out_expr [string map {- -ABS\[} $mom_qtn_var505]
                       append pull_out_expr "\]"
                 }
...
SYNCHRO_OFF
  ...
  if { [info exist pull_out_expr] && $pull_out_expr != "none" } {
         MOM_output_literal "M200(SPIN 1 MILL MODE)"
         MOM_output_literal "M300(SPIN 2 MILL MODE)"
         MOM_output_literal "M206(SPIN 1 OPEN)"
         MOM_output_literal "M542(TRANSFER MODE ON)"
         MOM_output_literal "G53 G1 G98 B\[\#5025+\[$pull_out_expr\]\] F500.0(MOVING PART IN MACHINE AREA)"
         MOM_output_literal "M207(SPIN 1 CLOSE)"
         MOM_output_literal "M543(TRANSFER MODE OFF)"
         set mom_var511 "\#511+\[$pull_out_expr\] (SHIFT Z)"
         MOM_output_text "\#511=$mom_var511"
     }
  ...
  if { $subspin_opt == 2 } {
   ...
   if { $subspin_opt != 0 } {
          MOM_output_text "\#520=\#511+ABS\[\#5025\]-\#502 (DATUM SHIFT)"
      }
...
#MOM_output_literal "HERE is mom_tool_direction= $mom_tool_direction"
}
...
# ============================================
global part_mod
global Var503 Var504 Var505 Var506

  MOM_output_text "(===================================)"
   if {[catch {MOM_ask_ess_exp_value "var502"}] == 0 } {
           set var502 [expr double([MOM_ask_ess_exp_value "var502"])]
           MOM_output_text "\#502=[format %0.4f $var502] (SET PART LENGTH)"
    } else {
           MOM_output_text "\#502=0 (USER DEFINED PART LENGTH)"
       }
  MOM_output_text "\#1=1 (0 DONT USE BARFEEDER)"
  MOM_output_text "(===================================)"

  if {[info exists part_mod] && $part_mod != "separator" } {
       if { [catch {MOM_ask_ess_exp_value "var503"}] == 0 } {
              set Var503 [expr double([MOM_ask_ess_exp_value "var503"])]
              MOM_output_text "\#503=[format %0.4f $Var503] (BASE PART LENGTH)"
              MOM_output_text "\#555=\#502-\#503 (DELTA LENGTH)"
       } else {
              set pause_ "Отсутствует переменная Var503.\n"
              append pause_ "Длина базовой детали неизвестна.\n"
              append pause_ "Работа постпроцессора будет прервана."
              PAUSE $pause_
              MOM_abort "$pause_"
        }
  }
      if { [catch {MOM_ask_ess_exp_value "var504"}] == 0 } {
              set Var504 [expr double([MOM_ask_ess_exp_value "var504"])]
              MOM_output_text "\#504=[format %0.4f [expr abs($Var504)]] (BASE PART. TAKEOVER LENGTH)"
         }
       if { [catch {MOM_ask_ess_exp_value "var505"}] == 0 } {
              set Var505 [expr double([MOM_ask_ess_exp_value "var505"])]
              MOM_output_text "\#505=[format %0.4f [expr abs($Var505)]] (BASE PART. PUSH/PULL IN/OUT CHUCK)"
          }
      if { [catch {MOM_ask_ess_exp_value "var506"}] == 0 } {
              set Var506 [expr double([MOM_ask_ess_exp_value "var506"])]
              MOM_output_text "\#506=[format %0.4f [expr abs($Var506)]] (BASE PART. MOVE SP2 WITHOUT PART)"
         }
   MOM_output_text "(===================================)"
      if {[info exists Var505]} {
              MOM_output_text "\#505=\#505+\#555 (NEW VAR505)"
         }
      if {[info exists Var506]} {
              MOM_output_text "\#506=\#506+\#555 (NEW VAR506)"
         }
   MOM_output_text "\#521=138.658 (SUB CHUCK LENGTH)"
   MOM_output_text "(===================================)"
#   MOM_output_text "\#1=1 (0 DONT USE BARFEEDER)"
#   MOM_output_text "IF \[\#1EQ0\] GOTO 99992 (SKIP BARFEEDER)"
}
...
#=============================================================
proc PB_CMD_SGT_subspin { } {
#=============================================================
uplevel #0 {

# INCR_GOTO
# set goto_mark_ [GET_GOTO]
# MOM_output_text "( BARFEEDER )"
# MOM_output_text "IF \[\#1EQ0\] GOTO $goto_mark_ (SKIP BARFEEDER)"
...
global CUTOFF_mode
    ...
    if {[info exist SYNCHRO_status] && $SYNCHRO_status != "OFF" } { return }
    set take_mode [string index $mom_qtn_takeover_opt 0]
    # "0. Рассчитать через переменные #521 #504","1. Настроить #501 на станке. #504 справочная.","2. Использовать готовую #501."

if {[info exists part_mod] && ($part_mod == "shpilka" || $part_mod == "shtock") } {

       if {[info exists mom_qtn_var505] && \
           [string length $mom_qtn_var505] > 0 } {
               if {[string match "*-\#*" $mom_qtn_var505] } {
                      set pull_on_expr [string map {- ""} $mom_qtn_var505]
                      set pull_on_expr "-$pull_on_expr"
               } else {
                      set pull_on_expr [string map {+ ""} $mom_qtn_var505]
                      set pull_on_expr "+$pull_on_expr"
                  }
       ...
       if {[info exists mom_qtn_var506] && \
           [string length $mom_qtn_var506] > 0 } {
               if {[string match "*-\#*" $mom_qtn_var506] } {
                      set move_on_expr [string map {- ""} $mom_qtn_var506]
                      set move_on_expr "-$move_on_expr"
               } else {
                      set move_on_expr [string map {+ ""} $mom_qtn_var506]
                      set move_on_expr "+$move_on_expr"
                  }
       ...
       if {[info exists mom_qtn_var505] && \
           [string length $mom_qtn_var505] > 0 } {
               if {[string match "*-\#*" $mom_qtn_var505] } {
                      set pull_on_expr [string map {- ""} $mom_qtn_var505]
                      set pull_on_expr "-\[$pull_on_expr+\#555\]"
               } else {
                      set pull_on_expr [string map {+ ""} $mom_qtn_var505]
                      set pull_on_expr "+\[$pull_on_expr+\#555\]"
                  }
       ...
       if {[info exists mom_qtn_var506] && \
           [string length $mom_qtn_var506] > 0 } {
               if {[string match "*-\#*" $mom_qtn_var506] } {
                      set move_on_expr [string map {- ""} $mom_qtn_var506]
                      set move_on_expr "-\[$move_on_expr+\#555\]"
               } else {
                      set move_on_expr [string map {+ ""} $mom_qtn_var506]
                      set move_on_expr "+\[$move_on_expr+\#555\]"
                  }
...
# MOM_output_text "(--------------------------)"
    ...
    if { $take_mode == 0 } {
          MOM_output_text "(--------------------------)"
          MOM_output_literal "M00( !!!WARNING!!! )"
          MOM_output_text "\#501=\#9101-634+\#521-\#504+\#511 (TAKEOVER POS)"
          MOM_output_text "(--------------------------)"
    } elseif { $take_mode == 2 } {
           if {![info exists mom_qtn_var501] } {
                PAUSE "Var 501 Is'nt Exists\nUpdate Var 501"
          ...
          MOM_output_text "(--------------------------)"
          MOM_output_literal "M00(SET SUBSPINDLE TOUCH POSITION)"
          MOM_output_text "(MOVE SUBSPINDLE $Var504 MM OVER PART. SET V501 = Z2)"
          MOM_output_text "\#501=0.0 (SPINDLE 2 TAKEOVER POSITION)"

          INCR_GOTO
          set goto_mark_ [GET_GOTO]
          MOM_output_text "IF \[\#501NE0\] GOTO $goto_mark_"
          MOM_output_text "\#3006=1( SET var501 )"
          MOM_output_text "M0"
          MOM_output_text "N$goto_mark_"
          MOM_output_text "(--------------------------)"
...
MOM_output_literal "G110 C2(C = C2)"
    ...
    if {[info exists mom_qtn_c2_pos] } {
          MOM_output_literal "G0 C[format %0.4f $mom_qtn_c2_pos](SUBSPINDLE ORIENT)"
          unset mom_qtn_c2_pos
    } else {
          MOM_output_literal "G0 C0.0(SUBSPINDLE ORIENT)"
      }

MOM_output_literal "G111(C = C1)"
MOM_output_literal "M901(SPIN 1 ACTIVE)"
MOM_output_literal "M200(SPIN 1 MILL MODE)"
MOM_output_literal "G0 C0.0(MAIN SPINDLE ORIENT)"
...
if { ![info exist mom_qtn_press] } { set mom_qtn_press 0.0 }
if { [info exist mom_qtn_clrs] && $mom_qtn_clrs != 0.0 } {
     MOM_output_literal "G53 G0 B\[\#501+[format "%0.3f" $mom_qtn_clrs]\](W RAPID POS)"
       MOM_output_literal "M508 (PRESS ON)"
     MOM_output_literal "G31 B\[\#501-[format "%0.3f" $mom_qtn_press]\] (W FEED POS)"
       MOM_output_literal "M509 (PRESS OFF)"
     unset mom_qtn_clrs
} else {
     MOM_output_literal "G53 G0 B\#501(W RAPID POS)"
  }
    MOM_output_literal "G4 G98 P2000 (PAUSE)"
    MOM_output_literal "M307(SPIN 2 CLOSE)"
    MOM_output_literal "M541(TAKEOVER MODE OFF)"

if { [info exist mom_qtn_var505] && $mom_qtn_var505 != "none" } {
     MOM_output_literal "G04 G98 P1000(PAUSE)"
     MOM_output_literal "M206(SPIN 1 OPEN)"
       ## MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#501$pull_on_expr\] F500.0(MOVING PART IN MACHINE AREA)"
...
if { [info exist mom_qtn_var506] && $mom_qtn_var506 != "none" } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025$move_on_expr\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
...
if {[info exists part_mod] && ($part_mod == "shpilka" || $part_mod == "shtock") } {
       ...
       if {[info exists mom_pull_free_1_expr] && \
           [string length $mom_pull_free_1_expr] > 0 } {
               if {[string match "*-\#*" $mom_pull_free_1_expr] } {
                      set free_1 [string map {- ""} $mom_pull_free_1_expr]
                      set free_1 "-$free_1"
               } else {
                      set free_1 [string map {+ ""} $mom_pull_free_1_expr]
                      set free_1 "+$free_1"
       ...
       if {[info exists mom_pull_free_2_expr] && \
           [string length $mom_pull_free_2_expr] > 0 } {
               if {[string match "*-\#*" $mom_pull_free_2_expr] } {
                      set free_2 [string map {- ""} $mom_pull_free_2_expr]
                      set free_2 "-$free_2"
               } else {
                      set free_2 [string map {+ ""} $mom_pull_free_2_expr]
                      set free_2 "+$free_2"
       ...
       if {[info exists mom_pull_detal_1_expr] && \
           [string length $mom_pull_detal_1_expr] > 0 } {
               if {[string match "*-\#*" $mom_pull_detal_1_expr] } {
                      set pull_1 [string map {- ""} $mom_pull_detal_1_expr]
                      set pull_1 "-$pull_1"
               } else {
                      set pull_1 [string map {+ ""} $mom_pull_detal_1_expr]
                      set pull_1 "+$pull_1"
       ...
       if {[info exists mom_pull_detal_2_expr] && \
           [string length $mom_pull_detal_2_expr] > 0 } {
               if {[string match "*-\#*" $mom_pull_detal_2_expr] } {
                      set pull_2 [string map {- ""} $mom_pull_detal_2_expr]
                      set pull_2 "-$pull_2"
               } else {
                      set pull_2 [string map {+ ""} $mom_pull_detal_2_expr]
                      set pull_2 "+$pull_2"
...
if { [info exists free_1] } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025$free_1\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
     MOM_output_literal "G04 G98 P1000(PAUSE)"
   }
if { [info exists pull_1] } {
     MOM_output_literal "M206(SPIN 1 OPEN)"
     MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#5025$pull_1\] F500.0(MOVING PART IN MACHINE AREA)"
     MOM_output_literal "M207(SPIN 1 CLOSE)"
     MOM_output_literal "M543(TRANSFER MODE OFF)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
     set mom_var511 "\#511$pull_1 (SHIFT Z)"
     MOM_output_text "\#511=$mom_var511"
   }
if { [info exists free_2] } {
     MOM_output_literal "M306(SPIN 2 OPEN)"
     MOM_output_literal "G53 G1 G98 B\[\#5025$free_2\] F500.0(SPIN 2 MOVE)"
     MOM_output_literal "M307(SPIN 2 CLOSE)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
   }
if { [info exists pull_2] } {
     MOM_output_literal "M206(SPIN 1 OPEN)"
     MOM_output_literal "M542(TRANSFER MODE ON)"
     MOM_output_literal "G53 G1 G98 B\[\#5025$pull_2\] F500.0(MOVING PART IN MACHINE AREA)"
     MOM_output_literal "M207(SPIN 1 CLOSE)"
     MOM_output_literal "M543(TRANSFER MODE OFF)"
     MOM_output_literal "G04 G98 P1000(TIME DWELL)"
     set mom_var511 "\#511$pull_2 (SHIFT Z)"
     MOM_output_text "\#511=$mom_var511"
   }
...
SYNCHRO_OFF
  ...
  if { [info exist mom_qtn_var505] && $mom_qtn_var505 != "none" } {
         ...
         MOM_output_literal "M206(SPIN 1 OPEN)"
         MOM_output_literal "M542(TRANSFER MODE ON)"
         MOM_output_literal "G53 G1 G98 B\[\#5025$pull_on_expr\] F500.0(MOVING PART IN MACHINE AREA)"
         MOM_output_literal "M207(SPIN 1 CLOSE)"
         MOM_output_literal "M543(TRANSFER MODE OFF)"
         set mom_var511 "\#511$pull_on_expr (SHIFT Z)"
         MOM_output_text "\#511=$mom_var511"
  ...
  if { $subspin_opt == 2 } {
   ...
   if { $subspin_opt != 0 } {
          MOM_output_text "\#520=\#511+ABS\[\#5025\]-\#502 (DATUM SHIFT)"
         # MOM_output_text "\#511=\#511+ABS\[\#5025\]-\#502 (DATUM SHIFT)"
      }
...
# ============================================
proc MOM_qtn_subspindle_cutoff { } {
# ============================================
## EVENT "MAZAK QTN : Контршпиндель : Забрать отрезанную деталь"
global mom_machine_mode
global SYNCHRO_mode SYNCHRO_status
global CUTOFF_mode
...
#=============================================================
proc PB_CMD_SHPILKA_start { } {
#=============================================================
global part_mod
  if { ![info exists part_mod] || $part_mod != "shpilka" } { return }
...
global Var502
    if { [catch {MOM_ask_ess_exp_value "var502"}] == 0 } {
           set Var502 [expr double([MOM_ask_ess_exp_value "var502"])]
    } else { set Var502 $Var503 }

global Var504
    if { [catch {MOM_ask_ess_exp_value "var504"}] == 0 } {
           set Var504 [expr double([MOM_ask_ess_exp_value "var504"])]
    } else {
           set pause_ "Отсутствует переменная Var504.\n"
           append pause_ "Глубина захвата детали в КШП неизвестна.\n"
           append pause_ "Работа постпроцессора будет прервана."
           PAUSE $pause_
           MOM_abort "$pause"
     }
...
global Var505
    if { [catch {MOM_ask_ess_exp_value "var505"}] == 0 } {
           ...
           set pause_ "Отсутствует переменная Var505.\n"
           append pause_ "Величина задвигания детали в ШП неизвестна.\n"
           append pause_ "Работа постпроцессора будет прервана."
           PAUSE $pause_
           MOM_abort "$pause"
     }
...
global Var506
    if { [catch {MOM_ask_ess_exp_value "var506"}] == 0 } {
           ...
           set pause_ "Отсутствует переменная Var506.\n"
           append pause_ "Расстояние перепозиционирования КШП неизвестно.\n"
           append pause_ "Работа постпроцессора будет прервана."
           PAUSE $pause_
           MOM_abort "$pause"
     }
   ...
   MOM_output_text "(===================================)"
   MOM_output_text "\#502=[format %0.4f $Var502] (DLINA GOTOVOY DETALI)"
   MOM_output_text "(===================================)"
   MOM_output_text "\#503=[format %0.4f $Var503] (DLINA BAZOVOY DETALI)"
   MOM_output_text "\#504=[format %0.4f $Var504] (ZAHVAT SP2)"
   MOM_output_text "\#505=[format %0.4f $Var505] (ZADVINUT DETAL)"
   MOM_output_text "\#506=[format %0.4f $Var506] (VYDVYNUT SP2)"
   MOM_output_text "\#521=139.0 (DLINA PATRONA SUB SPINDELYA)"
   MOM_output_text "\#555=\#502-\#503 (RAZNITSA DLIN)"
   MOM_output_text "(===================================)"

   MOM_output_text "IF \[#505LT0\] GOTO 11111"
   MOM_output_text "\#505=\#505+\#555"
   MOM_output_text "GOTO 11112"
   MOM_output_text "N11111"
   MOM_output_text "\#505=\#505-\#555"
   MOM_output_text "N11112"

   MOM_output_text "IF \[#506LT0\] GOTO 11113"
   MOM_output_text "\#506=\#506+\#555"
   MOM_output_text "GOTO 11114"
   MOM_output_text "N11113"
   MOM_output_text "\#506=\#506-\#555"
   MOM_output_text "N11114"

}


#=============================================================
proc PB_CMD_SHTOK_start { } {
#=============================================================
global part_mod
  if { ![info exists part_mod] || $part_mod != "shtock" } { return }
...
#=============================================================
proc PB_CMD_START_list { } {
#=============================================================
global SETUP_list
global PARAM_def_list
global START_list

    if {[info exists SETUP_list] } {
         foreach line_ $SETUP_list {
             MOM_output_text $line_
         }
       unset SETUP_list
    }
    if {[info exists PARAM_def_list] } {
         foreach line_ $PARAM_def_list {
             MOM_output_text $line_
         }
       unset PARAM_def_list
    }
    if {[info exists START_list] } {
         foreach line_ $START_list {
...
global part_mod
    if { [catch {MOM_ask_ess_exp_value "Part_type"}] == 0 } {
            ...
            PAUSE "Постпроцессор предназначен для серийных типовых изделий.\nИспользовать его в данном случае невозможно.\n"
            MOM_abort "User Exit"
      }
...
global PART_modification
    if { [catch {MOM_ask_ess_exp_value "MODIFICATION"}] == 0 } {
            ...
            set mom_current_modification "MODIFICATION"
       }

if { $part_mod == "shpilka" } { return }
if { $part_mod == "separator" } { return }
if { $part_mod == "shtock" } { return }
...
#=============================================================
proc PB_CMD_TOOL_save_direction { } {
#=============================================================
...
## Group Of operations defined for modification $mom_part_modification
## Can be defined as list in NX session
## For example PART_list_1 PART_list_2
...
global mom_group_name modification_group
    ...
    if { [catch {MOM_ask_ess_exp_value $mom_part_modification}] == 0 } {
            set MOD_list [format "%s" [MOM_ask_ess_exp_value $mom_part_modification]]
       }

   if {[info exists MOD_list] } {
            set MOD_list [string map { \{ "" \} "" \" "" , " " } $MOD_list]
            set MOD_list [split $MOD_list]
       if {[set mod_search_ [lsearch $MOD_list $PART_modification]] >= 0 } {
             MOM_output_text "( Part Modification Is Group $mom_part_modification )"
             # PAUSE "Исполнение задано для группы $mom_part_modification.\n"
             return
          }
     }
    ...
    if {[string match {*\*} $mom_part_modification]} {
          if {[string match "*$mom_part_modification*" $PART_modification] } {
                 MOM_output_text "( Operation for Part Modifications $mom_part_modification )"
                 return
             }
       }

    if {[string match "*$PART_modification*" $mom_part_modification] } {
          MOM_output_text "( Operation for Part Modifications $mom_part_modification )"
          return
       }
...
#==========================
## UI_LABEL "Исполнение детали"
## Defined at Start of main program Group and Currently Active Part modification
## Can be defined as "MODIFICATION" variable in NX session
...
global PART_modification
    ...
    if {![info exists mom_current_modification_defined] || $mom_current_modification_defined != 1 } { return }

    if { [catch {MOM_ask_ess_exp_value $mom_current_modification}] == 0 } {
              set PART_modification [format "%s" [MOM_ask_ess_exp_value $mom_current_modification]]
    } else {
              set PART_modification $mom_current_modification
      }
...
## EVENT use_etalon_shift
## UI_LABEL "Использовать смещение от основного исполнения"
global mom_etalon_modification ; # #503 Base parametr
global mom_use_parameters ; # #502 Использовать параметры
global mom_use_parameters_defined
global cutoff_offset
global part_mod

if { $part_mod == "shpilka" || $part_mod == "shtock" } {
       set cutoff_offset "\#555"
       return
}

    if {[info exists mom_use_parameters_defined] && $mom_use_parameters_defined == 1 } {
            set var502 $mom_use_parameters
            if {[info exists mom_etalon_modification] && [string length $mom_etalon_modification] > 1 } {
                   set var503 $mom_etalon_modification
            } else { return }
    } else { return }

  # MOM_output_text "\#555=$var503-$var502"
#  set cutoff_offset "$var503-$var502"
  # set cutoff_offset "\#555=$var503-$var502"
return

    if { [catch {MOM_ask_ess_exp_value $var503}] == 0 } {
           set value503 [expr double([MOM_ask_ess_exp_value $var503])]
    } else { set value503 0 }

    if { [catch {MOM_ask_ess_exp_value $var502}] == 0 } {
           set value502 [expr double([MOM_ask_ess_exp_value $var502])]
    } else { set value502 0 }

  set cutoff_offset [expr $Var503 - $Var502 ]
return
}
...
#=============================================================
proc PB_CMD_UPLEVEL_standart_subspindle { } {
#=============================================================
uplevel #0 {
...
MOM_output_text "\#520=\#511+ABS\[\#5025\]-\#502(DATUM SHIFT ALONG Z)"
MOM_output_literal "M202 M302(SPIN 1 2 TURN MODE)"
...
global SYNCHRO_mode SYNCHRO_status
  ...
  if { $operation_opt == 2 && [info exists mom_qtn_subspin_v501_defined] && $mom_qtn_subspin_v501_defined == 1 } {
        ...
        MOM_output_text "( \#501 IS A G53 BASED Z2 COORDINATE. V521 AND V504 WILL BE IGNORED)"

  } elseif { $operation_opt == 1 } {
        MOM_output_text "( \#521 AND \#504 IGNORED)"
        MOM_output_literal "M01 (SET V501 TAKEOVER POSITION)"
        MOM_output_text "\#501=0 ( DEFINE AND PRINT HERE SUB CHUCK TAKEOVER POSITION )"
        ...
        MOM_output_text "(V521 AND V504)"
        MOM_output_text "\#504=[format "%0.4f" $mom_qtn_subspin_v504] (TAKEOVER LENGTH)"
        MOM_output_text "\#501=-634+\#9101+\#521-\#504 (AUTO DEFINED TAKEOVER POSITION)"
        MOM_output_text "(V521 DEFINED AT PROGRAM START)"
  } else {
        PAUSE "Не задан вариант подхода\n"
    }
##
    if {[info exists mom_qtn_subspin_pull_defined] && $mom_qtn_subspin_pull_defined == 1 } {
         MOM_output_text "\#505=[format "%0.4f" $mom_qtn_subspin_pull] (PULL PART)"
       }
    if {[info exists mom_qtn_subspin_move_defined] && $mom_qtn_subspin_move_defined == 1 } {
         MOM_output_text "\#506=[format "%0.4f" $mom_qtn_subspin_move] (PULL PART \#505 AND REPOS SUBSPINDLE \#506)"
       }
...
## CODES
    ...
    if { $mom_qtn_subspin_press > 0 } {
                  MOM_output_literal "G53 G0 G90 B\[\#501+5.0\] (B axis RAPID POS)"
                  MOM_output_literal "G31 B\[\#501-[format "%0.3f" $mom_qtn_subspin_press]\] (B axis PRESS POS)"
    } else {
                  MOM_output_literal "G53 G0 G90 B\#501 (B axis RAPID POS)"
     }
  ...
  if {[info exists mom_qtn_subspin_pull_defined] && $mom_qtn_subspin_pull_defined == 1 } {
        ...
        MOM_output_literal "M543(TRANSFER MODE OFF)"
          ...
          set SHIFT_9101 [expr $SHIFT_9101 + $mom_qtn_subspin_pull]
          # MOM_output_literal "\#3024=\[\#3024+\#505\] (MCS SHIFT)"

      if {[info exists mom_qtn_subspin_move_defined] && $mom_qtn_subspin_move_defined == 1 } {
            MOM_output_literal "M306(SUB CHUCK  OPEN)"
            MOM_output_literal "G4 G98 P2000 (PAUSE)"
            MOM_output_literal "G53 G1 G98 B\[\#5025+\#506\] F500.0(MOVING PART)"
            MOM_output_literal "M307(SUB CHUCK CLOSE)"
            MOM_output_literal "G4 G98 P2000 (PAUSE)"
         }
  }
     ...
     UI_UNSET mom_qtn_subspin_move
     UI_UNSET mom_qtn_subspin_pull
     UI_UNSET mom_qtn_subspin_move_defined
     UI_UNSET mom_qtn_subspin_pull_defined
...
global SYNCHRO_mode SYNCHRO_status
    ...
    if {[info exists mom_qtn_move_subspin_1_defined] && $mom_qtn_move_subspin_1_defined == 1 } {
           MOM_output_text "\#512=[format "%0.4f" $mom_qtn_move_subspin_1] (MOVE SUBSPINDLE)"
    }
    if {[info exists mom_qtn_move_part_1_defined] && $mom_qtn_move_part_1_defined == 1 } {
           MOM_output_text "\#513=[format "%0.4f" $mom_qtn_move_part_1] (MOVE PART IN MACHINE AREA)"
    }
    if {[info exists mom_qtn_move_subspin_2_defined] && $mom_qtn_move_subspin_2_defined == 1 } {
           MOM_output_text "\#514=[format "%0.4f" $mom_qtn_move_subspin_2] (MOVE SUBSPINDLE)"
    }
    if {[info exists mom_qtn_move_part_2_defined] && $mom_qtn_move_part_2_defined == 1 } {
           MOM_output_text "\#515=[format "%0.4f" $mom_qtn_move_part_2] (MOVE PART IN MACHINE AREA)"
    }
    ...
    if {[info exists mom_qtn_move_subspin_1_defined] && $mom_qtn_move_subspin_1_defined == 1 } {
           MOM_output_literal "M306(SPIN 2 OPEN)"
           MOM_output_literal "G53 G1 G98 B\[\#5025+\#512\] F500.0(SPIN 2 MOVE)"
           MOM_output_literal "M307(SPIN 2 CLOSE)"
           MOM_output_literal "G04 G98 P1000(TIME DWELL)"
    }
    if {[info exists mom_qtn_move_part_1_defined] && $mom_qtn_move_part_1_defined == 1 } {
           MOM_output_literal "M206(SPIN 1 OPEN)"
           MOM_output_literal "M542(TRANSFER MODE ON)"
           MOM_output_literal "G53 G1 G98 B\[\#5025+\#513\] F500.0(PART MOVE)"
           MOM_output_literal "M207(SPIN 1 CLOSE)"
           MOM_output_literal "M543(TRANSFER MODE OFF)"
           MOM_output_literal "G04 G98 P1000(TIME DWELL)"
              MOM_output_literal "\#511=\[\#511+\#513\] (MCS SHIFT)"
              set SHIFT_9101 [expr $SHIFT_9101 + $mom_qtn_move_part_1]
              # MOM_output_literal "\#3024=\[\#3024+\#513\] (MCS SHIFT)"
    }
    if {[info exists mom_qtn_move_subspin_2_defined] && $mom_qtn_move_subspin_2_defined == 1 } {
           MOM_output_literal "M306(SPIN 2 OPEN)"
           MOM_output_literal "G53 G1 G98 B\[\#5025+\#514\] F500.0(SPIN 2 MOVE)"
           MOM_output_literal "M307(SPIN 2 CLOSE)"
           MOM_output_literal "G04 G98 P1000(TIME DWELL)"
    }
    if {[info exists mom_qtn_move_part_2_defined] && $mom_qtn_move_part_2_defined == 1 } {
           MOM_output_literal "M206(SPIN 1 OPEN)"
           MOM_output_literal "M542(TRANSFER MODE ON)"
           MOM_output_literal "G53 G1 G98 B\[\#5025+\#515\] F500.0(PART MOVE)"
           MOM_output_literal "M207(SPIN 1 CLOSE)"
           MOM_output_literal "M543(TRANSFER MODE OFF)"
           MOM_output_literal "G04 G98 P1000(TIME DWELL)"
              MOM_output_literal "\#511=\[\#511+\#515\] (MCS SHIFT)"
              set SHIFT_9101 [expr $SHIFT_9101 + $mom_qtn_move_part_2]
              # MOM_output_literal "\#3024=\[\#3024+\#515\] (MCS SHIFT)"
    }
    MOM_force once G_feed G_motion
...
global mom_machine_mode mom_spindle_direction
global mom_coolant_status
global SYNCHRO_mode SYNCHRO_status
global SHIFT_9101

  if {![info exists SYNCHRO_status] || $SYNCHRO_status == "OFF" } {
        ## return
     }
  ...
  if { $gohome_opt == 0 } {
       ...
       if {[info exists mom_qtn_subspin_pull_free_defined] && $mom_qtn_subspin_pull_free_defined == 1 } {
            MOM_output_text "\#505=[format "%0.4f" $mom_qtn_subspin_pull_free] (MOVE PART IN MACHINE AREA)"
          }
  } elseif { $gohome_opt == 1 } {
       ...
       if {[info exists mom_qtn_subspin_move_take_defined] && $mom_qtn_subspin_move_take_defined == 1 } {
             MOM_output_text "\#506=[format "%0.4f" $mom_qtn_subspin_move_take] (CHANGE TAKEOVER LENGTH)"
          }
  } elseif { $gohome_opt == 2 } {
       ...
       if {[info exists mom_qtn_subspin_pull_break_defined] && $mom_qtn_subspin_pull_break_defined == 1 } {
             MOM_output_text "\#505=[format "%0.4f" $mom_qtn_subspin_pull_break] (MOVE PART IN MACHINE AREA)"
          }
       if {[info exists mom_qtn_subspin_move_break_defined] && $mom_qtn_subspin_move_break_defined == 1 } {
             MOM_output_text "\#506=[format "%0.4f" $mom_qtn_subspin_move_break] (CHANGE TAKEOVER LENGTH)"
          }
    } ; # opt
  ...
  if { $gohome_opt == 0 } {
      if {[info exists mom_qtn_subspin_pull_free_defined] && $mom_qtn_subspin_pull_free_defined == 1 } {
       ...
       MOM_output_literal "M206(SPIN 1 OPEN)"
       MOM_output_literal "M542(TRANSFER MODE ON)"
          MOM_output_literal "\#511=\[\#511+ABS\[\#5025\]-\#502\] (MCS SHIFT)"
          # MOM_output_literal "\#3024=ABS\[\#5025\]"
       MOM_output_literal "G28 B0.0(SUB SPIN GO HOME)"
       MOM_output_literal "M543(TRANSFER MODE OFF)"
       MOM_output_literal "G04 G98 P1000(TIME DWELL)"
       MOM_output_text "(SPIN 2 GOHOME WITH PART COMPLETED)"
         ...
         MOM_output_literal "M202(SPIN 1 TURN MODE)"
         MOM_output_literal "M210(MAIN SPIN CLAMP ON)"
         MOM_output_literal "M312(MAIN SPIN CLAMP OFF)"
         MOM_output_literal "M300(SPIN 2 MILL MODE)"
  ...
  if {[info exists mom_qtn_subspin_move_break_defined] && $mom_qtn_subspin_move_break_defined == 1 } {
         MOM_output_literal "M306(SPIN 2 OPEN)"
         MOM_output_literal "G53 G1 G98 B\[\#5025+\#506\] F500.0(MOVING SUBSPIN)"
         MOM_output_literal "M307(SPIN 2 CLOSE)"
     }
     ...
     MOM_output_literal "G111(C = C1)"
     MOM_output_literal "M302(SPIN 2 TURN MODE)"
          # MOM_output_literal"\#3024=ABS\[\#5025\] (DATUM SHIFT ALONG Z)"
          # MOM_output_literal "\#511=\[\#511+ABS\[\#5025\]-\#502\] (MCS SHIFT)"
          MOM_output_literal "\#520=\[\#511+ABS\[\#5025\]-\#502\] (MCS SHIFT)"
     MOM_output_literal "G28 B0.0(SPIN 2 GO HOME)"
     MOM_output_literal "M58(SPIN 1 AIR BLAST)"
    ...
    UI_UNSET mom_qtn_subspin_pull_free
    UI_UNSET mom_qtn_subspin_pull_free_derined
    UI_UNSET mom_qtn_subspin_move_take
    UI_UNSET mom_qtn_subspin_move_take_derined
    UI_UNSET mom_qtn_subspin_pull_break
    UI_UNSET mom_qtn_subspin_move_break
    UI_UNSET mom_qtn_subspin_pull_break_defined
    UI_UNSET mom_qtn_subspin_move_break_defined
}

# ============================================
proc MOM_qtn_subspindle_cutoff__ { } {
# ============================================
## EVENT "MAZAK QTN : Контршпиндель : Забрать отрезанную деталь"
global mom_machine_mode
global mom_spindle_direction
...
##
     MOM_output_text "(SPIN CUTOFF PART MODE)"
##
  if { $mom_spindle_direction != "OFF" } {
         set mom_spindle_direction OFF
         MOM_output_literal "M05 M305"
     }
## Coolant OFF
...
global BARFEEDER_active
     ...
     if {[info exists BARFEEDER_status] && $BARFEEDER_status == "OFF" } {
           return
        }
...
if { $s_ind != 0 } {
       MOM_output_text "N$goto_mark_ (SKIP BARFEEDER)"
   }
}
...
## boremilling_1
    ...
    set types_ "drilling boremilling csink fin_holes"
...
global mom_sys_master_post

     set tcl_file_ "[file dirname $mom_sys_master_post]/$mom_sys_postname(SEPARATOR).tcl"
     set def_file_ "[file dirname $mom_sys_master_post]/$mom_sys_postname(SEPARATOR).def"

     set mom_load_event_handler "\"$tcl_file_\""
     MOM_load_definition_file    $def_file_

#    MOM_output_to_listing_device "tcl_file_ = $tcl_file_"
#    MOM_output_to_listing_device "def_file_ = $def_file_"
#    set mom_load_event_handler "{$tcl_file_}"
#    MOM_load_definition_file   "${def_file_}"
...
if { $mom_next_oper_has_tool_change == "YES" ||
     $mom_current_oper_is_last_oper_in_program == "YES" } {
       set mom_coolant_status OFF
       set mom_spindle_direction OFF
       if {[info exists TOOL_direction]} { unset TOOL_direction }
} else {
       PB_CMD_skip9_on
       set mom_coolant_status OFF
       set mom_spindle_direction OFF
...
set SPINDLE_mode(TURN) "M202"
set SPINDLE_mode(MILL) "M200"
set SUBSPINDLE_mode(TURN) "M302"
set SUBSPINDLE_mode(MILL) "M300"
...
#============================
global GOTO_mark
    if {![info exists GOTO_mark]} { INCR_GOTO }
    return $GOTO_mark
}
...
MOM_output_text "( POST VERSION 17.05 SERIAL TECHNOLOGY )"
...
set mom_var511 "0"
MOM_output_text "(===================================)"
MOM_output_text "\#511=0.0 ( Z AXIS DATUM SHIFT )"
MOM_output_text "(===================================)"
MOM_output_literal "G53.5"
...
#=============================================================
MOM_force once S M_spindle M_coolant X Y Z fifth_axis F G_mode G_motion G_feed
...
global mom_fixture_offset_value
global mom_tool_holder_num_toggle mom_tool_holder_num
...
if {([info exist mom_tool_holder_num_toggle] && $mom_tool_holder_num_toggle == 1) && \
                        ([info exist mom_tool_holder_num] && $mom_tool_holder_num > 0) } {

########################## TCL Event Handlers ##########################
#
#  sim13_turn_4ax_mm_channel_1.tcl - lathe
#
#    This is a 2-Axis Horizontal Lathe Machine.
#
#  Created by w5ygvn @ Friday, March 03, 2017 2:14:47 PM China Standard Time
...
# 03-May-2014 Allen - Bug fix for PR6999217. Add command PB_CMD_kin_start_of_program_2 and PB_CMD__sync_handle_bufferred_events.
...
#                  Replace MOM_abort with new MOM extension MOM_abort_program.
  ...
  set mom_sys_alt_unit_post_name                "sim13_turn_4ax_mm_channel_1__IN.pui"
...
#=========================
 ...
 set mom_sys_master_head                       "turret1"

 set mom_sys_postname(turret1)                 "$mom_sys_master_post"
 set mom_sys_postname(turret2)                 "sim13_turn_4ax_mm_channel_2"
  ...
  # Initialize current head to the master
   ...
   if { ![info exists mom_sys_postname($NEXT_HEAD)] } {
     ...
     # Execute the closing handler for current post
      ...
      global sim_mtd_initialized
      if $sim_mtd_initialized {
         if [CMD_EXIST PB_VNC_pass_head_data] {
            global mom_sys_sim_post_name
            set mom_sys_sim_post_name [file rootname "$tcl_file"]
            PB_VNC_pass_head_data
         }
      }
...
#
   ...
   if { ![CALLED_BY PB_CMD_handle_sync_event] } {
return
   }
...
#=============================================================
# This command will be executed automatically by the MOM_sync event handler.
#
...
return
   ...
   if { [CMD_EXIST PB_CMD_buffer_sync_event] } {
      global mom_sys_buffer_sync_flag

      PB_CMD_buffer_sync_event
...
#
   ...
   if [CMD_EXIST PB_CMD_reset_sync_code] {
      PB_CMD_reset_sync_code
   }
}
...
#=============================================================
proc PB_CMD_reset_seqnum { } {
#=============================================================
#<12-02-11 gsl>
   global mom_sequence_number mom_sequence_increment mom_sequence_frequency

   SEQNO_RESET
   if { [info exists mom_sequence_number] && [info exists mom_sequence_increment] && [info exists mom_sequence_frequency] } {
      MOM_reset_sequence $mom_sequence_number $mom_sequence_increment $mom_sequence_frequency
   }
}

...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_alt_unit_post_name                "Mikron_UCP_800_Duro__IN.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_post_description                  "Это 3+2 осевой фрезерный станок с двумя поворотными столами.\n\
                                                 Разработал: Черменский М. (battlerom@yandex.ru)\n\
                                                 Обрабатывающий центр: Mikron UCP 800 Duro\n\
                                                 Cтойка: Heidenhain iTNC530 (Conversational)\n\
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   COOLANT_SET ; CUTCOM_SET ; SPINDLE_SET ; RAPID_SET
   PB_CMD_tool_change_5axis
   PB_CMD_tool_change_comment
   MOM_force Once Text T Text S
   MOM_do_template tool_change_1
   PB_CMD_table_restore_5axis
   PB_CMD_linear_move_5axis
   PB_CMD_start_of_operation_force
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_motion X Y Z G_cutcom Text}
   set rapid_spindle_x_blk {G_motion X G_cutcom Text}
   set rapid_spindle_y_blk {G_motion Y G_cutcom Text}
   set rapid_spindle_z_blk {G_motion Z G_cutcom Text}
   set rapid_traverse_blk {G_motion X Y Z G_cutcom S Text M_spindle}
   set rapid_traverse_xy_blk {G_motion X Y G_cutcom S Text M_spindle}
   set rapid_traverse_yz_blk {G_motion Y Z G_cutcom S Text M_spindle}
   set rapid_traverse_xz_blk {G_motion X Z G_cutcom S Text M_spindle}
...
#=============================================================
   ...
   MOM_output_literal "; POSTPROCESSOR REV. 1.1 OCT 2010"
   MOM_output_literal "; POSTPROCESSOR DESIGNED FOR DMU-65 - HEIDENHAIN iTNC530 CONTROL - CONVERSATIONAL"
   MOM_do_template blank_form_1
   MOM_do_template blank_form_2
   MOM_do_template start_of_program_2
   PB_CMD_init_vars_at_start
...
#=============================================================
  ...
  if {![info exist mom_cutcom_status]} {
    set cutter_comp "R0"
  } elseif { ![string compare "OFF" $mom_cutcom_status] } {
    set cutter_comp "R0"
  } elseif { ![string compare "LEFT" $mom_cutcom_status] } {
    set cutter_comp "RL"
  } elseif { ![string compare "RIGHT" $mom_cutcom_status] } {
    set cutter_comp "RR"
  }
}
...
#=============================================================
  ...
  global mom_motion_event   mom_cycle_feed_to  mom_operation_type
  global mom_tool_diameter   mom_cycle_step1
  global cycle_peck_size cycle_type_number cycle_type_coment ;#js
...
# Heidenhain cycle types
  ...
  set cycle_type_number 200 ;#drill
  set cycle_type_coment "DRILLING ~"

  if { ![string compare "bore_move" $mom_motion_event] } {
    set cycle_type_number 202 ;#bore
    set cycle_type_coment "BORING ~"
  }

  if { ![string compare "drill_break_chip_move" $mom_motion_event] } {
    set cycle_type_number 203 ;#drill & break chip
    set cycle_type_coment "UNIVERSAL DRILLING ~"
  }

  if { ![string compare "tap_move" $mom_motion_event] } {
    set cycle_type_number 207 ;#rigid tapping
    set cycle_type_coment "RIGID TAPPING NEW ~"
  }

  if { ![string compare "drill_dwell_move" $mom_motion_event] } {
    set cycle_type_number 240 ;#centering & countersinking
    set cycle_type_coment "CENTERING ~"
  }
 ...
 # peck sizes
  ...
  if { ![string compare "drill_deep_move" $mom_motion_event] || ![string compare "drill_break_chip_move" $mom_motion_event] } {
 ...
 # to ensure the hole Z height Q203 is set correctly.
  ...
  if { ![string compare "TRUE" $cycle_init_flag] } {
    ...
    if { $cycle_type_number == 240 } {
       if { $mom_cycle_step1 == 0} {
             MOM_output_literal "Q343=+0    ;SELECT DIA./DEPTH ~" ;# Q343 SELECT 1 DIA./ 0 DEPTH
        } else {
             MOM_output_literal "Q343=+1    ;SELECT DIA./DEPTH ~"
        }
    }
    ...
    if { $cycle_type_number == 240 } {
      MOM_do_template cycle_block_344    ;# Q344 setup ( engage ) ht
    }
    ...
    if { $cycle_type_number == 240 || $cycle_type_number == 202 } {
        MOM_do_template cycle_block_7    ;# Q211 cycle dwell
    }

    if { $cycle_type_number == 208 } {    ;# only drilling
        MOM_output_literal "Q208=+99999 ;RETRACTION FEED RATE ~"
    }

    MOM_do_template cycle_block_6    ;# Q203 Z of top of hole ( absolute )


#Q204 - nevnjatnii - razobratsja
     set js_return_pos 50.0
    if { ![string compare "AUTO" $mom_cycle_retract_mode] } {
      set js_return_pos [expr $js_prev_pos - $mom_pos(2)] ;# calc incr retract
    }
      if { $cycle_type_number == 240 || $cycle_type_number == 207 } {
          MOM_do_template cycle_block_8_last    ;# Q204 Zreturn after cycle ( incremental )
      } else {
          MOM_do_template cycle_block_8    ;# Q204 Zreturn after cycle ( incremental )
      }

    if { $cycle_type_number == 200 } {
        MOM_do_template cycle_block_7_last    ;# Q211 cycle dwell
    }

    if { $cycle_type_number == 202 } {    ;# only drilling
        MOM_output_literal "Q214=+0    ;DISENGAGING DIRECTN ~"
        MOM_output_literal "Q336=+0    ;ANGLE OF SPINDLE"
    }
...
#=============================================================
proc PB_CMD_init_vars_at_start { } {
#=============================================================
# initializes global vars and flags at start of program


global mom_init_angle_pos_A
global mom_init_angle_pos_C
global tool_change_restored
global prev_tool_number

set mom_init_angle_pos_A 0.0
set mom_init_angle_pos_C 0.0
set tool_change_restored 0
set prev_tool_number -1
}
...
global feed
global last_feed_rate

set last_feed_rate $feed
MOM_output_literal ";$last_feed_rate"
}
...
#=============================================================
proc PB_CMD_linear_move_5axis { } {
#=============================================================
# Rotate tables to angle A, C position (change angle of work plane)
# and made 3d milling on this plane

global mom_init_angle_pos_A
global mom_init_angle_pos_C
global mom_out_angle_pos

set axis_A_rotated 0
set axis_C_rotated 0
set str_01 "PLANE SPATIAL SPA"
set str_02 " SPB0.0000 SPC"
set str_03 " TURN F3600. TABLE ROT"
set axis_A_formatted [format %-10.4f $mom_out_angle_pos(0)]
set axis_C_formatted [format %-10.4f $mom_out_angle_pos(1)]
set string_rotation "$str_01[string trim $axis_A_formatted  ]$str_02[string trim $axis_C_formatted  ]$str_03"
if { $mom_out_angle_pos(0) != $mom_init_angle_pos_A } {
    set mom_init_angle_pos_A $mom_out_angle_pos(0)
    set axis_A_rotated 1
  }
if { $mom_out_angle_pos(1) != $mom_init_angle_pos_C } {
    set mom_init_angle_pos_C $mom_out_angle_pos(1)
    set axis_C_rotated 1
  }
if { $axis_A_rotated == 1 } {
    MOM_output_literal "L Z-50 FMAX M91"
    MOM_output_literal $string_rotation
  }
if { $axis_C_rotated == 1 } {
    if {$axis_A_rotated != 1} {
    MOM_output_literal "L Z-50 FMAX M91"
...
#=============================================================
proc PB_CMD_linear_move_force { } {
#=============================================================
#  MOM_force once G_motion X Y Z M_spindle M_coolant F cutter_comp
...
# Dummy command to log changes in this post --
#
# 04-30-09 gsl - Added 5th axis word to motion events
#
}
...
#=============================================================
global mom_template_subtype mom_template_type

  MOM_force once G_motion X Y Z M_spindle M_coolant F cutter_comp
#MOM_force once G_motion X Y Z F cutter_comp

#  MOM_output_literal "; Operation Type: $mom_template_type"

#  MOM_output_literal "; Operation SUB Type: $mom_template_subtype"
}
...
#=============================================================
proc PB_CMD_table_restore_5axis { } {
#=============================================================
# Rotate tables to its rotated position after tool change

global mom_init_angle_pos_A
global mom_init_angle_pos_C
global tool_change_restored
global last_Z_tool_change
global mom_out_angle_pos

set axis_A_rotated 0
set axis_C_rotated 0
if { $tool_change_restored == 1} {
  set axis_A_formatted [format %-10.4f $mom_init_angle_pos_A]
  set axis_C_formatted [format %-10.4f $mom_init_angle_pos_C]
  set tool_change_restored 0
  if { $mom_out_angle_pos(0) != $mom_init_angle_pos_A } {
    set axis_A_rotated 1
  }
...
#if { $axis_A_rotated != 1 } {
#    MOM_output_literal "L A[string trim $axis_A_formatted  ] C[string trim $axis_C_formatted  ] FMAX"
#    MOM_output_literal "L Z[format %-10.3f $last_Z_tool_change] FMAX"
#  }
if { $axis_C_rotated != 1 } {
    if {$axis_A_rotated != 1} {
    MOM_output_literal "L A[string trim $axis_A_formatted  ] C[string trim $axis_C_formatted  ] FMAX"
...
#=============================================================
proc PB_CMD_tool_change_5axis { } {
#=============================================================
# Rotate tables to 0 before tool change
...
if { $mom_init_angle_pos_A != 0. } {
  set tool_change_restored 1
  set last_Z_tool_change $mom_pos(2)
#  set $mom_coolant_status "OFF"
#  MOM_output_literal ";COOANT STATUS: $mom_coolant_status"
  MOM_output_literal "L Z-50 FMAX M91"
  MOM_output_literal "L A0.0 C0.0 FMAX"
} elseif { $mom_init_angle_pos_C != 0. } {
  set tool_change_restored 1
  set last_Z_tool_change $mom_pos(2)
#  set $mom_coolant_status "OFF"
#  MOM_output_literal ";COOANT STATUS: $mom_coolant_status"
...
#=============================================================
proc PB_CMD_tool_change_comment { } {
#=============================================================
# iserts commentary before tool change
...
if {$prev_tool_number != $mom_tool_number} {
  MOM_output_literal ";--------INSTRUMENT T$mom_tool_number: $mom_tool_name --------"
  MOM_output_literal ";DIAMETER: [format %-3.2f $mom_tool_diameter],  ---  RADIUS KROMKI: [format %-3.2f $mom_tool_corner1_radius]"
  set prev_tool_number $mom_tool_number
 }
}
